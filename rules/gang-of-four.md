...
play_time: "45-60 min"
designer: "Lee F. Yih"
source_pdf: "gang-of-four-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/gang-of-four-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Gang%20of%20Four&game=gang-of-four)
<!-- verification:end -->
...
- 64-card deck:
  - 60 numbered cards ranked 1-10 (2 of each number in 3 colors: Green, Yellow, Red)
  - 1 Multi-colored 1
  - 2 Phoenix cards (1 Green, 1 Yellow)
  - 1 Red Dragon
- 1 rules booklet
- 1 score pad
...
- **Card Exchange:** At the start of each hand (except the first), the loser (most cards remaining) must give their highest card to the winner (first out). The winner then gives any card of their choice back to the loser. Both exchanged cards are shown to everyone. Ties for most cards: the player with the highest total game score gives up their card. If still tied, the player closest to the winner counter-clockwise.
- **Last Card Declaration:** When down to one remaining card, you must declare "Last Card!" Failing to declare: you cannot win that hand; the hand continues among other players, and you add 1 penalty point. Declaration is not required if going out with a multi-card combination.
- **Last Card Blocking:** The player sitting immediately before a "Last Card" player must play their highest-ranking single card if singles are being played, and must open new cycles with multi-card combinations if possible.
- **Passing:** A player may always choose to pass, even if they have playable cards. All play is optional.
- **Discards:** All played cards are left face-up in a pile. No touching or peeking at the discard pile.
- Any player may ask another how many cards they have remaining.
...
| Cards Left | Multiplier | Example |
|---|---|---|
| 1-7 | x1 | 5 cards = 5 pts |
| 8-10 | x2 | 9 cards = 18 pts |
| 11-13 | x3 | 12 cards = 36 pts |
| 14-15 | x4 | 15 cards = 60 pts |
| 16 | x5 | 16 cards = 80 pts |
