...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Glik&game=glik)
<!-- verification:end -->
...
- Game board (grid with starting and home areas for each player)
- Player pieces (multiple per player in distinct colors)
- Blocker pieces (special pieces that stop movement)
...
- **Regular pieces:** Move in one direction (straight line). The piece slides until it hits a wall, the edge of the board, or a blocker piece, at which point it stops.
- **Blocker pieces:** Move according to special movement rules. Blockers serve as obstacles that stop other pieces.
...
- Create stopping points for your own pieces (allowing them to land in desired positions)
- Block opponents' pieces from reaching their homes
- Control key lanes and intersections
