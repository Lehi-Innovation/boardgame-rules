...
source_pdf: "kill-doctor-lucky-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/kill-doctor-lucky-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Kill%20Doctor%20Lucky&game=kill-doctor-lucky)
<!-- verification:end -->
...
1. Deal cards face up one at a time until someone receives a room card. That player goes first; Doctor Lucky starts in that room.
2. All players start in the Drawing Room.
3. Reshuffle the deck and deal 6 cards face down to each player.
4. Place spite tokens in a central pool.
...
| Card Type | Effect | After Use |
|-----------|--------|-----------|
| Move card | Move self or Doctor Lucky up to N rooms | Discard pile |
| Room card | Move self or Doctor Lucky to named room | Discard pile |
| Weapon card | Murder attempt with stated value (may have room bonus) | Discard pile |
| Failure card | Foil a murder attempt (values 1, 2, or 3) | Removed from game |
