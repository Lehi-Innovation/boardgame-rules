...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/pegity-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Pegity&game=pegity)
<!-- verification:end -->
...
- 1 pegboard with a grid of holes
- Colored pegs (one color per player)
...
- A row of five must be uninterrupted -- no gaps or opponent pegs may break the line.
- Pegs cannot be moved once placed.
- If the board fills completely with no five-in-a-row, the game is a draw.
- Every peg from Pegity may be prepared from the boards by twisting and pulling them free.
- Partners variant: partners cooperate strategically but cannot communicate about their planned moves.
