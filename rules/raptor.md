...
source_pdf: "raptor-rules.pdf"
extracted_date: "2026-03-27"
summarized_date: "2026-03-27"
rulebook_version: "Matagot (2015)"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/raptor-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Raptor&game=raptor)
<!-- verification:end -->
...
- **Raptor wins** if 3 baby raptors escape the board OR all scientists are eliminated.
- **Scientist wins** if 3 baby raptors are captured OR the mother raptor has 5 sleep tokens on her.
...
1. Choose an environment (jungle or savannah) — flip all 10 tiles to that side.
2. Place the 6 square tiles randomly in a 2×3 rectangle.
3. Place the 4 L-shaped tiles randomly along the short sides of the rectangle.
4. Place 3D rocks on every space that does **not** show a circle or half-circle.
5. **Raptor player** places figurines first:
   - Mother raptor on one of the two central tiles.
   - One baby raptor on each of the remaining 5 tiles.
6. **Scientist player** places figurines second:
   - One scientist on a space of their choice on each of the 4 L-shaped tiles.
   - Remaining 6 scientists form the reserve beside the player aid.
   - Place 5 sleep tokens on the player aid (ammunition side up).
7. Each player shuffles their 9-card deck, draws 3 cards. First round begins.
...
1. **Simultaneously select**: Each player secretly chooses 1 of their 3 hand cards, places it face-down.
2. **Reveal**: Both cards are flipped simultaneously.
3. **Resolve**:
   - **Same value** → both cards have no effect; the round ends immediately.
   - **Different values** → the player with the **lower** card applies its **card effect**. Then, the player with the **higher** card receives **action points** equal to the difference (higher − lower). The higher-card player does **not** apply their card's effect.
4. **Draw**: Each player draws 1 card to return to 3 in hand. Played cards stay visible so opponents can track what's been played.
5. **Deck exhaustion**: When a player's deck is empty, shuffle all played cards into a new deck, then draw to 3.

---

## Actions

### Raptor Action Points
...
- **Raptor wins**: 3 babies escaped OR 0 scientists on the board.
- **Scientist wins**: 3 babies captured OR mother has 5 sleep tokens.
...
- **No diagonal movement or shooting** — everything is orthogonal only.
- **One figurine per space** — no stacking, no moving through occupied spaces.
- **Exit half-spaces** — only baby raptors can move there (via the "move a baby" action). No other figurine or token may be placed there.
- **Wound penalty timing** — when the mother moves, lose AP equal to sleep tokens **before** the first move, not per move.
- **Aggressive action limit** — each individual scientist can only shoot OR capture once per round. Moving is not aggressive.
- **Same-round restrictions** — a baby put to sleep this round cannot be woken this round. A scientist frightened this round cannot stand up this round.
- **Killed scientists are gone** — returned to the box, not the reserve. They cannot be brought back with Reinforcement cards.
- **Fire chain removal** — when the mother puts out a fire, remove the target token plus all fire tokens orthogonally connected to it (cascading).
- **Jeep extinguishes fire** — when a Jeep moves through a fire token, that fire is removed.
- **Sleeping Gas range** — targets a baby on the same tile as a scientist or on an orthogonally adjacent tile (not same space — same tile or adjacent tile).
- **Reinforcements placement** — long edges of the board only, on square tiles. Cannot place on L-shaped tiles.
- **Card 2/6 (Disappearance)** — the mother is off the board while the scientist spends action points. She is placed back afterward. The observation effect (seeing opponent's card) lasts for the **next** round only.
- **Recovery cards** — the effect can be split between removing sleep tokens from the mother and waking baby raptors.
- **Obstacles differ for movement vs. shooting**: Rocks and active scientists block both. Fire tokens and raptors block movement but not shooting. Frightened scientists block neither.
