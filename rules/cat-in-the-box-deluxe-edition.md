...
source_pdf: cat-in-the-box-deluxe-edition-rules.pdf
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/cat-in-the-box-deluxe-edition-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Cat%20in%20the%20Box%3A%20Deluxe%20Edition&game=cat-in-the-box-deluxe-edition)
<!-- verification:end -->
...
- 45 number cards (values 1-5, no suit printed)
- 1 research board (grid: 4 suits x 5 numbers)
- Player tokens (to mark claimed number-suit combos)
- Score track
- Trump indicator
- Round cards
...
1. Place the research board in the center.
2. Each player takes a set of player tokens.
3. Deal cards equally to each player (hand size depends on player count). Each player also places one card face-down as a prediction card.
4. Clear the research board at the start of each round.
...
1. **Which card to play** (choose a number from your hand).
2. **What suit to declare** (place your token on the matching grid space).
...
- **Trump suit:** One suit is designated as trump each round. Trump cards beat non-trump cards regardless of number.
- **You may declare any available suit** when you cannot follow the lead suit, including trump (which is how trumps enter play).
- **Paradoxes end the round immediately** for all players.
- **Connected groups** are only orthogonally adjacent tokens (not diagonal) of the same player on the research board.
- With 2 players, use modified rules with fewer cards and a dummy hand.
...
**Key rule:** You declare suit when you play — cards have no inherent suit
**Key constraint:** No two players can claim the same number-suit space
