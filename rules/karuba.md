...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/karuba-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Karuba&game=karuba)
<!-- verification:end -->
...
1. Each player takes an island, 4 adventurers, 4 temples, and 36 jungle tiles.
2. Set up temple treasure stacks by color, sorted in descending value. Stack sizes depend on player count:
   - 2 players: treasures worth 5, 3
   - 3 players: treasures worth 5, 3, 2
   - 4 players: treasures worth 5, 4, 3, 2
3. Place crystals and gold nuggets in the center of the table.
4. **Adventurer and Temple placement:** Players cooperatively decide positions. Place adventurers on beach numbers and temples on jungle numbers (minimum 3 numbers distance between matching colors). ALL islands must have identical placement.
5. **Expedition leader:** Shuffles their 36 tiles face-down into a stack. Other players sort their tiles face-up by number around their island.

## Turn Structure

Each turn:
1. Expedition leader reveals the top tile and calls out its number.
2. All players find the matching tile simultaneously.
3. Each player chooses Option A or Option B for that tile.
...
|---------------|--------|
| Temple treasure | 2, 3, 4, or 5 (varies) |
| Gold nugget | 2 |
| Crystal | 1 |
...
|-------------|-------|
| Tiles per player | 36 |
| Adventurers per player | 4 |
| Temples per player | 4 |
| Min distance adventurer-temple | 3 positions |
| Movement from discarded tile | 2-4 steps |
