...
source_pdf: "fire-and-ice-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/fire-and-ice-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Fire%20and%20Ice&game=fire-and-ice)
<!-- verification:end -->
...
- Wooden game board with 7 raised triangular islands (each island has 7 holes)
- 25 red "Fire" pegs
- 25 aqua "Ice" pegs
...
- **Same Island** — Move your peg to any open hole on the same island.
- **Same Position** — Move your peg to the same relative position on a different island (e.g., from "left corner" on one island to "left corner" on another island). The destination hole must be empty.
...
- **Board geometry:** Each island has 7 holes connected in 7 groups of 3 (Fano Plane). The 7 islands are also connected to each other in the same pattern.
- **Mutual exclusion:** If you control an island (three pegs in a connected line), your opponent cannot simultaneously hold a controlling triplet on that same island.
- **Extra pegs on controlled islands:** If you have additional pegs beyond the controlling three on an island, you can safely move them elsewhere while retaining control.
- **Handicap variant:** If one player is stronger, place a few of the weaker player's pegs on the board at setup (no more than one per island).
- **Solitaire variant:** Place 14 pegs of each color randomly. Alternate colors, moving pegs by the same rules but each move must capture and replace an opposite-color peg (no moves to empty holes). Win by reducing to one peg.
