---
title: "Husker Du?"
bgg_id: 14423
player_count: "2+"
play_time: "15 min"
designer: "Uncredited"
source_pdf: "h-sker-d-rules.pdf"
extracted_date: "2026-03-20"
summarized_date: "2026-03-20"
verification: "verified"
verification_date: "2026-06-14"
---
<!-- verification:begin -->
> ✅ **Verified** — this summary was fact-checked against the rulebook text and no significant issues were found.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/h-sker-d-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Husker%20Du%3F&game=h-sker-d)
<!-- verification:end -->


## Overview

Husker Du? (Danish for "Do you remember?") is a classic memory matching game. Players take turns removing pegs from a game platform to uncover hidden pictures. By matching pairs of identical pictures, players collect pegs. The player who collects the most pegs wins.

## Components

- 1 Cardboard Game Platform with Picture Wheel (rotatable for 18 different layouts)
- 36 Plastic Game Pegs
- Illustrated Instructions

## Setup

1. Place all 36 playing pegs into the 36 openings on the gameboard, covering all the pictures.
2. Turn the wheel clockwise so the arrows on the gameboard line up with the next arrow on the wheel.
3. There are 18 different positions of the wheel, making memorization virtually impossible.

## Turn Structure

1. The youngest player goes first.
2. Randomly remove 2 pegs from the gameboard.
3. **If the uncovered pictures match:** Keep both pegs and continue by removing 2 more pegs. A player's turn continues as long as matches are made.
4. **If the uncovered pictures do not match:** Replace both pegs. Play passes to the next player on the left.
5. Play continues until all pairs have been uncovered.

## Actions

- **Remove pegs:** Take 2 pegs from the board to reveal hidden pictures
- **Collect pegs:** Keep matched pairs
- **Replace pegs:** Put pegs back if pictures do not match

## Scoring / Victory Conditions

- The game ends when all pegs are removed from the board.
- Players count the pegs they have collected.
- The player with the most pegs wins.

## Special Rules & Edge Cases

- The picture wheel can be turned to any of 18 positions, ensuring a different layout each game.
- There is no limit on the number of consecutive matches a player can make in one turn.
- To start a new game, cover all openings with pegs and turn the wheel clockwise to the next arrow.

## Player Reference

| Item | Count |
|------|-------|
| Total pegs | 36 |
| Total picture pairs | 18 |
| Wheel positions | 18 |
| Pegs removed per attempt | 2 |
