...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Mesopotamia&game=mesopotamia)
<!-- verification:end -->
...
1. Build the starting map from modular hex tiles.
2. Each player places their starting tribe members.
3. Place resource tokens on appropriate hexes.
4. Set up the offering requirements.

## Turn Structure

On your turn:
1. **Move** your tribe members across the hex map.
2. **Gather resources** from hexes where your members are located.
3. **Build** huts or other structures using resources.
4. **Make offerings** to the gods at sacred sites.
