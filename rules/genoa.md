...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/genoa-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Genoa&game=genoa)
<!-- verification:end -->
...
- 1 game board (city grid with 18 buildings, streets, Market, and Park)
- 1 Trade tower (5 stackable discs)
- 1 round marker
- 2 eight-sided dice (1 red, 1 blue)
- 40 Commodity tiles (5 each of 8 types: pepper, saffron, silver, copper, linen, silk, ale, wine)
- 35 wooden Ownership markers (7 per player color)
- 27 special tiles (Starting space, Trade 1:1, Commodity of your choice, Building action, Extra action)
- 60 cards: 14 Contract, 14 Message, 16 Large order, 16 Small order
- 80 coins (5, 10, 50, and 100-ducat denominations)
- 1 first player marker, 5 player tiles
...
- Each building/street/Market = 1 space
- No space visited twice per turn
- Merchant may stop early or stay on the starting space
...
3. Count all ducats. Wealthiest player wins. Tiebreaker: most goods (cards, commodities, special tiles).

**Note:** Cards, commodities, and special tiles have no value at game end (only Contract cards score).

## Special Rules & Edge Cases

- **2-player variant:** Offers to the merchant must always include at least 1 good (not just ducats). The merchant gets 2 actions if the other player takes at least 1 action.
- The first player is the same throughout the entire game.
- Players keep ducats hidden (in hand or stacked).
- Small orders do not require using your action to complete — you just need to take the building's action shown on the card (which can be negotiated to someone else visiting that building... no, you must be the one taking the action).
- Cannot deliver two orders at the same building in the same turn.
- Messages can be delivered in any building order; multiple messages can be delivered in the same turn.
- The Market space triggers the round marker moving down only when it's the starting space (not when the tower passes through).
- The last round is always played completely, even if Market is rolled during it.
...
**Key Payouts:**
- Building ownership income: 10 ducats/use + 10 ducats at game end
- Large order: 100 ducats + 1 special tile
- Small order: 40 ducats
- Message: 30 ducats
- Contracts: 10-150+ ducats based on adjacency
