...
source_pdf: "arcs-rules.pdf"
extracted_date: "2026-03-02"
summarized_date: "2026-03-02"
rulebook_version: "1st Edition (August 27, 2025)"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/arcs-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Arcs&game=arcs)
<!-- verification:end -->
...
### Map & Markers
- 1 map board (6 clusters, each with 3 planets and 1 gate)
- 1 chapter marker (tracks current chapter, 1-5)
- 3 ambition markers (double-sided: blue/lower-number side and higher-number side)
- 1 zero marker (covers lead card number when ambition is declared)
- 1 initiative marker (shows which player leads each round)
- 4 path markers (2 long, 2 short; cover gates of out-of-play clusters and connect neighboring gates)
- 6 out-of-play markers (cover planets in out-of-play clusters)
- 4 aid sheets

### Player Pieces (per player, 4 sets)
- 1 player board (holds resources, unbuilt cities, Trophies, and Captives)
- 15 ships (can be fresh/upright or damaged/tipped)
- 5 cities (placed from leftmost slot on player board; can be fresh or damaged/flipped)
- 5 starports (built on planets; can be fresh or damaged/flipped)
- 10 agents (placed on Court cards or used for Outrage)
- 2 Power markers (1 for main tracking, 1 for expansions/negative or very high Power)

### Cards
- 28 action cards (numbered 1-7 in 4 suits; "1" and "7" cards used only in 4-player games)
- 12 setup cards (4 each for 2, 3, and 4 players)
- 25 Guild cards (gained from the Court; have suit, rules, and raid cost)
- 6 Vox cards (gained from the Court; resolve immediately)
- 8 leader cards (used in Leaders & Lore mode)
- 14 lore cards (used in Leaders & Lore mode)

### Other Pieces
- 25 resource tokens (5 each of Material, Fuel, Weapons, Relics, Psionic)
- 18 battle dice (6 Assault, 6 Skirmish, 6 Raid)
...
- The lead card's **suit** determines which actions are available.
- The lead card's **action pips** (number in top-left corner) determine how many actions may be taken.

**Action suits and their available actions:**
| Suit | Available Actions |
|---|---|
...
- "1" cards have no ambition (4 players only).
- "2" through "6" cards let you declare the matching ambition.
- "7" cards let you declare any ambition (4 players only).
...
- Playing an extra action card face down alongside their played card (no actions from the extra card), OR
- Surpassing with a "7" card (4-player games only; you still get its actions).
...
- If no one seized, the initiative goes to the player who **Surpassed with the highest card number**.
- If no one Surpassed, the initiative stays with its current holder.
...
- **Captives:** Place captured agents in the Captives box on your player board. They add to the Tyrant ambition.

### Build (Construction)

You may build either a building or a ship:

- **Building:** Place 1 starport or city in an empty building slot in a system where you have a Loyal piece. When building a city, use the leftmost city on your player board (uncovering resource slots and Power bonuses).
...
| **Raid** | Steals resources/cards from the defender, might damage them, but risky |
   ...
   - You may collect fewer dice than attacking ships, and you may mix types, but you cannot collect more than 6 dice of a single type.
   - **Raid Dice Limit:** You can only collect raid dice if there are defending buildings OR if the defender has no Loyal buildings anywhere on the map.

4. **Roll and Resolve Dice:** Roll all collected dice and resolve all faces in this strict order:
...
- **First place** gains the higher Power shown on all ambition markers in that box.
- **Second place** gains the lower Power shown on all ambition markers in that box.
- **Ties for first:** Tied players get second-place Power only. Other players gain nothing.
- **Ties for second:** Tied players gain no Power.
- **Qualifying:** You cannot gain Power from an ambition if you have none of the thing it counts (e.g., if Warlord is declared and you have no Trophies, you gain nothing).
...
- +2 extra Power if the "+2 to won ambitions" city slot is uncovered.
- +5 extra Power total (not +7) if both the "+2 to won ambitions" and "+3 to won ambitions" city slots are uncovered.
...
- **Suit:** Matches 1 of the 5 resource types. Material and Fuel Guild cards add to Tycoon. Relic cards add to Keeper. Psionic cards add to Empath. Weapon cards do not add to any ambitions.
- **Rules:** Grant new actions, modifiers, or Prelude actions.
- **Raid Cost:** How many raid icons an attacker must spend in battle to steal the card.
...
- **New Actions:** Cards may grant a new action followed by a standard action name in parentheses, e.g., "Assassinate (Battle)." When you would take the standard action, you may instead take the new action.
- **Modifiers:** Cards can modify standard actions (shown in bold text). Modifiers do not modify new actions but do modify standard actions contained within new actions.
...
- Tax Rival cities there
- Capture Rival agents
- Restrict how their ships move
- Cause pieces they build there to be placed damaged

### Damage and Destruction

- **Fresh pieces:** Ships standing upright, buildings on their lighter/fully colored side.
- **Damaged pieces:** Ships tipped over, buildings flipped to their darker side.
- A fresh piece hit becomes damaged. A damaged piece hit is destroyed (removed from the map).
...
- Cards supersede the rulebook, but any rule using the term "cannot" is absolute and cannot be overridden unless explicitly named using "ignore."
- Sidebars, italics, and parenthetical text are clarifications, superseded by core rules they reference.
...
- Do not use "1" and "7" action cards.
- 2 clusters are out of play (vs. 1 for 4 players).
- Court row starts with 3 cards (vs. 4 for 3-4 players).
- 6 resources placed on ambition boxes act as a phantom third player for scoring.
- The non-initiative player gets a mulligan after each card draw (may discard and redraw 6 cards once).
...
- Chapters per game: up to 5
- Cards per hand: 6
- Action cards in deck: 20 (2-3 players) or 28 (4 players)
- Court row size: 3 cards (2 players) or 4 cards (3-4 players)
- Starting pieces per player: 3 ships + 1 city (system A), 3 ships + 1 starport (system B), 2 ships (system C)
- Starting resources: 2 (matching planet types of A and B systems)
- Max pieces per player: 15 ships, 5 cities, 5 starports, 10 agents
- Battle dice: 18 total (6 Assault, 6 Skirmish, 6 Raid)
- Resource tokens: 25 total (5 each of 5 types)
