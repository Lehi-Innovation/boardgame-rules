...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/okiya-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Okiya&game=okiya)
<!-- verification:end -->
...
- 16 garden tiles (each featuring one of 4 vegetation types and one of 4/5 symbols)
- 8 character tokens per player (2 sets of 8, distinguished by background pattern)
...
- The **same vegetation** (Maple Leaves, Cherry Tree, Pine Tree, or Iris), **OR**
- The **same symbol** (Rising Sun, Poem, Bird, Rain, or Cloud)
...
- The starting player's first move is restricted to the **outside border** tiles only (not the 4 center tiles).
- If no remaining garden tile matches the current constraint (same vegetation or same symbol), the active player **loses** because they are blocked.
- Garden tiles removed stack on top of each other; only the topmost tile matters for the current constraint.
