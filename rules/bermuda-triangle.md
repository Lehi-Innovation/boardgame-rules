...
player_count: "2-4"
play_time: "45-60 min"
source_pdf: "bermuda-triangle-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/bermuda-triangle-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Bermuda%20Triangle&game=bermuda-triangle)
<!-- verification:end -->
...
- 1 cloud with rotating base and cylinder with fins
- Small magnets (1 per ship)
- Spinner (with 3 rings: inner letters A-H, middle numbers, outer directions N/S/E/W)
...
1. Insert a small magnet into each ship (yellow side of magnet face up).
2. Assemble the cloud onto its base (art side up over the rim).
3. Each player receives 4 ships of one color and places 1 ship in a port space of each port.
4. Sort freight cards by product type, shuffle each pile, place money side up behind the corresponding port.
5. Place home port cards in a pile behind the "Home Port."
6. Place the cloud with its locater on the center red circle, notch pointing North, cloud's "A" on the notch.
7. High roll of dice goes first; play proceeds clockwise.

## Turn Structure

Each turn has two phases:

1. **Ship Movement:** The active player rolls the dice and moves one of their ships the number of spaces rolled.
2. **Cloud Movement (after all players have moved):** After all players have taken a turn, the spinner is spun and the cloud is rotated and moved.
...
### Cloud Movement
1. The cloud notched pointer must always point exactly **North**.
2. **Rotation:** The inner ring letter on the spinner determines cloud rotation. Turn the cloud clockwise until that letter aligns with the notch.
3. **Movement:** The middle ring gives the number of dots to move; the outer ring gives the direction (N, S, E, W). If the spinner shows the full word (e.g., "North"), the cloud travels to the last dot in that direction.
4. The cloud locater never moves past any outside dot on the grid.
...
|------|--------|
| Ships per player | 4 |
| Ports | Multiple (with specific products) |
| Freight types | Sugar, Oil, Bananas, Lumber |
| Home port bonuses | 8 cards total |
| Win condition | $350,000 in cards or highest value when a player loses all ships |
| Port capacity | Equal to number of players |
