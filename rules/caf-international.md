...
source_pdf: "caf-international-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/caf-international-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Cafe%20International&game=caf-international)
<!-- verification:end -->
...
- 1 game board depicting the cafe with 24 tables and a bar
- 100 guest tiles (depicting men and women of 12 nationalities)
- 50 red chips (worth 1 point each)
- 16 black chips (worth 5 points each)
- 15 white chips (worth 10 points each)
- 1 cloth bag

## Setup

1. Place the game board in the middle of the table.
2. Place the 100 guest tiles in the bag and shuffle.
3. Place chips near the game board by value.
4. Each player draws 5 guest tiles from the bag (keep hidden from others).

## Turn Structure

On your turn, you must perform one of the following:

1. **Place 1 or 2 guests at tables** — Score points based on placement.
2. **Place 1 guest at the bar** — Score negative points.
...
- A table can seat a maximum of **2 ladies and 2 gentlemen**.
- Guests must match the **nationality** of the table, OR be seated with a guest from the same nationality but different table.
- The allowed combinations at any table:
  - 1 man and 1 woman
  - 2 men and 1 woman (or vice versa)
  - 2 men and 2 women
  - Other arrangements following nationality rules
...
- Each guest at the bar costs the player negative points.
...
- A player places a guest and the **last bag tile** has been drawn.
- A player has no legal placement available and cannot refill hand.
- A player places the **last bar** seat.
- A player draws the last guest tile from the bag and cannot draw one more.
...
- Players may not look at tiles in the bag.
- A guest placed between two tables scores for both.
- Once a table is full (4 guests), no more guests can be added.
- Players must place at least 1 guest per turn if legally possible.
- If no legal table placement is available, the player must place a guest at the bar.
- Joker exchange does not count as a turn action — it happens in addition to normal placement.
