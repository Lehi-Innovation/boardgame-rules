...
source_pdf: "roll-for-it-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Roll%20For%20It%21&game=roll-for-it)
<!-- verification:end -->
...
- 30 target cards (each showing a specific combination of dice values and a point value)
- 24 dice (6 dice in each of 4 colors)
...
- You can only match dice to cards, never remove individual dice from a card (must take all back at once with the Take Back action).
- Multiple players can compete for the same card; only the player who completes all images first scores it. The other player's dice are returned.
- If the deck runs out, play continues with remaining face-up cards until someone reaches 40 points.
- The Take Back action is all-or-nothing; you cannot selectively retrieve dice from specific cards.
- With 5+ players, combine two sets (60 cards, 48 dice) and reveal 4 cards instead of 3.
