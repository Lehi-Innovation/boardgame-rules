---
title: "Burg der 1000 Spiegel"
bgg_id: 40962
player_count: "2-4"
play_time: "20-30 min"
designer: "Inka Brand, Markus Brand"
source_pdf: "burg-der-1000-spiegel-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Burg%20der%201000%20Spiegel&game=burg-der-1000-spiegel)
<!-- verification:end -->
...
2. Insert all treasure windows into their slots around the castle, with one slot left open.
3. Place the mirrors in the center of the board in their starting positions.
4. Place the player pawn on the starting space.
5. Give each player a starting supply of jewels (or place the jewel supply nearby).

## Turn Structure

On each turn, the current player:
1. Chooses any treasure window and lifts it out (showing it to all players), then swaps it with the open window position.
2. Rolls the dice and moves the pawn that number of spaces clockwise.
3. The space landed on indicates which treasure the player must find by looking through the corresponding window.
4. The player may optionally move up to 3 mirrors in the center of the board (must move at least 1).
5. The player looks through the window to try to spot the required treasure reflected in the mirrors.
6. If successful, the player earns jewels.
...
|---|---|---|
| 1 | Swap a treasure window with the open slot | Yes |
| 2 | Roll dice and move pawn | Yes |
| 3 | Move 1-3 mirrors | Yes (at least 1) |
| 4 | Look through window for target treasure | Yes |
