...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Chapayev&game=chapayev)
<!-- verification:end -->
...
- 1 standard checkerboard (8x8)
- 8 dark-colored pieces (checkers)
- 8 light-colored pieces (checkers)
- Pieces should be smaller than the squares for proper play

## Setup

1. White pieces are placed on the first row (closest to the White player).
2. Black pieces are placed on the last row (closest to the Black player).
3. White opens the game (goes first).
...
- If you successfully knock an opponent's piece off the board AND keep all your pieces on the board, you may flick again (continue your turn).
- If you fail to knock off an opponent's piece, your turn ends and the opponent goes.
- If you knock one of your own pieces off the board, your turn ends immediately.
...
- **Traitor rule:** If a piece is flipped upside-down after an impact, it becomes a "traitor" and control of it passes to the opponent.
- Pieces that land exactly on a line between squares are typically returned to the board at the nearest valid position.
- If both players lose their last piece simultaneously, the round is typically replayed.
- Standard checkers pieces work well, though any small disc-shaped pieces can be used.
