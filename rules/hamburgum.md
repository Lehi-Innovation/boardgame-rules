...
source_pdf: "hamburgum-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/hamburgum-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Hamburgum&game=hamburgum)
<!-- verification:end -->
...
- 1 game board (City Hall Square, Binnenalster, Speicherstadt, Harbor, Stock Exchange areas)
- 1 "Planten un Blomen" display board for cards
- 280 playing cards (12 each of 22 categories in 5 colors, plus 20 Zoo and 20 Park cards)
- 75 workers (15 each in purple, orange, grey, pink, brown)
- 30 "1 mark" coins and 10 "3 marks" coins
- 10 intrigue tokens
- 50 wall segments
- 6 dice (5 matching card colors + 1 black)
- 4 time tokens
- 40 activation markers
- 8 clerics
- 8 "Hans Hummel" statues with stands
- Per player: 1 player board, 1 "100/200" tile, 1 city tower, 2 player pawns, 5 majority tokens, 2 summary cards, 15 disaster level markers
...
- **Disasters:** When all 3 markers of a color reach the warning triangle, the disaster triggers immediately: Decay (lose 1 wall segment from an end), Fire (discard 1 building), Unrest (lose all money), Disease (lose all workers), Flood (discard 1 construction site; return any building on it to hand). After resolving, reset all 3 markers left (no VP for this reset).
- **Majority rules:** A player must have the exclusive majority (more than all others, no ties) to flip their token. Once flipped, it stays active for the rest of the game even if the majority is later lost. Multiple players can have the same majority active.
- **Cards trump rules:** If card text contradicts the rulebook, the card takes priority.
- Workers and coins are unlimited; use substitutes if needed.
- If a draw pile is exhausted, no more cards of that color can be drawn.
- When reaching the City Hall Square's last space (18 VP), instead of advancing further, earn 4 VP each time advancement would occur.
