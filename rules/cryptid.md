...
source_pdf: "cryptid-rules.pdf"
extracted_date: "2026-03-02"
summarized_date: "2026-03-02"
rulebook_version: "1st Edition (2018)"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/cryptid-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Cryptid&game=cryptid)
<!-- verification:end -->
...
- 6 numbered map tiles (each divided into hexagonal spaces with five terrain types: desert, forest, water, mountain, swamp)
- 4 standing stones (octagon-shaped, in white, green, blue, and black)
- 4 abandoned shacks (triangle-shaped, in white, green, blue, and black)
- 1 pawn (used to indicate questioned/searched spaces)
- 5 sets of player tokens, each set in a matching colour:
  - Cubes (indicate "this space could NOT be the habitat according to my clue")
  - Discs (indicate "this space COULD be the habitat according to my clue")
- 5 clue books (one per player)
- 54 cards (clear-bordered cards for normal mode, black-bordered cards for advanced mode)
...
- You may place a piece on a space containing a structure.
- You **cannot** place a piece on a space containing another player's cube.
...
- **Honesty is mandatory.** You must always place cubes and discs truthfully according to your clue.
- **Pieces are never removed from the board** (unless correcting a mistake -- see below).
- **You may never interact with a space that contains any cube** (yours or any other player's). You cannot question that space, search that space, or place any of your pieces on it.
- **If someone places a cube on your turn, you must also place a cube.** This applies whether you questioned or searched. Your cube goes on a different space that could not be the habitat according to your clue.
- **No duplicate pieces on a space.** If one of your pieces (cube or disc) is already on a space, you cannot add another of your pieces to that space.

### Cube and Disc Placement Details

- A space can contain **only one cube** (from any player). Once a cube is on a space, no other pieces can be placed there.
- A space **can** contain **multiple discs** from different players (stack them so the space remains visible).
- You may place pieces on spaces that contain structures.
...
- **Agreement required:** All players must agree, OR all but one player if playing with 4 or 5 players.
- **Using cards:** Check the number on the game card and look it up in the Hints section in the back of the rulebook. Hints reveal which categories of clues are NOT in play (e.g., "no clues that mention any animal territory," "no within one space clues").
- **Using the app:** Follow the in-app instructions.

### Distance and Adjacency

- "Within X spaces" means counting by hexagonal adjacency. Any element is **zero** spaces from itself.
- "Within one space of forest" includes every forest space AND every space touching a forest space.
- "Within two spaces of a standing stone" includes the standing stone's space AND every space within two hex-steps of it.
...
| Action | Procedure |
|---|---|
| **Question** | Place pawn on any space. Ask one other player "Could the creature be here?" They place a disc (yes) or cube (no). If they placed a cube, you must also place a cube elsewhere. |
| **Search** | Place pawn on a space that could be the habitat per your clue. Place your disc. Ask each player clockwise. If anyone places a cube, search fails and you place a cube. If no one does, you win. |

### Piece Meanings
| Piece | Meaning |
|---|---|
| Cube | "This space could NOT be the habitat" (according to that player's clue) |
| Disc | "This space COULD be the habitat" (according to that player's clue) |

### Space Restrictions
- A space with any cube: no further interaction allowed (no questioning, searching, or placing pieces)
- A space can hold multiple discs from different players (stack them)
- A space can hold at most one cube total
- You cannot have two of your own pieces on the same space
...
### Key Numbers
- Players: 3-5
- Initial sharing: 2 cubes per player before first turn
- Map tiles: 6
- Structures: 4 standing stones + 4 abandoned shacks (3 of each in normal mode)
- Structure colours: white, green, blue, black
- Terrain types: desert, forest, water, mountain, swamp
- Animal territories: bear, cougar
- Cards in deck: 54 (clear border = normal, black border = advanced)
- Hint agreement: all players, or all but 1 (with 4-5 players)
