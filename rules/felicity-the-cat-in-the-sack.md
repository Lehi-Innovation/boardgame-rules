...
source_pdf: "felicity-the-cat-in-the-sack-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/felicity-the-cat-in-the-sack-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Felicity%3A%20The%20Cat%20in%20the%20Sack&game=felicity-the-cat-in-the-sack)
<!-- verification:end -->
...
**For 4-5 players:**
1. Each player takes 1 set of 10 Pet Cards in their color.
2. Each player randomly draws 1 Pet Card from the player to their right and discards it unseen to the box (so each player has 9 cards).
3. Each player takes 15 Mice Money (1x5 + 10x1).
4. Create a bank: 33 mice for 5 players, 27 mice for 4 players. Return excess to box.
5. Lay out Mouse Cards in numerical order. For 4 players, remove the "3" Mouse Card.
6. Place the Cat in the Sack Card to the left of the lowest Mouse Card.
7. Place Mice Money from the bank on each Mouse Card equal to its value (e.g., 6 mice on the "6" card).
...
| Players | 3-5 |
| Rounds | 9 |
| Starting Mice | 15 per player |
| Cat values | -8, -5, 3, 5, 8, 11, 15 |
| Mouse Card values | 2, 3, 4, 6 (remove 3 for 4p) |
| Dog effect | Large: removes highest positive cat; Small: removes highest negative cat |
| Win condition | Most points (cat values + remaining mice) |
