...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/pig-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Pig&game=pig)
<!-- verification:end -->
...
- 1 set of specially designed Pig Dice (or standard dice can be used)
- 1 Dice Shaker
...
- **Standard Pig (1 die):** Rolling a 1 ends your turn with 0 points for that turn. Any other number adds to your running total.
- **Two-die Pig variant:** Various combinations have special scoring:
...
- A player who reaches 100 points first does NOT automatically win; all other players get a final turn.
- After reaching 100, the leading player may continue rolling to increase their lead, but risks losing their turn score.
- If a Head and Tail turn up on the first roll, there is no score (nothing to double), so roll again.
- If a 7 turns up on the first roll, there is no score, so the die must be passed to the next player.
- Multiple players may exceed 100 in their final turns; highest score wins regardless.
