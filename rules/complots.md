...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Complots&game=complots)
<!-- verification:end -->
...
- Character cards: 6 different characters, with multiple copies of each (typically 3 copies per character for 3-6 players, 4 copies for 7-8 players)
- Coins/tokens (treasury)
- Reference cards
...
### Blocking
- Certain actions can be blocked by claiming to have the blocking character.
- The player whose action is blocked can challenge the blocker's claim.

### Challenging
- When a player claims a character (for an action or block), any other player may challenge.
- **Challenge succeeds (bluff caught):** The challenged player loses 1 influence (turns over a card).
- **Challenge fails (claim was true):** The challenged player reveals the claimed card, shuffles it into the court deck, and draws a replacement. The challenger loses 1 influence.
...
- If you have 10 or more coins at the start of your turn, you **must** coup — no other action is allowed.
- When you lose influence, you choose which of your face-down cards to turn face-up. Once revealed, it has no further effect.
- If an assassination is challenged and the challenge fails, the target still loses an influence to the assassination (potentially losing 2 total).
- If an assassination is successfully blocked by Contessa, the 3 coins spent are still lost.
- The Inquisitor replaces the Ambassador in some game configurations.
- Complots differs from standard Coup by including the Inquisitor character and supporting up to 8 players.
- With 2 players, a variant rule gives each player 1 coin at start instead of 2.
