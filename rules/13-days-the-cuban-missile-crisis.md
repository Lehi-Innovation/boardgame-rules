...
player_count: "2"
play_time: "45 min"
designer: "Daniel Skjold Pedersen, Asger Sams Granerud"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/13-days-the-cuban-missile-crisis-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%2013%20Days%3A%20The%20Cuban%20Missile%20Crisis&game=13-days-the-cuban-missile-crisis)
<!-- verification:end -->
...
- 1 Game board
- 39 Strategy cards (13 UN, 13 US, 13 USSR)
- 1 Personal Letter card
- 13 Agenda cards
- 34 Influence cubes (17 red, 17 blue)
- 6 DEFCON markers (3 red discs, 3 blue discs)
...
- **Television:** Dominating player may escalate or deflate one of their DEFCON tracks by 1 step.
- **United Nations:** Dominating player receives the Personal Letter card. No effect if they already hold it.
- **Alliances:** Dominating player draws the top Strategy card and adds it face down to the Aftermath stack or discards it face up.
...
- Any of their DEFCON markers is in the DEFCON 1 area, OR
- All 3 of their DEFCON markers are in the DEFCON 2 area.
...
- **DEFCON Effect:** Placing/removing cubes affects the connected DEFCON track. The effect is 1 step per cube **after the first** (so 1 cube = no DEFCON change, 3 cubes = 2 steps).
- **Opponent's Cards for Command:** Cards of your opponent's superpower can only be played for Command. Hand the card to your opponent first — they may choose to trigger the Event before your Command action.
...
### During Play
- Prestige is awarded when Agendas are resolved (step 6).
- Basic Prestige = difference in Influence cubes or DEFCON steps on the Agenda's battleground/track.
- Bonus Prestige: +0 to +2 as shown on the battleground. DEFCON tracks always award +1. Cuba (military), Cuba (political), and Atlantic are connected — each awards +1 for each other connected battleground also dominated.
- Maximum Prestige lead: 5.
...
- DEFCON conditions may be temporarily violated during steps 1-6; they are only checked in step 7.
- No DEFCON marker may be moved beyond the limits of the track — leave it at the top or bottom.
- The Personal Letter Agenda awards 2 Prestige to the player holding the Personal Letter card (not based on Influence cubes).
- When the UN wins the Personal Letter at world opinion, and the holder already has it, there is no effect.
- Cards played as Events override normal placement rules but never exceed the 5-cube limit per battleground.
- Opponent Events triggered on your turn are resolved as if the opponent played the card (but on your turn).
...
**Key Numbers:**
- 17 Influence cubes per player (hard limit)
- 5 cubes max per player per battleground
- 5 Prestige max lead
- 3 rounds, 4 card plays per player per round
- DEFCON effect: 1 step per cube after the first
