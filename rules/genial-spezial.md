---
title: "Genial Spezial"
bgg_id: 40830
player_count: "2-4"
play_time: "45 min"
designer: "Reiner Knizia"
source_pdf: "genial-spezial-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Genial%20Spezial&game=genial-spezial)
<!-- verification:end -->
...
- 1 hex game board with tower positions
- Towers in 4 colors (varying sizes: small, medium, large)
- Colored chips placed on towers (showing point values)
- Double-hex tiles (domino-shaped, each half showing a color)
- Score tracks (1 per player, with tracks for each of the 4 colors)
- Tile rack/screen for each player

## Setup

1. Place the game board in the center of the table.
2. Arrange towers of different sizes on the designated board positions. Place colored chips on each tower (chip color and value depend on the tower).
3. Each player takes a score track and sets all four color markers to 0.
4. Shuffle all tiles face-down. Each player draws tiles to fill their rack (typically 6 tiles).
...
- Score points in the color of each connected tower equal to the chip value shown on that tower.
- When a chip is scored, flip it to its black side (which shows a different, usually lower, value for future scoring).
- **Large tower bonus:** For each large tower in a connected chain, score 1 bonus point in every color.
...
| Colors | 4 |
| Rack size | ~6 tiles |
| Final score | Lowest color value |
| Large tower bonus | +1 per large tower in chain, all colors |
