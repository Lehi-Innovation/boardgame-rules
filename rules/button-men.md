...
source_pdf: "button-men-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/button-men-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Button%20Men&game=button-men)
<!-- verification:end -->
...
- Button Men fighter pins/cards (each specifying a set of polyhedral dice)
- Polyhedral dice as specified by each fighter (d4, d6, d8, d10, d12, d20, etc.)
- Some fighters include Swing dice (variable size)
...
**Scoring formula:**
- Each captured opponent die: score its full number of sides (e.g., a captured d10 = 10 points)
- Each of your own surviving dice: score half its number of sides (e.g., your surviving d8 = 4 points)
...
- **Swing dice** can be set to any even-sided die from d4 to d20. The size is chosen secretly before the match begins and can be changed between rounds.
- **Trip dice** (marked with a "p"): when you make a special attack called a "Trip," you and the target both re-roll. If the Trip die now shows an equal or greater number than the target, the target is captured. Otherwise, the attack fails (nothing happens, but the dice stay at their new values).
- **Poison dice** (marked with "p"): flip the trip die and the target. Then compare the numbers they show. If the Trip die shows equal or greater, capture succeeds.
- **Rerolling:** After every successful capture (Power or Skill), the attacking die or dice are re-rolled.
- If you can make any legal attack on your turn, you must do so -- passing is only allowed when no attacks are possible.
- In a Power Attack, the attacker must show a value >= the target. In a Skill Attack, the attackers must sum to exactly the target's value.
