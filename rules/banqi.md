...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Banqi&game=banqi)
<!-- verification:end -->
...
1. All 32 pieces are shuffled and placed face-down randomly on the 32 squares of the 4x8 grid.
2. The first player turns any one piece face-up. The color of that piece determines which color that player controls for the rest of the game.

## Turn Structure

On each turn, a player performs exactly one of three actions:
1. **Flip**: Turn a face-down piece face-up.
2. **Move**: Move one of your face-up pieces one square orthogonally (up, down, left, or right).
3. **Capture**: Capture an adjacent enemy piece (following capture rules).
...
### Turn Options
1. Flip any face-down piece
2. Move your piece 1 square orthogonally
3. Capture an adjacent enemy piece (or Cannon jump-capture)
