...
source_pdf: "go-gecko-go-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Go%20Gecko%20Go%21&game=go-gecko-go)
<!-- verification:end -->
...
1. Assemble the river course on the table, placing bridges at designated positions along the route.
2. Each player takes their set of four animals (crocodile, turtle, frog, gecko) in their chosen color.
3. Place all animals at the starting position of the river.
4. Place the three dice within reach.
...
2. **Choose a combination** of dice results to use.
3. **Move animals** according to the chosen dice symbols -- each die showing an animal symbol lets you move the corresponding animal type forward.

### Option B: Add a River Tile
1. **Place a river tile** to extend the stream.
2. **All animals in the river shift forward** simultaneously due to the current.
3. This can move animals past obstacles without triggering bridge height checks in the same way.
