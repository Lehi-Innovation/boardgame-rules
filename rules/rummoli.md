...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rummoli&game=rummoli)
<!-- verification:end -->
...
- 1 Rummoli game board (with labeled pot sections: Poker Pot, Rummoli Pot, Ace of Spades, King of Hearts, Queen of Diamonds, Jack of Clubs, 10 of Spades, Sequence 7-8-9 of Diamonds)
- 1 standard deck of 52 playing cards (no jokers)
- Playing chips (distributed equally among players)
...
**Poker Phase Actions:**
- Select 5 cards for your poker hand.
- Bet, call, raise, or fold (if using the betting variant).
- Win the Poker Pot with the best hand.

**Rummy Phase Actions:**
- Play cards in sequential order within a suit.
- When you play a pay card matching a pot label on the board (e.g., Ace of Spades, King of Hearts, Queen of Diamonds, Jack of Clubs, 10 of Spades, or the 7-8-9 of Diamonds sequence), collect all chips from that pot.
- Switch suits when the current sequence is blocked.
...
- The Widow can contain key pay cards. If no one takes the Widow, those cards (and their pot bonuses) remain uncollected for the round, and the chips roll over to the next round.
- If pay cards are in the Widow, the chips on those pot sections carry over and grow larger each round, creating increasingly valuable jackpots.
- The poker hand variant (with betting) adds a gambling element; the showdown variant is simpler and faster.
- Cards do not distribute evenly with all player counts. The extra card(s) in some hands is normal.
- The Rummy phase requires tracking which suits are blocked; a suit blocks when the next sequential card is in the Widow or has already been played.
