...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rococo&game=rococo)
<!-- verification:end -->
...
- 1 double-sided game board (different sides for different player counts)
- 42 dress tiles (various colors and types)
- 28 employee cards (levels 1-6, with Apprentice/Journeyman/Master ranks)
- 5 player boards with 5 base employee cards each (2 Apprentices, 2 Masters, 1 Journeyman)
- 48 coins (Livres)
- Prestige point tokens
- Property markers (16 per player)
- Lace markers and thread markers (wooden)
- Resource tiles
- Cloth bag for drawing dress tiles
- Favor card
...
- **Masters** can perform any action.
- **Journeymen** cannot Hire employees.
- **Apprentices** can only Acquire Resources, Depute, or Fund Decorations.
...
- Dress colors must match the ball space where they are rented; you cannot rent a dress to the wrong color space.
- Some employee cards grant bonuses when played (crown symbol) that can provide extra resources, prestige, or movement options.
- Deputing (removing) employees is a key deck-thinning strategy; a leaner deck draws more consistently.
- The double-sided board adjusts the game for different player counts (fewer spaces with fewer players).
- Fountain decorations provide recurring income each round in addition to end-game scoring.
