...
source_pdf: "rommel-s-panzers-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rommel%27s%20Panzers&game=rommel-s-panzers)
<!-- verification:end -->
...
- Small hex map (depicting the North African desert)
- Unit counters (German panzers and British tanks/vehicles)
- 10 scenario cards
- Combat results table (CRT)
- Dice
- Rules sheet (microgame format, fits in a small ziplock bag)
...
- Basic movement and combat (scenarios 1-2)
- Overrun attacks (scenario 3)
- Supply and fuel rules (scenario 4-5)
- Specialized unit types (later scenarios)
- Combined arms effects (final scenarios)

## Scoring / Victory Conditions

Victory conditions vary by scenario. Common objectives include:
- Destroying a specified number of enemy units within a time limit.
- Controlling key map positions (supply hexes, objectives).
- Surviving for a set number of turns while inflicting casualties.

The scenario card specifies exact victory conditions for each game.

## Special Rules & Edge Cases

- No infantry and no terrain features: this is a pure tank-vs-tank game on open desert.
- Playing scenarios in order teaches the rules progressively, with new mechanics added each game.
- Supply rules in later scenarios mean units can run out of fuel and become immobile.
- Overrun attacks allow moving units to attack without stopping, representing fast armored thrusts.
- The microgame format means setup is quick and games are short (30-60 minutes even for complex scenarios).
