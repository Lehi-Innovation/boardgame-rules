---
title: "Raid on Iran"
bgg_id: 5437
player_count: "2"
play_time: "60-120 min"
designer: "Steve Jackson"
source_pdf: "raid-on-iran-rules.pdf"
extracted_date: "2026-03-22"
summarized_date: "2026-03-22"
rulebook_version: "Steve Jackson Games (1980)"
verification: "unverified"
verification_date: "2026-06-12"
---

# Raid on Iran

<!-- verification:begin -->
> ⚠️ **Unverified** — this AI-generated summary has not yet been fact-checked against the rulebook. Double-check critical rules against the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/raid-on-iran-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Raid%20on%20Iran&game=raid-on-iran)
<!-- verification:end -->

## Overview

Raid on Iran is a two-player wargame simulating a hypothetical rescue mission of American hostages from Tehran in 1980. One player commands US Special Forces attempting to infiltrate Iran, rescue hostages, and extract them safely. The other player controls Iranian forces defending Tehran. Originally published as a Metagaming microgame in a small format.

## Components

Game map (16" x 21"), counters representing US and Iranian units, rules sheet, scenarios.

## Setup

Set up units per the scenario chosen. The US player deploys special forces outside Iran; the Iranian player places defenders in Tehran and along approaches.

## Turn Structure

Alternating player turns with movement, combat, and event phases.

## Actions

### US Player
- Deploy helicopters and special forces.
- Infiltrate toward Tehran using cover of darkness.
- Assault the embassy compound to rescue hostages.
- Extract forces via helicopters to safety.

### Iranian Player
- Alert and mobilize defense forces.
- Patrol and search for infiltrating US units.
- Defend key locations (embassy, airfields).
- Pursue and intercept retreating US forces.

### Combat
Combat uses odds-based CRT (Combat Results Table). Terrain, surprise, and unit quality modify odds.

## Scoring / Victory Conditions

The US player wins by rescuing a sufficient number of hostages and extracting them to safety. The Iranian player wins by preventing the rescue or destroying US forces. Partial victories possible based on hostages rescued vs. forces lost.

## Special Rules & Edge Cases

- Night operations rules affect detection and movement.
- Helicopter availability is limited and vulnerable.
- The game includes historical notes and a "Sauce for the Gander" variant scenario (Iranian attack on US assets).
- Intelligence and surprise are critical factors.

## Player Reference

| Side | Objective |
|---|---|
| US | Rescue hostages, extract forces |
| Iran | Prevent rescue, destroy attackers |
