...
source_pdf: "saboteur-the-duel-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Saboteur%3A%20The%20Duel&game=saboteur-the-duel)
<!-- verification:end -->
...
1. **Play a path card**: Extend the tunnel network by placing a card adjacent to existing cards, following connection rules.
2. **Play an action card**: Use a special action (place a troll, use a map to peek at a goal card, cause a rockfall, etc.).
3. **Play a dwarf card**: Place one of your dwarf meeples on the path network.
4. **Discard a card**: Pass by discarding one card from hand.
...
1. At the end of each round (when the draw deck is exhausted and players cannot play), gold is distributed based on goal cards reached.
2. After 3 rounds, total all gold collected.
3. The player with the **most gold** wins.
...
| Card Type | Effect |
|-----------|--------|
| Path card | Extend tunnel network |
| Dwarf card | Place dwarf meeple on network |
| Troll | Block a path |
| Map | Peek at goal card |
| Rockfall | Destroy a placed path card |
| Door | Block a passage |
| Key | Remove troll or door |
