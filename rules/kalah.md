...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Seeds per pit wrong: summary states 4, source states "usually six" (repeated twice in source); changes total from 72 seeds to 48. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/kalah-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Kalah&game=kalah)
<!-- verification:end -->
...
- 1 Mancala board with 12 small pits (6 per side) and 2 large stores (1 per player)
- 48 seeds/stones (4 per pit in the standard version)
...
- The seed you just placed
- All seeds in the opposite pit
...
- You must make a move if you have seeds in any pit; you cannot pass.
- If you cannot move (all your pits are empty), the game ends immediately.
- Multiple extra turns are possible in sequence (each time your last seed lands in your store).
- Captures only occur on YOUR side of the board.
- Landing in your own empty pit on the opponent's side does NOT result in a capture.
- Seeds pass through and accumulate; pits can hold any number of seeds.
