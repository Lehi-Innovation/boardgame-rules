...
play_time: "45 min"
designer: "Uncredited"
source_pdf: "formula-1-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/formula-1-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Formula%201&game=formula-1)
<!-- verification:end -->
...
- 1 Playing board (race circuit with corners and straights)
- 2 Decks of cards (Tactical Cards)
- 2 Dice
- 6 Race cars
- 6 Dashboards with speedometers and lap indicators
...
- If speedometer is set at 20 m.p.h.: move 1 space
- If at 40 m.p.h.: move 2 spaces
- If at 60 m.p.h.: move 3 spaces
- And so on (each 20 m.p.h. = 1 additional space)
...
- **Increase**: Up to +60 m.p.h. per turn
- **Decrease**: Up to -40 m.p.h. per turn (normal braking). A player must pay any penalty shown on the Speed Reduction Chart on their dashboard for sudden stops.
- As indicated by the Speed Reduction Chart, a -40 m.p.h. reduction may be made on any turn, but reductions above that require spending additional turns.
- As the game progresses, the line may become extensive and it becomes difficult to maneuver. In this case, speed may be reduced by a maximum of 3 points (60 m.p.h.) with a penalty of -1 point on the lap indicator. This lost speed penalty cannot be reduced to less than 0.

### Corners

There are corners on the race track represented by curved arrows and coloring:
- The arrow shows the safe speed of each corner (the safe speed at which a car can negotiate the corner).
- A car coming into a corner at or below the safe speed proceeds normally.
- A car coming into a corner above the safe speed risks spinning off (see Spinning Off).
...
- **Yellow Car**: Always moves first each turn.
- **No Jumping**: Cars cannot jump over other cars.
- **One Car Per Space**: Only one car may occupy a space at a time.
- **Speed Reduction Penalty**: Reducing speed by more than 40 m.p.h. in a single turn incurs a penalty on the dashboard.
- **Pit Stop Timing**: Available only at the end of a lap; resets all damage except the lap indicator.
- **Tactical Card Limit**: Only one Tactical Card may be played per turn.
- **Backward Movement**: Cars may never move backwards.
- **Corner Safety**: Each corner has a marked safe speed; exceeding it risks a spin-off.
...
### Key Rules
- Max speed increase per turn: +60 m.p.h.
- Max speed decrease per turn: -40 m.p.h. (penalty for more)
- Corner spin-off: exceeding posted safe speed
- Pit stop: resets all damage except lap indicator
