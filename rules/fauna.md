...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/fauna-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Fauna&game=fauna)
<!-- verification:end -->
...
- 1 Game board (world map with land/marine areas, measurement scales, victory track, scoring table)
- 180 Animal cards (360 animals total, double-sided; green border = simpler, black border = exotic)
- 1 Card box
- 42 Guess pieces (7 per player in 6 colors)
- 30 Black evaluation pieces
- 1 Starting player's lion token
- 1 Accompanying booklet with animal information
...
- **Place a guess piece** on an unoccupied area or vacant scale space
- **Pass** (permanent for the round)
...
- **Marine areas** include all islands within them unless those islands are specifically marked as land areas.
- **Not all scales apply to every animal.** Check the top of the card for which measurements are relevant before placing pieces on scales.
- **Green vs. black cards:** Green-bordered cards have more commonly known animals; black-bordered cards are more exotic and challenging.
- **Guess piece recovery:** Only 1 wrongly placed piece is returned per round. If you have fewer than 3 total, you refill to 3.
- **No duplicate placements:** You cannot place a piece on a space already occupied by any player's piece (yours or opponents').
- **Animal data:** Values on cards are averages from multiple zoological sources. Some discrepancy with other references is possible.
