...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/coyote-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Coyote&game=coyote)
<!-- verification:end -->
...
1. Shuffle all 17 Playing cards and place them face-down as a deck in the center.
2. Each player takes a card stand and places it in front of them.
3. Each player takes 3 Peek cards and places them with 2 open eyes and 1 closed eye face-up in front of them. Return remaining Peek cards to the box.
...
### Start of Each Round
1. Each player draws 1 Playing card from the deck without looking at it.
2. Place the card in your card stand facing outward (only other players can see it).
3. Place 1 additional Playing card face-down in the center of the table (unknown to all).
4. The start player (youngest in round 1, or the loser of the previous round) makes the first bid.
...
**Trickster Variant:**
1. Start with 1 open eye and 2 closed eyes on Peek cards.
2. Before resolving a challenge, all players (except the challenger and previous bidder) simultaneously point at who they think will win. Correct guess: flip 1 closed eye to open. Wrong guess: flip 1 open eye to closed.
3. The loser receives the Coyote card. At the start of their turn (not the first turn), they may draw the top card from the deck, look at it secretly, and place it face-down next to the existing center card. They must then raise the bid. Other players cannot peek at this new card. The new card counts toward the total.

## Player Reference

| Item | Detail |
|------|--------|
| Starting Peek cards | 3 (2 open, 1 closed) |
| Cards per round | 1 per player + 1 hidden center card |
| Elimination | Lose all Peek cards = out |
| Number card range | -10 to 20 |
| Special cards | ? (random replacement), Max 0 (highest becomes 0) |
