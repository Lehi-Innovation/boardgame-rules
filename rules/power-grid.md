...
source_pdf: "power-grid-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/power-grid-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Power%20Grid&game=power-grid)
<!-- verification:end -->
...
- 1 double-sided board (Germany/USA) with scoring track, player order, and resource market
- 132 wooden houses (22 per player in 6 colors)
- 84 wooden resource tokens (24 coal, 24 oil, 24 garbage, 12 uranium)
- Money in Elektro (various denominations)
- 6 summary cards
- 43 power plant cards (numbered 03-50) + 1 "Step 3" card
...
- **Auction:** Bid on power plants; winner pays and takes plant.
- **Buy Resources:** Purchase coal, oil, garbage, or uranium at market prices.
- **Build:** Expand network by connecting to new cities.
- **Power:** Fuel plants and earn income for cities supplied.
...
- **Step transitions:** Step 2 begins when any player connects to a threshold number of cities. Step 3 begins when the Step 3 card is drawn from the power plant deck.
- **Step 2:** Remove lowest-numbered plant from market. Cities now accept 2 houses each.
- **Step 3:** Remove lowest-numbered plant and the future market. Cities accept 3 houses. Draw replacement plants from reduced deck.
- Max 3 power plants per player. Buying a 4th requires discarding one (resources on discarded plant may transfer if compatible).
- Resources returned to supply (not market) when discarded from plants.
- Ecological plants require no resources.
- In the first round, re-determine player order after auctions.
