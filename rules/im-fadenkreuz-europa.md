...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Im%20Fadenkreuz%3A%20Europa&game=im-fadenkreuz-europa)
<!-- verification:end -->
...
- 1 Large map of Europe
- Picture question cards (showing landmarks, buildings, locations)
- Cardboard targeting rings in multiple sizes (different diameters)
- Scoring tokens or track

## Setup

1. Spread the large map of Europe flat on the table.
2. Shuffle the picture question cards and place them in a draw pile.
3. Each player receives a set of targeting rings in different sizes.
4. Determine who goes first.

## Turn Structure

Each round:
1. **Draw a question card** — Reveal a picture card showing a European landmark or location with a question (e.g., "Where is this bridge?" or "Where is the airport Leifur Eiriksson?").
2. **Select a ring** — Each player secretly selects which size ring they want to use (smaller = riskier but worth more).
3. **Place rings** — Players take turns placing their ring on the map where they believe the answer is.
4. **Reveal answer** — The correct location is revealed.
5. **Score** — Players whose rings contain the correct location score points based on their ring size.
...
| Ring Size | Points if Correct |
|-----------|------------------|
| Largest | Fewest points |
| Medium | Moderate points |
| Smallest | Most points |
