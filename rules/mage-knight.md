...
designer: "Jordan Weisman, Kevin Barrett"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/mage-knight-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Mage%20Knight&game=mage-knight)
<!-- verification:end -->
...
1. Players agree on a point total for army building (typically 200-300 points). Each figure has a point cost.
2. Each player assembles their army, selecting figures whose total points do not exceed the agreed limit.
3. Set up a playing area (typically 3x3 feet) with optional terrain.
4. Players deploy armies on opposite sides of the battlefield.
5. Determine starting player by dice roll.

## Turn Structure

Players alternate turns. On each turn:

1. **Clear Phase:** Remove action tokens from figures that acted last turn.
2. **Action Phase:** Give actions to your figures (up to a limit based on army size).
3. Each activated figure may: Move, Attack, or use a Special Ability.
...
|------------------|-------|
| 1. Declare attack | Choose attacker and target |
| 2. Check range/adjacency | Must be in range and have line of fire |
| 3. Roll 2d6 | Add attack value |
| 4. Compare to defense | If total >= defense, hit |
| 5. Deal damage | Click target dial equal to damage value |
| 6. Check KO | If past last slot, figure is defeated |
