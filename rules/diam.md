---
title: "Diam"
bgg_id: 11730
player_count: "2-4"
play_time: "20 min"
source_pdf: "diam-rules.pdf"
extracted_date: "2026-03-19"
summarized_date: "2026-03-19"
verification: "unverified"
verification_date: "2026-06-12"
---
<!-- verification:begin -->
> ⚠️ **Unverified** — this AI-generated summary has not yet been fact-checked against the rulebook. Double-check critical rules against the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/diam-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Diam&game=diam)
<!-- verification:end -->


## Overview

Diam is a strategy game for 2, 3, or 4 players. The goal is to place two pawns of the same color on two squares that are diametrically opposite and at the same level. This configuration is called a "DIAM." The first player to achieve this wins.

## Components

- 1 eight-sided wooden playing board
- 16 wooden pawns: 4 red, 4 orange, 4 black, 4 brown

## Setup

**2 Players:** Each player has a reserve of 8 pawns: 4 red and 4 orange for Player 1, 4 black and 4 brown for Player 2. The board has no pawns on it.

**4 Players:** Players play by teams of 2. Each player has their own color. Team 1 plays with red and orange pawns; Team 2 plays with brown and black pawns.

**3 Players:** 2 players play against one. The solo player uses 2 colors. Basic rules are the same as for 4 players; the solo player plays with the restrictions that follow the 4-player rules.

## Turn Structure

Players take turns. On each turn, a player can perform one of two actions:

1. Place a pawn from their reserve on a free square or on a pawn already on the board.
2. Move one of their pawns that is already on the board.

## Actions

| Action | Details |
|--------|---------|
| Place a pawn | Place from reserve onto any free square or on top of an existing pawn on the board |
| Move a pawn | Move one square at a time, to the left or to the right. The new square can be free or have one or several pawns stacked on it |

**Stacking Rules:**
- A square can only have a maximum of 4 pawns after you have moved.
- If on the starting square, one or several pawns are stacked above the pawn to be moved, the player must move the whole stack from the chosen pawn up to the top one.
- If the new square already has a pawn or stack, the moved pawn or stack is placed above the existing stack.

## Scoring / Victory Conditions

**Victory (The DIAM):** The winner is the first player who succeeds in placing two of their pawns of the same color on two diametrically opposite squares at the same level (with the exception of level 1). This configuration is called a DIAM.

**Important:** 2 DIAMs can be created during a move. In this case, the player having the highest DIAM is declared the winner.

## Special Rules & Edge Cases

- Level 1 (ground level) does not count for creating a DIAM.
- A DIAM requires same-color pawns at diametrically opposite positions at the same height.
- Two DIAMs created simultaneously: the highest one wins.
- **4-Player restriction:** A player cannot place a pawn on the board from the reserve of their partner. A player can, however, move their partner's pawns that are already on the board.
- **3-Player turn order:** The solo player alternates with the team players in a specific sequence (player alone, player 1, team players 2 and 3, etc.).

## Player Reference

| Item | Value |
|------|-------|
| Players | 2, 3, or 4 |
| Pawns per player (2p) | 8 (two colors of 4) |
| Max stack height | 4 pawns |
| Win condition | DIAM at level 2+ |
| Level 1 exception | Cannot form DIAM at level 1 |
