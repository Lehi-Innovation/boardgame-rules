...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/blackbeard-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Blackbeard&game=blackbeard)
<!-- verification:end -->
...
- Play as many cards as desired, but only ONE card may be played for Actions. For each card, choose one option:
  1. **Play for Event** ("Event OR Actions" cards): Resolve the event immediately.
  2. **Play for Actions** ("Event OR Actions" cards): Conduct the number of actions shown on the card. Only one card per turn may be played this way.
  3. **Play for Event AND Action** ("Event AND Action" cards): Resolve the event, then conduct one action.
