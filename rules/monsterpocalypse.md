...
source_pdf: "monsterpocalypse-rules.pdf"
...
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Turn structure wrong: summary describes Unit Turn + Monster Turn each turn, but source says choose ONE activation per turn; also "Screen" unit action invented with no source support. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/monsterpocalypse-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Monsterpocalypse&game=monsterpocalypse)
<!-- verification:end -->
...
1. Assemble the city map with buildings placed in designated positions.
2. Each player selects a monster and supporting units.
3. Place monster figures on starting positions on opposite sides of the map.
4. Deploy unit figures in starting zones.
5. Each player assembles their action dice pool.

## Turn Structure

Each turn alternates between the two players. A player's turn has two phases:

1. **Unit Turn**: Spend action dice to activate and move units, attack with units, or secure power zones.
2. **Monster Turn**: Spend action dice to move your monster, attack, use special abilities, or perform power attacks.
...
| Phase | Action |
|-------|--------|
| Unit Turn | Move, attack, or secure with units using action dice |
| Monster Turn | Move, brawl, blast, or power attack with monster |
| Dice | Roll action dice; allocate to actions |
| Buildings | Destructible; provide cover and strategic options |
| Win | Destroy the opposing monster |
