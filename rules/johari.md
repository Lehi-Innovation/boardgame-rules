...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/johari-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Johari&game=johari)
<!-- verification:end -->
...
- 96 Gem Cards (gems in 4 colors plus Gold and Prestige Cards; some marked with Supervisor Symbol as fakes)
- 4 Overview Cards
- 1 Promo Card
- 24 Nobles (with 3 different reverse sides: 1, 2, 3)
- 28 Action Cards in 4 player colors (7 per player)
- 1 two-part Game Board
- 4 Player Boards in 4 player colors
- 4 Gold Tokens in 4 player colors
- 16 Gem Tokens (4 each in blue, red, green, translucent)
...
- Move Nobles one space right; place a new Noble on space 5 from the stack. Nobles moving off space 1 are removed.
- Add Gem Cards equal to player count to Stores (by type).
- Add 1 Gem Card to each Bazaar Booth from the draw pile.
- If the draw pile runs out, shuffle discards to form a new pile.
...
- 1st slot: Full cost
- 2nd slot: Cost reduced by 2 (minimum 0)
- 3rd slot: Free
...
Choose one:
- **Trade in a Store:** Take all cards from one Store and place on your Trade Action Card. Cards are added to your display only at the end of the Market Day (safe from Supervisors but cannot be sold).
- **Hire a Noble:** Pay gems of one type equal to or exceeding the number printed below the Noble. Use the Noble's ability immediately.
...
- Nobles' special abilities
- Prestige Cards
- Gem Token positions on the Prestige Track

The player with the most Prestige Points wins. Tiebreak: highest sum of Gem Tokens on the Prestige Track.

## Special Rules & Edge Cases

- If a player plays Sale but cannot execute it, they do not pay costs and opponents do not lose fake gems.
- Gold from the Gold Track and Gold Cards are both valid resources.
- When moving a Gold Token onto an occupied space, place it on top.
- After each payment, change the row of the Gold Token (top to bottom or vice versa) for clarity.
- Some Nobles reduce costs further but never below 0.
