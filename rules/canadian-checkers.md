...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Canadian%20Checkers&game=canadian-checkers)
<!-- verification:end -->
...
- 1 checkerboard with 12x12 squares (144 total, 72 dark squares used for play)
- 30 light pieces (one player)
- 30 dark pieces (one player)
...
- Moves diagonally forward only (one square per move).
- Captures forward or backward by jumping over adjacent opponent pieces.

### King (Flying King) Movement
- Moves diagonally any number of squares in any direction (forward or backward).
- Captures by jumping over opponent pieces at any distance along a diagonal.
- After a capture, may land on any empty square beyond the captured piece.
...
- The same board position occurs **three times** with the same player to move.
- Neither player can force a win.

## Special Rules & Edge Cases

- **No backward movement for regular pieces:** Regular pieces can only move forward, though they can capture both forward and backward.
- **Flying kings:** Kings are extremely powerful -- they can move and capture across any number of empty squares diagonally.
- **Multi-capture continuation:** During a multi-capture, a piece must keep jumping as long as captures are available. Captured pieces are removed only after the entire sequence completes.
- **Promotion during multi-capture:** A piece passing through the back row during a multi-capture is NOT promoted. It must end its move on the back row to become a king.
- **Maximum capture is mandatory:** When multiple capture paths exist, the player must take the path capturing the most pieces.
- **Pieces removed after sequence:** In multi-capture, jumped pieces remain on the board until the capturing sequence is complete, then all jumped pieces are removed simultaneously.
