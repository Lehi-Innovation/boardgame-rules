...
source_pdf: "gumball-rally-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Gumball%20Rally&game=gumball-rally)
<!-- verification:end -->
...
1. Shuffle the racing card deck and deal 3 cards to each player.
2. Prepare the hazard deck by shuffling checkpoint and finish line cards into position (checkpoints and finish appear between turns 4-15).
3. Arrange all players' cars in a starting line.
...
1. **Play cards:** Starting with the player in the lead and proceeding in line order, each player plays one card from their hand of three.
2. **Resolve overtaking:** Cards are used to pass the car ahead or defend position. Higher numbers pass lower numbers; a matching color passes the same color if that car is ahead.
3. **Draw:** Players draw back up to 3 cards.
4. **Hazard check:** If a hazard card is drawn from the hazard deck, resolve it. Hazards may cause leading cars to move to the back of the line.
5. **Checkpoint/Finish scoring:** If a checkpoint or finish line card appears, score points based on current positions.
