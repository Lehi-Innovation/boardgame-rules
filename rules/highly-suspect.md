...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Highly%20Suspect&game=highly-suspect)
<!-- verification:end -->
...
1. Set up the tiltable game board on a flat surface.
2. Place obstacle elements on designated positions within the park.
3. Each player selects a detective piece.
4. Place the crook piece at a designated starting position.
5. Place detective pieces at their starting positions.

## Turn Structure

On your turn:
1. **Tilt the board** in one direction (up, down, left, or right).
2. **All pieces slide** — Every piece on the board (detectives and crook) slides in the tilt direction until hitting an obstacle or the board edge.
3. **Check for capture** — If any detective piece ends up in the same space as the crook, the crook is caught.
...
| Action | Effect |
|--------|--------|
| Tilt board | All pieces slide in tilt direction |
| Hit obstacle | Piece stops at obstacle |
| Reach crook | Detective captures crook |
