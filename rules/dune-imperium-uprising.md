...
source_pdf: "dune-imperium-uprising-rules.pdf"
extracted_date: "2026-05-17"
summarized_date: "2026-05-17"
rulebook_version: "23-10-12"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/dune-imperium-uprising-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Dune%3A%20Imperium%20%E2%80%93%20Uprising&game=dune-imperium-uprising)
<!-- verification:end -->
...
- 9 Leaders (two-sided; 1 used only with the CHOAM module — Shaddam Corrino IV)
- 69 Imperium Deck cards (including 4 used only with the CHOAM module)
- 44 Intrigue cards (including 4 used only with the CHOAM module): Plot, Combat, Endgame types
- 18 Reserve cards: 8 Prepare the Way, 10 The Spice Must Flow
- 16 Conflict cards: 3 Conflict I, 9 Conflict II, 4 Conflict III
- 5 Objective cards (some marked "1-3P" or "4/6P")
- 8 sandworms (4 plastic, 4 wood — use either or both)
- 4 Maker Hooks tokens
- Shield Wall token
- 4 Alliance tokens, First Player marker, Feyd token (used with Feyd-Rautha Harkonnen Leader)
- Spice: 7 large (worth 5), 20 small (worth 1); Solari: 8 large (worth 5), 20 small (worth 1); 20 water
- CHOAM Module: 20 Contract tokens (10 standard + 10 with different backs for use with Rise of Ix), 4 Imperium cards, 4 Intrigue cards, Shaddam Corrino IV Leader
- Per player (one color each): 3 Agents (one is the Swordmaster), 3 Control markers, 1 Combat marker, 16 cubes, 2 discs, 1 Score marker, 1 Councilor token, 3 Spies, and a 10-card starting deck
...
1. Place the game board (correct side up). Place the **Shield Wall token** in the marked area below the Spice Refinery board space. Place the four **Alliance tokens** on the marked spaces of the Emperor, Spacing Guild, Bene Gesserit, and Fremen Influence tracks.
2. **Conflict Deck:** Separate by back (I/II/III). Shuffle the four Conflict III and place all four face down in the marked area. Shuffle the nine Conflict II, deal five face down on top. Shuffle the three Conflict I, deal one face down on top. Result: a 10-card deck with 1 Conflict I on top, 5 Conflict II, then 4 Conflict III on the bottom. Return unused Conflict cards to the box without looking.
3. Each player takes a **Leader** (choose or random). Do **not** use Shaddam Corrino IV unless playing with the CHOAM Module. Leaders have 1–3 complexity icons after their name; first game, pick a 1-icon Leader. (If two leaders are from the same House, ideally don't use both in one game — but nonstandard combos are allowed.)
4. Prepare cards along the board edge (omit any marked with the CHOAM symbol if not using that module). Shuffle the **Intrigue Deck** face down. Shuffle the **Imperium Deck** face down and deal **five** cards face up as the **Imperium Row**. Next to it, place the Reserve cards in two stacks: **Prepare the Way** and **The Spice Must Flow**.
5. Shuffle the **Objective cards**. Omit those whose "1-3P"/"4/6P" marking doesn't match the player count; return them to the box. Each player draws one Objective at random and places it face up in their supply. The player whose Objective shows the First Player marker takes it.
6. Each player chooses a color and takes all its components. Place two Agents on your Leader; place your third Agent (the **Swordmaster**) next to the board. Place one disc on the Score track: at the **1** space in a 4-player game, otherwise at **0**. Place your Combat marker on **0** of the Combat track. Place four cubes, one each, on the bottom space of each of the four Faction Influence tracks. The other 12 cubes are troops: place **3** in your garrison (the circular garrison closest to you), the rest in supply. Take **1 water**. Place remaining components (including 3 Spies) in your supply.
7. Create a bank of Solari, spice, remaining water, sandworms (your choice of wood/plastic/both), and the 4 Maker Hooks tokens. These are not limited — substitute if you run out.
...
- **Reveal Cards:** Reveal all remaining cards in hand face up (kept separate from cards played on Agent turns).
- **Resolve Reveal Effects:** Gain the **Reveal box** effects of the just-revealed cards only (not cards played on Agent turns). Resolve in any order. Use gained **Persuasion** to acquire cards before/between/after Reveal effects. Set your Combat strength at any point while resolving (and update as needed — this lets cards like the Gurney Halleck Leader reference your strength during the Reveal turn).
- **Clean Up:** Move all cards from in front of you (Agent and Reveal) to your discard pile.
...
### Strength (set during Reveal turn)
- Each troop in the Conflict = **2 strength**; each sandworm = **3 strength**; each sword revealed = **1 strength**.
- Units in garrison or supply contribute nothing. You must have at least one unit in the Conflict to have any strength; if your last unit is removed, strength becomes 0.
- Announce strength and move the Combat marker. Over 20 strength: flip the marker to the +20 side and continue from the track's start.
...
**VP sources:**
- Faction Influence: reaching **2** Influence = 1 VP (lost if you drop below 2); reaching **4** Influence = the track bonus (kept even if you later drop below 4; can be earned again if you drop and re-reach).
- **Alliance tokens:** the first player to reach 4 Influence with a Faction earns its Alliance — take the token and gain the VP shown on it. If later passed by an opponent rising higher on that track, you give them the token (you lose the VP, they gain it).
- Conflict card rewards (1st/2nd/3rd place).
- **Battle-icon matches:** flipping a matching pair = 1 VP.
- Control bonuses, Imperium cards, and Intrigue cards (Plot/Combat/Endgame) as printed.

Whenever you gain/lose a VP, move your Score marker accordingly.

## Special Rules & Edge Cases

- **Spies / observation posts:** The Spy icon lets you place a Spy from supply onto an unoccupied observation post (if none in supply, first recall one of your Spies for no effect). Some effects require the post to connect to a specific Agent icon. The **Spy Agent icon** lets you send an Agent to a board space connected to a post where you have a Spy (Spy is NOT recalled). A Spy may be recalled (without a card) for one of two effects: **Infiltrate** — send your Agent to a space occupied by another player (ignoring their Agent) by recalling your Spy from a connected post; **Gather Intelligence** — when you send an Agent to a space, recall your Spy from a connected post to draw a card (decide immediately after placing your Agent, before any effects). You can't recall the same Spy for both, but on some spaces you can recall one Spy to Infiltrate and another to Gather Intelligence. The Recall Spy card icon returns a Spy from a post to supply (usually a cost). Infiltrate only ignores an **opponent's** Agent — it does not let you place a second Agent on a space you already occupy yourself; the Spy Agent icon likewise only changes which icon reaches a space, which must still be unoccupied. Recalling a Spy "for no effect" (when you must place a Spy but have none in supply) is the only way to reposition a Spy once all three are already on posts.
- **Critical Locations & Control:** Conflicts titled for Arrakeen, Spice Refinery, or Imperial Basin grant control of that space (place a Control marker on its flag, replacing any opponent's). While you control it, you get a bonus whenever **any** player (including you) sends an Agent there: 1 Solari for Arrakeen or Spice Refinery, 1 spice for Imperial Basin. When a Conflict is revealed for a space you already control, you may deploy one troop from your **supply** to the Conflict (defensive bonus).
- **Shield Wall:** Protects Arrakeen, Spice Refinery, and Imperial Basin from sandworms while the token is on the board. The Shield Wall detonation icon (on a card or space) lets you remove the token (return to box); once removed, sandworms may be summoned to any Conflict.
- **Influence track:** Reaching 2 = 1 VP (lost below 2). Reaching 4 = bonus (kept) + Alliance for the first to reach it. "Gain one / gain two / lose one Influence" with a free choice means choose one of the four Factions; when gaining two you can't split across two Factions (Faction-icon Influence is fixed to that Faction).
- **Influence requirement spaces:** e.g., Imperial Privilege (2+ Emperor), Shipping (2+ Spacing Guild), Sietch Tabr (2+ Fremen).
- **Arrows / costs:** A cost (left/above the arrow) yields an effect (right/below). Never forced to pay; if you don't pay you don't gain. You may pay such a cost only **once per turn** (exception: sandworm reward doubling on a Conflict card).
- **Intrigue cards:** Kept face down, separate from your deck; look at them anytime; reveal only when played; discard face up after resolving. **Plot** — anytime on your Agent/Reveal turn; **Combat** — only during the Combat phase; **Endgame** — only at game end.
- **Ties:** Tie for first — all tied players gain the **second** reward; no one wins the Conflict or takes the card. With ≤3 players, no other rewards. With 4/6 players: if 3+ tied for first, no other rewards; if exactly 2 tied for first, remaining players compete for the third reward. Tie for second — each gains the third reward; the Conflict winner keeps their rewards (and the card) but no further rewards are given. Tie for third — those players receive nothing.
- **Acquire box:** Effect gained once at the moment you acquire the card (not when later played).
- **Fremen Bond:** Usable if you have one or more other Fremen cards in play; two Fremen Bond cards activate one another regardless of play order.
- **Recall Agent (card icon):** Return one of your **other** Agents on the board to your Leader (not the one sent this turn); reusable on another Agent turn the same round.
- **Swordmaster:** Your third Agent, gained from the Swordmaster space, then usable for the rest of the game. There is **no Mentat** in Uprising.
- **Leader clarifications:** *Feyd-Rautha Harkonnen* — place the Feyd token on the leftmost Training-track space at setup; once it reaches the rightmost space it stays there. *Lady Jessica* — begin with the Lady Jessica side up (not Reverend Mother Jessica). *Lady Margot Fenring / Princess Irulan* — you "reach" 2 Influence even when moving past 2 while gaining multiple Influence at once; you can re-reach 2 by losing then regaining Influence (only when moving up, not down). *Shaddam Corrino IV* (CHOAM Module only) — playing his Signet Ring to send an Agent applies the "Emperor of the Known Universe" deploy restriction immediately. His **Sardaukar Commander** ability: the two Sardaukar contracts are set aside as a private pile reserved exclusively for him (no other player can ever take them) — he does **not** start the game holding them. He still acquires a Sardaukar contract through the normal contract-taking mechanism (a contract icon/effect), choosing to take one from his set-aside pile *in place of* one of the two face-up board contracts. *Spacing Guild's Favor* — moving it from play to discard during Clean Up does not trigger its discard ability; only discarding from hand does.

### CHOAM Module (optional mini-expansion)
...
### Combining with other Dune: Imperium products
