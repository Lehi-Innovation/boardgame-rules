...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/catan-dice-game-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Catan%20Dice%20Game&game=catan-dice-game)
<!-- verification:end -->
...
- 6 resource dice (each showing: brick, lumber, wool, grain, ore, gold)
- 1 pad with 60 double-sided sheets (Island One on one side, Island Two on the other)
- 1 rules booklet

## Setup

1. Give each player one sheet (Island One / green side for the first game).
2. Each player needs a pencil or pen.
3. Give the 6 dice to the youngest player (starting player).

## Turn Structure

Starting with the first player, each player takes a turn consisting of:

**1. Rolling the Dice (up to 3 rolls):**
- Roll all 6 dice.
- After the first roll, set aside any number of dice and re-roll the rest.
- After the second roll, set aside more dice (or pick up previously set-aside dice) and roll one final time.
- You may stop after the first or second roll if satisfied.

**2. Building:**
- Use the rolled resources to build structures on your game map.
- Mark built structures by circling or filling in their symbols.
- You may build multiple structures in one turn if resources allow.
- Enter the total points earned this turn in the next empty Scoring Track box.
- If you build nothing, you must enter "X" (worth -2 points).
...
**Building rules:**
- Roads must be built in sequence, each connecting to a previously built road. The first road (starting road) is pre-built.
- Settlements must be adjacent to an already-built road and built in ascending point-value order.
- Cities must be adjacent to an already-built road and built in ascending point-value order.
- Knights must be built in ascending point-value order.
