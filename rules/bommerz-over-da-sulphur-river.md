---
title: "Bommerz over da Sulphur River"
bgg_id: 4646
player_count: "1-5"
play_time: "45 min"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/bommerz-over-da-sulphur-river-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Bommerz%20over%20da%20Sulphur%20River&game=bommerz-over-da-sulphur-river)
<!-- verification:end -->
...
- 3 Board sections
- 12 Defence counters (Interceptor, Laser, Flak, Rock Spires)
- 6 Damage markers
- 8 Dice
- Rules sheet
- Requires 4 Ork Fighta-Bommer and 2 Imperial Thunderbolt Interceptor miniatures (sold separately)
...
### Movement Rules
- Fighta-Bommerz move space to space along links on the map.
- Cannot enter a space just left (point tail toward previous space as reminder).
- Only 1 craft per space at end of movement (may pass through occupied ally spaces).
- Being driven off ends a craft's move even if Speed remains.
...
- 1 hit: Driven Off 1 space (Imperial player's choice); move ends.
- 2 hits: Driven Off + 1 damage.
- 3 hits: Shot down.
Moveable flak can move 1 space per turn. Bridge flak (1D6) is permanent.
...
- 1st damage: -1 to ALL dice rolls (movement, dodges, dogfights, bombing).
- 2nd damage: Craft is destroyed.
...
- Flak counters: destroyed by 1 hit.
- Bridges: destroyed by a total of 4 hits (track with a die).
A Bommer may make multiple bombing runs in one turn.
...
- **Speed selection:** Picking a lower Speed die makes bombing and dodging easier (roll >= Speed) but limits movement range. Higher Speed = more movement but harder to hit targets.
- **Dogfight continuation:** The craft that moved into the space continues moving if it wins; otherwise its move ends.
- **Bridge flak:** Each bridge has a permanent 1D6 flak battery. If a flak defence counter is also on a bridge, roll all flak dice together.
- **Solo Interceptors:** Always move at max speed toward nearest Fighta-Bommer.
- **Craft stacking:** Only 1 craft can end movement in a space, but craft can pass through allied-occupied spaces.
