...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Victim count wrong (summary 10; source 8); rescue threshold wrong (summary 7; source 6); death defeat trigger wrong (summary 4; source 3); Scoring says 24 damage collapse (source 30). Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/flash-point-fire-rescue-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Flash%20Point%3A%20Fire%20Rescue&game=flash-point-fire-rescue)
<!-- verification:end -->
...
1. Assemble the two board panels side by side.
2. Place white door discs on all interior doorways (closed side up). Do not place on exterior entrances.
3. Roll both dice 3 times: place a fire disc (fire side up) on each resulting coordinate.
4. Roll both dice 3 times: place a smoke disc on each resulting coordinate (if fire already there, apply explosion rules).
5. Roll both dice 3 times: place black POI discs on resulting coordinates (must be inside the house).
6. Each player places their pawn on any space outside the house.
7. Give each player an AP surplus card.
...
| Action | AP Cost |
|--------|---------|
| Move (to adjacent space) | 1 AP |
| Move (carrying victim) | 2 AP |
| Open/Close door | 1 AP |
| Extinguish smoke | 1 AP |
| Extinguish fire | 2 AP (1 AP converts fire to smoke, 1 more removes smoke) |
| Chop wall | 2 AP (place 1 damage token on wall section) |
...
### Turn Sequence
1. Detect POI (if < 3 on board, roll and place)
2. Spend AP (4 per turn + saved, max 4 saved)
3. Advance Fire (roll dice, place fire/smoke, resolve explosions, flashover)
