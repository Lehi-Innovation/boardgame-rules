...
player_count: "1-6"
play_time: "30-60 min"
source_pdf: "dorfromantik-the-board-game-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/dorfromantik-the-board-game-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Dorfromantik%3A%20The%20Board%20Game&game=dorfromantik-the-board-game)
<!-- verification:end -->
...
1. Shuffle all task tiles face-down. Form stacks at the edge of the playing area.
2. Shuffle all landscape tiles face-down. Draw 3 and return to the box unseen. Form stacks from the remaining tiles.
3. Shuffle the 25 task tokens face-down, separated by color/symbol. Place next to task tile stacks.
4. Set aside scoring and campaign sheets for end of game.
5. Do NOT open the 5 mystery boxes until instructed.
...
On your turn:
1. **Check task tile count:** If fewer than 3 task tiles with tokens are in the play area, reveal 1 task tile. Otherwise, reveal 1 landscape tile.
2. **Place the revealed tile** in the play area following placement rules.
...
| Task Token Numbers | 4, 5, or 6 tiles exactly |
|---|---|
| Task Types | Forest, Grain, Village, River, Railroad |
| Must-match edges | River, Railroad |
| Free-match edges | Forest, Grain, Village, Meadow |
| Active tasks in play | Always maintain 3 |
| First 3 turns | Always task tiles |
| Flags | Score only if area is closed |
| Campaign | Unlock mystery boxes at point thresholds |
