...
source_pdf: "exit-the-game-dead-man-on-the-orient-express-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/exit-the-game-dead-man-on-the-orient-express-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Exit%3A%20The%20Game%20%E2%80%93%20Dead%20Man%20on%20the%20Orient%20Express&game=exit-the-game-dead-man-on-the-orient-express)
<!-- verification:end -->
...
### Solving a Riddle
- Identify a locked object in the book or on a Riddle Card (marked with a symbol matching the decoder disk edge).
- Determine the three-digit code through logical deduction from the available clues.
...
- **1st Clue** — Tells you which Riddle Cards you need and gives an initial hint.
- **2nd Clue** — Provides more concrete guidance toward the solution.
- **Solution** — Reveals the answer outright.
...
- **One-time play:** The game can only be played once since materials are permanently altered.
- **Logical solutions only:** All codes can be solved logically; do not brute-force combinations on the disk.
- **Card management:** Always return Answer Cards to the pile after checking them, whether the code was correct or incorrect.
- **Progressive discovery:** Some riddles require Riddle Cards that have not yet been revealed. If stuck, work on a different riddle first.
- **Set aside used materials:** Keep successfully used materials visible (not put away) for reference, but do not mix them with unsolved puzzles.
- **Room illustrations** (train cars and compartments) may be needed for multiple riddles.
- **You must find an object before opening it** — you cannot enter a code for a symbol if you have not yet seen that symbol on a Riddle Card or in the book.
