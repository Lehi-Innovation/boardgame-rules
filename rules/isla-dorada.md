...
player_count: "3-6"
play_time: "60-90 min"
designer: "Bruno Faidutti, Alan R. Moon, Andrea Angiolino, Piergiorgio Paglia"
source_pdf: "isla-dorada-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/isla-dorada-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Isla%20Dorada&game=isla-dorada)
<!-- verification:end -->
...
1. Place the board; put 16 exploration markers on numbered spaces (red on spaces 4 and 9).
2. Place the expedition figure at base camp.
3. Each player takes 10 gold.
4. Separate goal cards by type and shuffle each deck.
5. Deal each player: 3 starting treasure cards, 2 destiny cards, 1 curse card.
6. Each player keeps 2 starting treasures, 1 destiny card; discards the rest (face down).
7. Shuffle and deal 6 adventure cards per player. Place rest as a face-down deck with 4 face-up cards.
8. Discard and replace any bolt-symbol cards in starting hands or face-up display.
9. Youngest player takes the leader token.
...
| Card | Travel Type | Notes |
|------|-------------|-------|
| Yak | Mountain path | 1 card = 1 bid value |
| Kamel | Sand path | 1 card = 1 bid value |
| Gonogo | Jungle path | 1 card = 1 bid value |
| Kayak | Water path (river/lake/sea) | 1 card = 2 bid value |
| Drako | Air travel / joker | Value 2, costs 1 gold |
| Zeppelin | Air travel / joker | Value 5, costs 3 gold |
| Shaman | Changes animal card type | Play with identical animal cards |

### Special Cards

| Card | Effect |
|------|--------|
| Ovetos | End bidding immediately (requires bid of 4+) |
| Juju | Remove target player from bidding this round |
| Sangaia | During movement, continue on another same-type path instead of stopping |
| Bonus | +3 points when played with matching treasure card |
| Bongo | Play 1 treasure card of a neighboring site |

### Bolt-Symbol Cards (immediate effect on draw)
...
| Phase | Action |
|-------|--------|
| 1 | Bid adventure cards for expedition movement |
| 2 | Move expedition + place exploration marker |
| 3 | Play curse cards (mandatory) |
| 4 | Play treasure cards (optional) |
| 5 | Cable news (rounds 4 & 9 only) |
| 6 | Draw adventure cards (1 free + 1 for 1 gold) |
