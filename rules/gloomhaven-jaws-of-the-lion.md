...
- 1 Scenario Book (with hex maps printed inside)
- 1 Supplemental Scenario Book
- 1 City Map Board with stickers
- 4 character boxes containing: character mat, figure, ability card decks (level A through 9), attack modifier decks, character sheets, initiative order tokens
- Monster zip bags with: standees, stat cards, ability decks, initiative order tokens
- 6-section stat sleeves (for monster stat cards)
- Plastic stands (white for normal, gold for elite monsters)
- Token tray with: damage tokens, money tokens, condition tokens (Immobilize, Stun, Muddle, Strengthen, Poison, Wound, Disarm, Curse, Bless), destruction tiles, trap tiles, treasure tiles, door tiles, obstacle tiles
- 4 hit point dials
- Attack modifier cards (character decks + monster deck "M" + Curse/Bless cards)
- Battle goal cards
- Event cards
- Item cards
- Element board with element tokens
- Sticker sheets
...
### 4. End of Round
- Remove end-of-round conditions.
- Characters may perform a short rest (shuffle discard, randomly lose one card permanently to hand; may pay 1 HP to lose a different card instead).
- Shuffle attack modifier decks if a x2 or null card was drawn.
- Advance elements on the element board (if used).
...
- **Melee:** Targets adjacent enemies (no Range specified).
- **Ranged:** "Range X" under attack. Targets enemy within X hexes. Ranged attacks on adjacent enemies gain Disadvantage.
- **Target X:** Attack up to X different enemies within range (separate modifier card for each).

### Attack Modifier Cards
Every attack flips one card from the attacker's modifier deck:
- Numerical modifier: Add/subtract from attack value
- x2 card: Double the attack value
- Null card: Attack does 0 damage
- Both x2 and null trigger deck shuffle at end of round
...
- Cannot stack; Advantage and Disadvantage cancel each other.
- Ranged attacks on adjacent targets always have Disadvantage.

### Conditions
- **Immobilize:** Cannot perform Move abilities.
- **Stun:** Cannot perform any abilities.
- **Muddle:** All attacks gain Disadvantage.
- **Disarm:** Cannot perform Attack abilities.
- **Poison:** All healing has no effect; add +1 to all attack damage suffered.
- **Wound:** Suffer 1 damage at start of each turn.
- **Strengthen:** All attacks gain Advantage.
- **Curse:** Add curse cards (-null) to attack modifier deck.
- **Bless:** Add bless cards (x2) to attack modifier deck.
...
- Complete scenarios to unlock new ones (place stickers on city map).
- Earn gold to buy items at the city shop.
- Gain experience to level up characters (gain new ability cards, increase HP, earn perks to improve attack modifier deck).
- Perks: Earned by completing battle goals and leveling up. Each perk modifies your attack modifier deck (add better cards, remove worse ones).

## Special Rules & Edge Cases

### Character Exhaustion
A character is exhausted (eliminated for the scenario) when:
- Their HP reaches 0 and they cannot negate the damage by losing cards, OR
- They have no cards in hand at the start of a round and fewer than 2 cards in their discard pile (cannot select 2 cards to play).

### Short Rest vs. Long Rest
- **Short Rest** (end of round): Shuffle discard, randomly lose 1 card to lost pile, draw rest back to hand. May pay 1 HP to lose a different random card instead.
- **Long Rest** (replaces card selection): Initiative 99. Heal 2 HP. Recover items. Shuffle discard, choose 1 card to lose, draw rest back to hand.
...
- **Consumed:** One use per scenario (flipped facedown).
- **Spent:** Used once, recovered during long rest.
- **Free:** No cost to use, always available.

### The Four Characters
| Character | Role | Key Mechanics |
|-----------|------|---------------|
| **Demolitionist** | Melee damage, obstacle destruction | Destroys obstacles, area damage |
| **Hatchet** | Ranged damage, looting | Thrown attacks, retrieval mechanics |
| **Red Guard** | Protection, monster manipulation | Shield, positioning control |
| **Voidwarden** | Healing, support | Grants actions to allies/enemies, healing |
...
| Item | Value |
|------|-------|
| Starting ability cards | 6 (level A) |
| Starting modifier deck | 20 cards |
| Setup hand for items | Up to 8 cost |
| Short rest HP cost | 1 (to reroll lost card) |
| Long rest initiative | 99 |
| Long rest heal | 2 HP |
| Scenarios in campaign | 25 |
