...
source_pdf: "fearsome-floors-rules.pdf"
...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Starting player tile direction reversed: summary says clockwise, source says to the left (counterclockwise). Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/fearsome-floors-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Fearsome%20Floors&game=fearsome-floors)
<!-- verification:end -->
...
- 1 Folded game board (fortress plan)
- 17 Floor tiles (basic: stones, blood pools; advanced: crystals, turning stones, teleporters)
...
1. Place the board centrally. Shuffle monster movement tiles face-down nearby.
2. Each player takes all tokens of one color:
   - **2-4 players:** 4 tokens each (values 6/1, 4/3, 3/4, 2/5)
   - **5-7 players:** 3 tokens each (values 6/1, 3/4, 2/5)
3. Place all tokens near the entrance with white (colored) numbers face-up.
4. Assemble the monster; place it on the corner space by the exit, facing toward the "M" on the board edge.
5. **Basic game:** Use stones and blood pools only. Place them per the diagram. Remove teleporters.
...
1. Reveal the top monster movement tile.
2. The monster moves that many spaces (or until it eats 1-2 tokens for "hit" cards).
3. Each step: the monster **looks** straight, left, and right (never backward, never diagonally), then **moves** one space.
4. If it sees a token, it turns toward the closest one and advances. Equal distances: it keeps its current direction.
5. After the last step, the monster looks one final time.
6. The monster **cannot look through stones** (but can look through crystals in the advanced game).
7. Pass the starting player tile clockwise.
...
|------|---------|
| Players | 2-7 |
| Tokens per player | 4 (2-4p) or 3 (5-7p) |
| Token values | Sum of both sides = 7 |
| Tokens needed to escape | 3 (2-4p) or 2 (5-7p) |
| Monster movement | 5, 7, 7, 8, 8, 10, 1-hit, 2-hit |
| Monster vision | Straight, left, right (never back/diagonal) |
| Stages | 1st: eaten tokens return; 2nd: eaten tokens removed |
