...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/dominant-species-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Dominant%20Species&game=dominant-species)
<!-- verification:end -->
...
- 1 game board with hex grid, action display, VP track, and bonus points table
- 6 animal displays (one per animal class)
- 27 cards: 1 Survival card + 26 Dominance cards (including Ice Age)
- 31 large hexagonal terrain tiles + 12 small tundra tiles
- 330 wooden cubes (species) in 6 colors (55 per animal)
- 60 wooden cylinders (action pawns) in 6 colors (10 per animal)
- 60 wooden cones (dominance markers) in 6 colors (10 per animal)
- 120 round element markers (6 types: grass, grub, meat, seed, sun, water)
- 6 square initiative markers
- 1 cloth draw bag
...
- **Domination scoring:** Tile scoring during Domination actions (VPs vary by terrain: sea is richest, tundra is poorest)
- **Bonus VPs:** From Glaciation, Wanderlust, and the Survival card (use the Bonus Points Table on the board)
- **Dominance cards:** Various one-time VP effects
- **Survival card:** Held by the player with the sole most species on tundra tiles; scores bonus VPs each Reset Phase
...
- **Food chain order (highest to lowest):** Mammal, Reptile, Bird, Amphibian, Arachnid, Insect. Used for tie-breaking and some card effects.
- **Dominance:** An animal is dominant on a tile if it has species there and matches more elements than any other animal with species there. Ties mean no animal is dominant. Endangered species cannot claim dominance. Dominance must be verbally claimed and physically marked.
- **Endangered species:** Species on a tile where their animal matches zero elements. Eliminated during Extinction each turn.
- **Matching elements:** Multiply each element type on your animal display by the count of that type on the tile's corners. Sum all types.
- **Species removed from earth** (except by Glaciation) are placed in the box permanently, NOT back to gene pools. Species removed by Glaciation return to gene pools.
- **Actions are never mandatory:** A player may forfeit any action when their AP is removed.
- **Some actions trigger even without APs:** Regression, Wasteland, Speciation (insect free species), Competition (arachnid free competition).
- **Animals cannot be eliminated:** A player with no species on earth and none in gene pool still plays (still has AP placement and VP total).
- **Max 6 elements per animal display** (including defaults).
- **2-3 player variant:** Each player controls multiple animals (2 or 3 each). All 6 animals are used. Final score = your lowest-scoring animal's VP total.
