...
designer: "Hal Eccles"
source_pdf: "mythos-tales-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/mythos-tales-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Mythos%20Tales&game=mythos-tales)
<!-- verification:end -->
...
1. Choose an investigation (recommended: play in order, starting with "A Grain of Evil").
2. Unfold the map of Arkham and place in center.
3. Place Requirement Cards in a stack near the map.
4. Place the Time-Tracker; set the Time Limit X marker per investigation instructions.
5. Place the Progress marker nearby (moves to Day 1, Morning when players first travel).
6. Find the newspaper matching the investigation date; also place any previous newspapers nearby.
7. Select a first player; give them the Investigation Book, Directory, and Ally list.
...
1. **Travel Step:** Discuss as a group, then the lead player selects a location coordinate (letter + number) using the map, directory, or ally list. Advance the Progress marker one space on the Time-Tracker. (First travel places it on Day 1, Morning.)
2. **Encounter Step:** Look up the coordinate in the Investigation Book and read the entry aloud. Follow any instructions in the text.

### Solution Phase
1. **Question and Answer Step:** Answer questions based on notes and memory. Do not re-read materials.
2. **Scoring Step:** Compare answers to solutions. Calculate points, apply penalties.
...
| Time-Tracker Flow | Sequence |
|---|---|
| Day progression | Morning -> Afternoon -> Evening -> Next Day Morning |
| Progress marker | Advances 1 space per location visit |
| X marker | Investigation ends when reached |

| Scoring | Points |
|---|---|
| Correct answers | As indicated |
| Sanity penalties | Per investigation |
| Excess time | -1 per space past X |
| Excess locations | -1 per location beyond Armitage's count |
