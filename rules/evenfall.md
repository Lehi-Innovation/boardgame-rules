...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: SOURCE_MISMATCH: extracted text is the Evenfall Crystal Path expansion rulebook not the base game. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/evenfall-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Evenfall&game=evenfall)
<!-- verification:end -->
...
1. Place the three Region boards centrally. Populate each Region with Places of Power cards and 4 random Powerstones.
2. Each player takes a Clan board (choose the basic Sun side for first game, or the advanced Moon side), a Mana dial, Witches, and Elders.
3. Each player receives starting resources: 3 Herbs and 3 Potions.
4. Prepare the card decks (Rituals, Specialists, Council Members) and place them in supply areas.
5. Determine a first player.
