...
play_time: "10-15 min"
designer: "Unknown"
source_pdf: "rubik-s-flip-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rubik%27s%20Flip&game=rubik-s-flip)
<!-- verification:end -->
...
- 8 Red/Blue Rubik's tiles (double-sided, one player)
- 8 Yellow/White Rubik's tiles (double-sided, other player)
...
- Each player has 8 tiles and the board has 16 spaces, so all tiles will be placed by game's end if no winner emerges earlier.
- A tile is locked only when all adjacent spaces are occupied; edge and corner tiles are easier to lock since they have fewer adjacent spaces.
- The game has a maximum of 8 turns per player.
- Strategic placement near edges and corners helps lock tiles faster.
- A player cannot flip a locked tile; they must choose an unlocked opponent tile.
- If a player has no unlocked opponent tiles to flip, they skip the flip step and only place a tile.
