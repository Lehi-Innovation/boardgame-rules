...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/biblios-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Biblios&game=biblios)
<!-- verification:end -->
...
- For each card drawn, the active player must immediately assign it to one of three locations:
  1. **Their hand** (keep for themselves) -- exactly 1 card per turn
  2. **Public space** (face-up, available to other players)
  3. **Auction pile** (face-down, saved for Phase 2) -- exactly 1 card per turn
