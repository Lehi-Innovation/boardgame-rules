...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/ghosts-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Ghosts&game=ghosts)
<!-- verification:end -->
...
1. Each player starts with 1 Fear point — place a Fear marker in front of them with value 1 visible.
2. The start player is the last person to have seen a ghost. In subsequent rounds, the start player moves to the left.
3. The start player shuffles all cards and deals 5 to each player. Remaining cards form a face-down draw pile in the center.
4. If a player's hand consists entirely of Mist cards, shuffle them back into the draw pile and draw a new hand of 5 cards.
...
2. Announce the current Fear level based on the card played (see Actions below).
3. Draw a card from the draw pile to return your hand to 5 cards.
...
1. Take the entire discard pile and place it face-down in front of you.
2. Take 1 Fear point from the supply as a penalty.
3. Play a new card from your hand, announcing the Fear level equal to the number of ghosts on that card (starting a new pile).
4. Play continues in the same direction as before.
