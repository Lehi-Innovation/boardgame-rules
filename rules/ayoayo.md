...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Ayoayo&game=ayoayo)
<!-- verification:end -->
...
1. Place the board between the two players so each player has a row of 6 pits closest to them.
2. Place exactly 4 seeds in each of the 12 pits (48 seeds total).
3. Each player's store is the large pit to their right.
...
1. Choose one of your 6 pits that contains seeds.
2. Pick up all seeds from that pit.
3. Distribute (sow) the seeds one at a time, counterclockwise, into consecutive pits. On your own row, move left to right. When you reach the end, continue right to left on your opponent's row, then back to your row, and so on.
4. **Do not** drop seeds into either store during sowing.
...
| Term | Meaning |
|------|---------|
| Sow | Distribute seeds one per pit counterclockwise |
| Relay | If last seed lands in occupied pit, pick up all and continue |
| Capture | Last seed in empty pit on own side: take opposite pit's seeds |
| Store | Personal scoring pit to your right |
