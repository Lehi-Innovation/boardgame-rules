...
player_count: "2-6"
play_time: "60-90 min"
designer: "Bruce Nesmith, Andria Hayday"
source_pdf: "dragonstrike-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/dragonstrike-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20DragonStrike&game=dragonstrike)
<!-- verification:end -->
...
- VHS Video Cassette
- 2 game boards (printed on both sides for 4 settings: castle, valley, cavern, city)
- Instruction Book and Adventure Book
- Map Book (Dragon Master's secret maps)
- 6 Hero Cards (warrior, wizard, male thief, female thief, elf, dwarf)
- 110 Playing Cards: Hero Spell Cards (First/Second/Third Level), Sneak Attack Cards (5), Teraptus Spell Cards (10), Magical Treasure Cards (38), Trap Cards (12), Monster Cards (10), Hero Reference Cards (5)
- 24 plastic figures: 6 heroes, 1 dragon (Darkfyre), 1 evil wizard (Teraptus), 1 manscorpion, 1 fire elemental, 1 troll, 3 orcs, 3 bugbears, 1 giant, 2 gargoyles, 3 Death Knights
- Dragon Master Screen
- 24 plastic clips and 24 plastic bases (12 small, 10 large)
- 43 cardboard markers (doors, treasure, etc.)
- 3 dice: 1 blue 8-sided (d8), 1 white 10-sided (d10), 1 black 12-sided (d12)
...
### A Hero's Turn
A hero can either:
- **Move, then perform one action**, OR
- **Perform one action, then move**

You may also skip any portion of your turn (pass).

### The Dragon Master's Turn
- Move any or all monsters on the game board during your turn.
- A monster's turn: move then attack, OR attack then move.
- Only the evil wizard (Teraptus) can cast spells instead of attacking.
- Move the Hourglass Marker one space on the Time Track toward the dragon picture. When it reaches the picture, Darkfyre enters the game board.
...
- If no treasure is at that location, the Dragon Master says so.
- If a "random" treasure exists, draw from the Treasure Deck.
- If the Adventure Book specifies a treasure for that location, receive that specific treasure.
- Searching the same location again finds nothing.
- A monster cannot be searched until it is dead, asleep, or tied up.
...
- **Movement:** Every figure has a Speed value = maximum spaces per turn. Movement is in any direction, including diagonal. You cannot pass through walls, closed doors, or spaces containing enemies (but can pass through friendly spaces). Only one figure per space.
- **Flying and Hovering:** Figures with flight can glide over terrain obstacles (forests, boulders, water, cliffs, portcullis). Flying figures do not block line of sight. A hovering figure (Wing Marker) cannot be targeted by normal melee attacks. Attacks from air to ground and vice versa are long-range.
- **Leaving the Board:** A hero can exit via squares marked on the Dragon Master's adventure map. Figures cannot re-enter once they leave.
- **Triggering Traps:** Some squares contain hidden traps. If a hero steps on one, draw a Trap Card immediately. The trap triggers once, then disappears. Monsters never trigger traps.
- **Doors:** Opening a door is free (not an action). If it is unlocked, it opens automatically. If locked, a hero must perform a Feat to open it. A hero can close a door if they are in a space touching it and no monster is also touching it. Closing is free. The evil wizard and dragon can also open/close doors. The dragon breaks any lock automatically.
- **Dying:** A monster that reaches 0 Hit Points is dead. A dead hero is out of the game. A dead monster remains on the board for 2 rounds (searchable for treasure), then the Dragon Master removes it.
- **Line of Sight:** Draw a line from the center of your square to the center of the target square. Walls, closed doors, boulders, and forests always block sight. Heroes and monsters do not block line of sight but do block the path for long-range attacks and spells.
- **Darkfyre (the Dragon):** Speed 12, Armor Class 8. Can open/close doors, break locks, breathe fire (targets 3 spaces in a breath area, hitting every space in between; diagonal allowed; figures save vs. magic or lose 2 HP; those who save lose 1 HP). Darkfyre can also launch a fireball at up to 10 squares. Has 3 regular attacks per turn: d10 claw + d8 claw + d12 bite. Covers 2x2 squares. When counting squares in Darkfyre's move, only the front of the figure counts.
- **Special Terrain:** Portcullis (blocks movement and attacks, not spells or sight), Stairs (connect floors when "in play"), Water (entering = must stop; wading downstream = 1 space/turn; leap across via Feat of Dexterity), Boulders (block movement and sight; movable via Feat of Strength or levitation spell), Forest (each square costs double movement; hinders sight), Cliffs (cross via Feat of Dexterity; cliffs do not block line of sight except at the edge).
- **Hourglass/Time Track:** The Dragon Master advances the Hourglass Marker each turn. When it reaches the dragon picture, Darkfyre enters the board.
