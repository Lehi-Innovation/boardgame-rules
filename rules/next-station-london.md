...
source_pdf: "next-station-london-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/next-station-london-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Next%20Station%3A%20London&game=next-station-london)
<!-- verification:end -->
...
- 1 Pad of London maps
- 4 Colored pencils (purple, blue, pink, green)
- 11 Station cards: 6 Street cards (blue background) and 5 Underground cards (pink/yellow background)
- 5 Shared Objective cards (advanced module)
- 4 Pencil Power cards (advanced module)
...
- The first section must start from your departure station and connect to a station matching the flipped card symbol.
- Subsequent sections extend from either end of your line to a matching station.
- Sections must be straight lines (horizontal, vertical, or diagonal) following the grey potential lines on the map.
- Drawing is always optional; you may skip a turn.
- You cannot pass through a station already on the same line.
- You cannot draw a duplicate section between the same two stations.
- Sections cannot cross over previously drawn sections (any color).
...
- Railroad Switch on turn 1 or 2 of a round is ignored.
...
- **13 districts:** 9 main districts + 4 secondary (corner) districts with 1 station each.
- **4 station types:** Square, Triangle, Pentagon, Circle.
- **Round end trigger:** The 5th Underground card (pink/yellow background) ends the round.
- **Solo mode:** Place 4 pencils in front of you in chosen order. Play all 4 rounds using each pencil in order. Compare score to achievement table.
- **Tourist site cap:** Maximum 10 circles can be crossed off (max 25 tourist points).
...
- Treat any card as a Joker
- Draw a second section to the same symbol on the same turn
- Treat any card as having Railroad Switch
- Circle one station to count double for route scoring in that district
