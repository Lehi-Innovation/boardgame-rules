...
source_pdf: "river-dragons-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20River%20Dragons&game=river-dragons)
<!-- verification:end -->
...
- 1 double-sided game board (one side with pre-placed stones, one side open for free placement)
- 6 colored pawns
- 36 numbered planks (6 per player, in various lengths)
- 27 stones
- 78 action cards (13 per player, including dragon cards)
- 1 first player card

## Setup

1. Choose a board side. The side with pre-placed stone positions is recommended for beginners.
2. Each player selects a color and takes the matching pawn, 6 planks, and 13 action cards.
3. Remove dragon cards matching the colors of absent players from each player's deck.
4. Each player places their pawn on their starting village (positions assigned based on player count).
5. The youngest player receives the first player card; it rotates clockwise each round.
...
**Phase 1 -- Programming:**
All players simultaneously select 5 action cards from their hand of 13 and arrange them face-down in a row from left to right in their desired execution order. Cards cannot be changed once placed.

**Phase 2 -- Action Resolution:**
All players reveal their first card simultaneously. Starting with the first player and proceeding clockwise, each player executes their revealed action. Then all reveal their second card, and so on through all 5 cards.
...
- If a player cannot complete a movement action (no valid moves), their pawn falls into the river and returns to the starting village.
- Planks may extend over the edges of stones but must rest stably on both supports.
- When removing a stone, all planks it supports would also fall; this is not allowed -- you cannot remove a stone that is supporting planks.
- Dragon cards can only target players whose colors are in the game.
- The programming phase creates opportunities for bluffing and anticipating opponents' moves.
- In rare cases, a plank might be too short to reach between two stones; it must be placed in a different valid position or the action is wasted.
