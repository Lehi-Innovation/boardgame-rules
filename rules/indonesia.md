...
source_pdf: "indonesia-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/indonesia-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Indonesia&game=indonesia)
<!-- verification:end -->
...
- 1 Game board (Indonesian archipelago with land areas, sea areas, provinces)
- 5 Player aids
- 30 Player markers (6 wooden disks per color, 5 colors)
- 48 Ships (6 sets of 8)
- 23 City markers (12 size 1/green, 8 size 2/yellow, 3 size 3/red)
- 24 Company deeds (sorted into eras a, b, c)
- 214 Goods markers (54 rice, 44 spice, 40 rubber, 40 siap faji, 36 oil)
- 15 City cards (3 per player, one per era: a, b, c)
- Money: coins (1 and 5 rupiah) and banknotes (10, 50, 100, 500 rupiah)
...
- Players place new cities (in order of play) using their city card for that era. Cities must be built in an empty coastal land area in one of the 3 indicated provinces. Each province may only contain one city. New cities start at size 1.
- Discard remaining unacquired companies from the previous era; place next era's deeds on the map.
...
- **Production company:** Place 1 goods counter in the indicated province on any empty land area. Cannot be placed adjacent to the same goods type.
- **Shipping company:** Place 1 ship in a sea area bordering the indicated province.
...
- Companies that cannot start (no valid starting area) are discarded, which may trigger a new era earlier.
- Each company deed can only be involved in one merger per year.
- Merged companies occupy only one slot but operate as one entity.
- Goods cannot be transported over land to cities, even if adjacent; shipping is always required.
- If a company has multiple production zones (from merger), each zone needs its own shipping chain. Goods cannot move between non-adjacent zones.
- Goods can move freely between adjacent areas within a single production zone.
- A production company cannot expand such that two companies producing the same good would have adjacent areas.
- Ships can be used anew by each production company operating that turn, but each ship can only transport 1 good per production company (modified by Hull R&D).
- It is allowed to pass on announcing mergers and rejoin later in the same phase.
- Once you pass on acquisitions, you cannot acquire again that phase.
- A player may announce a merger of companies they don't own (if they can hold the result).
- **2-player game:** Each player gets 2 city cards per era and builds 2 cities each in the New Era phase.
- Money in the "Bank" box from turn order bids is permanently spent but counts toward final score.
