---
title: "Microworld"
bgg_id: 203042
player_count: "2"
play_time: "15-30 min"
designer: "Paolo Vallerga, Martino Chiacchiera"
source_pdf: ""
extracted_date: "2026-03-21"
summarized_date: "2026-03-21"
verification: "unverifiable"
verification_date: "2026-06-12"
---

# Microworld

<!-- verification:begin -->
> ⚠️ **Unverifiable** — no source rulebook text is available to check this AI-generated summary against. Treat all details with caution.
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Microworld&game=microworld)
<!-- verification:end -->

## Overview

Microworld is a two-player tile-laying strategy game where one player controls viruses and the other controls antibodies. Players place tiles on a shared board to control areas and score points. On each turn, you place a tile next to the tile your opponent just placed. Special tokens on the board can be collected and used as special actions. Points are earned for tiles in neighborhoods, and at game's end, players score for majority in each area.

## Components

- Game tiles in 2 types (virus and antibody)
- Game board or play area
- Special action tokens
- Score track

## Setup

1. Place the starting tile in the center.
2. Each player takes their set of tiles.
3. Place special action tokens on designated spaces.
4. Determine first player.

## Turn Structure

Players alternate turns. On your turn:
1. Place one of your tiles adjacent to the tile your opponent placed on their last turn.
2. Collect any special token on the space where you placed.
3. Optionally use a collected special token for a special action.

## Actions

### Tile Placement
- Place your tile orthogonally adjacent to your opponent's last-placed tile.
- Your tile extends your influence in that area of the microworld.

### Special Tokens
- Tokens on the board provide special one-time abilities when collected.
- Use collected tokens on any future turn for their effect.

### Area Control
- The board is divided into areas.
- Score points for having majority presence in each area at game's end.

## Scoring / Victory Conditions

At the end of the game (when all tiles are placed or no legal moves remain):
- Score points for your tiles in each neighborhood.
- Score bonus points for majority in each area.
- The player with the most points wins.

## Special Rules & Edge Cases

- You must place adjacent to your opponent's last tile, creating an interactive back-and-forth dynamic.
- Special tokens add tactical depth beyond simple tile placement.
- The game has expansions (First Expansion, BGG Promos) that add new tokens and tiles.

## Player Reference

**Turn:** Place tile adjacent to opponent's last tile -> Collect special token (if any) -> Use token (optional)

**Scoring:** Points per tile in neighborhoods + majority bonuses per area

**Win condition:** Most points at game end
