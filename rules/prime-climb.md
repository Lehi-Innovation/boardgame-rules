---
title: "Prime Climb"
bgg_id: 159375
player_count: "2-4"
play_time: "10 minutes per player"
designer: "Daniel Finkel"
source_pdf: "prime-climb-rules.pdf"
extracted_date: "2026-03-22"
summarized_date: "2026-03-22"
verification: "unverified"
verification_date: "2026-06-12"
---
<!-- verification:begin -->
> ⚠️ **Unverified** — this AI-generated summary has not yet been fact-checked against the rulebook. Double-check critical rules against the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/prime-climb-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Prime%20Climb&game=prime-climb)
<!-- verification:end -->


## Overview

Prime Climb is a mathematical board game where players race to get both their pawns from Start (0) to the center circle (101) by adding, subtracting, multiplying, or dividing with dice rolls. The board's color coding teaches number relationships: each number's colors show its prime factorization. Landing on prime numbers earns Prime Cards.

## Components

- 1 Prime Climb board (spiral from 0 to 101)
- 8 pawns (2 per player)
- 2 ten-sided dice (0 = 10)
- 24 Prime Cards
- 4 blank Prime Cards
- 1 multiplication table

## Setup

1. Lay out the board. Shuffle the 24 Prime Cards and place them nearby.
2. Each player places both pawns on Start (counts as 0).
3. Roll dice to determine first player.

## Turn Structure

A turn consists of four phases:

### 1. Roll
Roll both dice. The two numbers are used one at a time (not combined). If you roll doubles, you may use that number four times instead of twice. The "0" on the die stands for "10." You must use all your rolls.

### 2. Move
Apply each die number to one of your pawns using any arithmetic operation:
- **Add** the rolled number to your pawn's current position
- **Subtract** the rolled number from your pawn's current position
- **Multiply** your pawn's current position by the rolled number
- **Divide** your pawn's current position by the rolled number (only if the result is a whole number)

Each die is applied one at a time (you cannot multiply your two dice together). You may apply both dice to the same pawn or one to each. Keeper cards may be played before, between, or after dice rolls. Pawns may never move to negative numbers, non-whole numbers, or numbers greater than 101.

### 3. Bump
If either of your pawns ends on the same space as another pawn (including your own), send the other pawn back to Start. Bumping is mandatory. You only bump when ending your move on an occupied space, not when passing through.

### 4. Draw
After moving and bumping, draw 1 Prime Card if:
- At least one of your pawns is on an entirely red space (a prime number greater than 10), AND
- That pawn did not start the turn on that space.
Draw only 1 card per turn, even if both pawns land on red spaces.

## Actions

### Prime Cards
- **Action Cards:** Immediately perform the action when drawn. If the card moves your pawn to an occupied space, bump that pawn to Start. Landing on a new red space from an Action Card does not trigger another draw.
- **Keeper Cards:** Keep face-up for a future turn. May play any number during your Move phase (not on the turn drawn).

## Scoring / Victory Conditions

The first player to land **both** pawns on 101 exactly wins. You may never move a pawn past 101.

## Special Rules & Edge Cases

- You must use ALL your rolled numbers each turn.
- "0" on the die = 10.
- Multiplying by 0 is allowed (sends a pawn to 0/Start).
- Adding 0 or subtracting 0 is allowed (no movement).
- You cannot divide if the result would not be a whole number.
- Doubles grant 4 uses of that number instead of 2.
- Bumping your own pawn is allowed and mandatory.
- No card trading between players.
- If the Prime Card deck runs out, shuffle the discard pile.
- Both pawns must land on 101 exactly (not pass through).

## Player Reference

| Phase | Action |
|-------|--------|
| 1. Roll | Roll 2 ten-sided dice (0 = 10); doubles = 4 uses |
| 2. Move | Add, subtract, multiply, or divide pawn position by each die value |
| 3. Bump | Send any pawn on your space back to Start (mandatory) |
| 4. Draw | Draw 1 Prime Card if a pawn newly occupies a prime > 10 |

**Win:** Both pawns on 101 exactly.
