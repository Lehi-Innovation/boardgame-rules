...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/celtica-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Celtica&game=celtica)
<!-- verification:end -->
...
- 1 game board (19 locations connected by paths)
- 5 druid figures (shared by all players)
- Druid cards (in 5 colors matching the druids)
- Experience cards
- Amulet tiles (multiple types, each divided into several pieces)
- Rules sheet
...
**Strongholds (Castles, Monasteries, Villages, Journey's End):**
Take the number of amulet tiles indicated at the stronghold from the face-up display. Place them in front of you in the correct pattern. If a new tile matches a type you already have, it must start a **separate** new amulet (you cannot combine duplicates). Restock the display to 9 tiles.

**Ruins:**
Vikings steal amulet pieces. Discard the number of tiles shown at the ruin (your choice which tiles, even from completed amulets). Even if you have no tiles, you still receive 1 experience card from the draw pile. Discarded tiles go to a separate face-down pile.

**Shrines:**
You may (optionally) draw 1 druid card from the display. This card must be played during the current round. Taking extra cards gives more movement options but may force undesirable moves.
...
- Spend 1 experience card to swap one of your amulet tiles with one from another player's display.
- Spend 2 experience cards to draw 1 additional amulet tile from the 9-tile display.
...
The game ends when:
- The first druid reaches the Journey's End location (finish the current round), OR
- All druids have reached Journey's End, OR
- The amulet tile supply (display and draw piles) is exhausted.
...
- If a druid has already reached Journey's End, playing cards of that color has no effect (cards are discarded).
- If the druid card draw pile is exhausted, shuffle the discard pile to form a new draw pile.
- If the experience card draw pile is exhausted, shuffle its discard pile for a new draw pile.
- If the amulet tile supply runs out, shuffle discarded tiles to form a new face-down supply.
- If only one player has druid cards remaining in a round, they take consecutive turns until all cards are played.
- A player with only experience cards and no druid cards cannot play them; the round is over for that player. Experience cards are retained for future rounds.
...
- 1 card = swap 1 of your amulet tiles with another player's
- 2 cards = draw 1 amulet tile from display
