...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20HitStory&game=hitstory)
<!-- verification:end -->
...
2. Insert the DVD into a player connected to a TV.
3. Shuffle question cards by category.
4. Each player selects a token and places it at the start.
5. Determine who goes first.

## Turn Structure

On your turn:
1. **Roll the die** and move your token on the board.
2. **Draw a question** from the category indicated by the space you land on.
3. **Answer the question** — the DVD may play an audio clip, show an image, or present a video question.
4. **Score** — If correct, collect a card from that category. The DVD tracks scoring.
...
| Win Condition | Collect 3 cards from each of 6 categories |
| Question Types | Text, audio clip, image, video |
| DVD Required | Yes |
