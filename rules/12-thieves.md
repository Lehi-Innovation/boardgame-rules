...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%2012%20Thieves&game=12-thieves)
<!-- verification:end -->
...
- Wooden thief meeples (multiple colors, 12 per player)
- Colored guard pieces (per player)
- Black neutral guards (6)
- Palace cards (6 suits matching the 6 palaces)
- Dancer cards (wild cards, usable as any palace)
- 6 Treasure chest stacks (4 chests per stack, showing 4, 5, 6, and 7 people required)
- Gameboard with 6 palace locations
...
- If you performed any actions: draw 3 cards.
- If you performed no actions: draw 4 cards and may choose 1 as a Dancer (wild) card.
- When the draw deck is exhausted, shuffle the discard pile to form a new deck.
...
- Guards block thieves — each guard at a palace costs 1 additional card to place a thief there.
- Guard spaces at palaces are limited; you cannot move a guard to a palace with no open guard space.
- You may only perform up to 3 thief-related actions per turn, but guard movements are separate.
- If you cannot or choose not to perform any actions, you get a better card draw (4 cards + potential Dancer).
- Multiple players may have thieves at the same palace, but only the player whose thief count meets the treasure requirement gets to steal.
...
|---|---|
| Place thief | 1 per guard at that palace |
| Move own guard | 1 (matching current or destination) |
| Move guard + thief | 1 (matching current or destination) |
| Move neutral guard | 2 (1 for current, 1 for destination) |
