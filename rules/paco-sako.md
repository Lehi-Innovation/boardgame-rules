...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Paco%20Sako&game=paco-sako)
<!-- verification:end -->
...
- 32 specially designed interlocking pieces (16 White, 16 Black): King, Queen, 2 Bishops, 2 Knights, 2 Rooks, 8 Pawns per side
...
- **No captures:** Pieces are never removed from the board. All 32 pieces remain in play throughout.
- **Unions stay on the board:** A union occupies one square with both pieces interlocked.
- **A piece in a union can move using either piece's movement:** This creates dynamic, changing movement possibilities.
- **Chain reactions** can create complex sequences of unions forming and breaking.
- **Pawns:** Follow standard pawn movement rules (forward 1, capture diagonally 1, optional double first move). Can promote upon reaching the 8th rank.
- **No check:** There is no concept of check -- the game simply ends when the King is embraced.
- **King can be in a union:** If a King is part of a union, it can be freed or taken over like any other piece.
- The game plays significantly faster than standard chess (10-20 minutes).
