---
title: "Engine, Engine No. 9"
bgg_id: 83157
player_count: "2-4"
play_time: "15-20 min"
designer: "Haim Shafir"
source_pdf: "engine-engine-no-9-rules.pdf"
extracted_date: "2026-03-19"
summarized_date: "2026-03-19"
verification: "unverifiable"
verification_date: "2026-06-12"
---
<!-- verification:begin -->
> ⚠️ **Unverifiable** — no source rulebook text is available to check this AI-generated summary against. Treat all details with caution.
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Engine%2C%20Engine%20No.%209&game=engine-engine-no-9)
<!-- verification:end -->


## Overview

Engine, Engine No. 9 is a children's roll-and-move strategy game where players are train conductors racing to get all three of their trains into the depot first. The game features a unique board with colored track tiles that shift positions each turn, potentially rerouting trains. Simple rules (five or fewer) make it accessible for young players while providing light strategic decisions.

## Components

- 1 game board (with train tracks and depot)
- 12 toy trains (3 per player in 4 colors)
- 3 colored track tiles
- 1 colored die
- 2 numbered dice

## Setup

1. Place the board in the center of the table.
2. Each player takes all three trains of one color and places them in their starting area.
3. Place the three colored track tiles on the board in their starting positions. Tiles can be oriented in either direction.

## Turn Structure

On your turn:

1. **Roll all three dice:** one colored die and two numbered dice.
2. **Move the track tile:** The colored die determines which colored track tile shifts this turn. Move the indicated track tile into the empty slot on the board.
3. **Move a train:** Use the value shown on the numbered dice to move one of your trains forward along the track. Choose which of the two numbered dice values to use for movement.

## Actions

### Track Tile Movement
The colored die result determines which of the three colored track tiles moves. The moving tile slides into the open slot on the board, shifting the track layout. This may reroute trains or move trains that are sitting on the tile itself.

### Train Movement
After the track tile moves, move one of your trains forward by the number shown on one of the numbered dice. Trains move along the track from start toward the depot.

### Bumping
If your train lands on a space occupied by another player's train, the bumped train moves forward the remaining spaces from your roll. This can chain if multiple trains are in the way.

### Going Off the Track
If a train goes off the track (moves past the end without reaching the depot properly), the player removes it from the board.

### Doubles
If you roll doubles on the numbered dice, you may take an immediate second turn. However, you must move the same train you moved on the first roll.

### Reaching the Depot
When a train reaches the finish spot, it is placed on the depot and can no longer be moved.

## Scoring / Victory Conditions

### Winning
The first player to get all three of their trains into the depot wins the game.

## Special Rules & Edge Cases

- Track tile shifts can move trains that are on the tile, potentially helping or hindering any player.
- You choose which numbered die to use for movement, giving some control over distance.
- Doubles grant a bonus turn but restrict you to the same train.
- The shifting track tiles mean the board configuration changes every turn, making the path unpredictable.
- Trains that are in the depot are safe and cannot be affected by track shifts or bumps.

## Player Reference

**Dice:** 1 colored (track selection) + 2 numbered (movement)

**Turn:** Roll dice > Shift colored track tile > Move one train by one die value

**Doubles:** Take a bonus turn (same train)

**Bumping:** Landed-on train moves forward remaining spaces

**Win condition:** First to get all 3 trains into the depot
