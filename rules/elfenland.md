...
source_pdf: "elfenland-rules.pdf"
...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Magic Cloud transport table: Woods costs 1 card in summary but source states 2 cards required for both plains and woods. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/elfenland-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Elfenland&game=elfenland)
<!-- verification:end -->
...
- 1 game board
- 6 Elf Boots (player pawns)
- 120 Town Pieces (20 per player color)
- 48 Transportation Counters (8 each of Dragon, Unicorn, Giant Pig, Elfcycle, Troll Wagon, Magic Cloud)
- 6 Obstacles (red border)
- 72 Travel Cards (12 each of 6 types)
- 12 Town Cards (green backs, for variant)
- 4 Round Cards
...
- The road must have a Transportation Counter on it.
- Play matching Travel Card(s) for the counter and terrain.
- If terrain shows a double symbol: play 2 matching cards.
- If an Obstacle is present: play 1 additional matching card.
- Rafts: no counter needed; play Raft cards per the chart (1 downriver, 2 upriver/lakes).
- At turn end, reduce hand to 4 cards max.

### Phase 6: Finish the Round
- Remove the top Round Card. Pass Starting Player Card left.
- Each player returns all Transportation Counters except one (kept secret or face up).
- Remove counters from the board; shuffle returned and face-down counters together.
- Used Obstacles are removed from the game.
...
### Special Moves
- **Caravan:** Instead of the matching card(s), play any 3 cards (any types) to travel a road that has a counter. With an Obstacle, play 4 cards instead. Caravans cannot use rivers/lakes.
- **Ferries:** Cross the great lakes (Mare Magnum and Mare Nebulae) by playing 2 Raft cards per lake segment.
...
- A player may use the same road multiple times in one turn, paying the full card cost each time.
- Transportation Counters placed on roads may be used by all players, not just the one who placed them.
- Only one Transportation Counter per road; only one Obstacle per road.
- Obstacles cannot be placed on rivers or lakes.
- Players may pass during the route planning phase and still place later, but if all players pass consecutively, the phase ends.
- Players retained from previous rounds may carry over one counter (secret or face up).
