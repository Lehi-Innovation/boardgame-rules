---
title: "Saboteur: The Dark Cave"
bgg_id: 365702
player_count: "2-8"
play_time: "45 min"
designer: "Frederic Moyersoen"
source_pdf: "saboteur-the-dark-cave-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Saboteur%3A%20The%20Dark%20Cave&game=saboteur-the-dark-cave)
<!-- verification:end -->
...
1. **Play a tunnel card**: Extend the cave system by placing a path card adjacent to existing cards. Paths must connect properly (no dead ends against open passages).
2. **Play an action card**: Use a special card for its effect (equipment, monster placement, identity reveal, etc.).
3. **Discard**: If unable or unwilling to play a card, discard 1-2 cards from hand.
...
1. Reveal all identities.
2. Dwarfs still in the cave lose all gold.
3. Count gold for each clan: Loyal dwarfs' gold counts for their own clan. Saboteurs' gold counts for the opposing clan.
4. The clan with the most total gold wins.
5. Individual victory goes to the player who contributed the most to the winning clan.
...
| Action | Effect |
|--------|--------|
| Play tunnel card | Extend cave path system |
| Play action card | Use special ability (equipment, monster, reveal) |
| Discard 1-2 cards | Skip playing but thin hand |
| Draw card | End of turn, draw from deck |
