...
source_pdf: "51st-state-ultimate-edition-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/51st-state-ultimate-edition-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%2051st%20State%3A%20Ultimate%20Edition&game=51st-state-ultimate-edition)
<!-- verification:end -->
...
- 200+ cards including base set and all expansions
- 4 Faction boards
- 1 Victory Point board with 4 VP markers
- 1 First Player token
- Resource pieces: Brick, Fuel, Gun, Iron (min. 30 each), Workers (min. 38)
- Tokens: Ammo (13), Red/Blue/Grey/Universal Contact tokens, Development tokens (9), Shield tokens (10), Multiplier tokens (4)
...
- Reveal top card of each Connection pile.
- Draw (players + 1) cards face-up. Draft clockwise from First Player (1 each). Discard remainder.
- Draw (players + 1) again. Draft counter-clockwise from last player (1 each). Discard remainder.
- No hand limit.
...
- **End trigger:** Any player reaches 25 VP.
- Complete the Action Phase, skip Cleanup.
- Add 1 VP per Location in each player's State.
- Highest score wins. Tie: most Goods, then most Locations, then shared victory.

## Special Rules & Edge Cases

- Card-specific rules override the rulebook.
- Production Locations produce immediately when Built.
- Ruins count as any Type for Developing.
- Ammo is wild (replaces any basic Resource).
- Universal Contact tokens replace any Contact type.
- Cannot Raze your own Locations or use your own Open Production.
- Once you Pass, you are immune to all interaction.
- Resources are unlimited; use substitutes if needed.
- All possessions are open information.
