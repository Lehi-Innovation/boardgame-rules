...
bgg_id: 3818
player_count: "4+"
designer: "Tactic Games"
source_pdf: "alias-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/alias-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Alias&game=alias)
<!-- verification:end -->
...
1. Shuffle the cards, turning over the last card in the pack to indicate when reshuffling is needed.
2. Each team chooses a colored playing piece and places it on the Start space.
3. Determine which player on each team explains first.

## Turn Structure

1. The explaining player takes 15-20 cards from the pack.
2. The other teams choose a number from 1 to 8, which determines which word on each card must be explained.
3. Flip the sand timer. The explaining player describes words while teammates guess.
4. When the timer runs out, other teams yell "stop." If a word is being explained, guessing becomes open to all teams -- the fastest correct answer earns that team 1 space forward.
5. Count cards successfully guessed. Subtract mistakes and skips. The team moves that many spaces forward.
6. Play passes to the next team. The word number is now determined by the space the team's piece occupies on the board.
...
| Rule | Effect |
|------|--------|
| Correct guess | +1 space forward |
| Mistake (said part of word) | -1 space |
| Skipped word | -1 space |
| Stealing space | Open guessing, no timer, 6 cards only |
| Timer runs out mid-word | Open guessing to all teams for that word |
