...
source_pdf: "zooloretto-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Zooloretto&game=zooloretto)
<!-- verification:end -->
...
- 5 Zoo boards (each with 3 enclosures of different sizes and 1 barn)
- 5 Expansion boards (1 additional enclosure + stall space)
- 5 Delivery trucks (each holds 3 tiles)
- 88 Animal tiles (11 types, 8 of each)
- 12 Vending stall tiles (3 types, 4 of each)
- 12 Coin tiles and 30 Coins
- 16 Offspring tiles
- 1 Wooden disc marker
- Summary cards
- Rulebook
...
### Placement Rules
- **Animals:** One type per enclosure. If no enclosure space is available, the animal goes to your barn.
- **Vending Stalls:** Place on designated stall spaces or in the barn.
- **Coins:** Added to your coin supply.
...
| Condition | Points |
|---|---|
| Full enclosure | Higher printed value |
| 1 empty space | Lower printed value |
| 2+ empty spaces with adjacent stall | 1 point per animal |
| 2+ empty spaces without stall | 0 points |

### Additional Scoring
- Each vending stall type on stall spaces: +2 points
- Each vending stall type in barn: -2 points
- Each animal type in barn: -2 points

**Winner:** Most points. Ties broken by most coins.

## Special Rules & Edge Cases

- **One Type Per Enclosure:** Each enclosure holds only one animal type. You cannot mix types.
- **Barn Overflow:** The barn has unlimited capacity but every animal type in it costs -2 points.
- **Truck Timing:** Once you take a truck, you pass for the rest of the round. Taking early gives choice but risks missing better tiles.
- **Expansion Board:** Once flipped, the expansion is permanent. It adds 1 enclosure and 1 stall space.
- **Vending Stalls on Stall Spaces:** Only count for +2 points if placed on a designated stall space (not in the barn).
- **Exchange Restrictions:** Must swap between two different locations, and the types must be different. Vending stalls cannot be exchanged.
- **Offspring Limit:** Each pair produces only 1 offspring per game.
- **2-Player Variant:** Each player uses 2 expansion boards. Remove 3 animal/offspring types. Use 3 trucks with 3 randomly blocked face-down tiles.
- **Coin Tiles:** Some tiles drawn from the stacks are coins instead of animals or stalls.
- **Round Completion:** A round ends when all delivery trucks have been claimed. Return empty trucks to the center and start a new round.
