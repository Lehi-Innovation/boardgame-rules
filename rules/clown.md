...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20CLOWN&game=clown)
<!-- verification:end -->
...
1. Shuffle all cards and place them in a face-down draw pile.
2. Each player creates a building area in front of them for their clown.
...
2. **Decide** whether to add it to your clown stack or discard it.
3. **Build upward** — cards must go in body-part order from bottom (shoes) to top (hat).
...
| Build Order | Shoes → Legs → Body → Arms → Head → Hat |
|------------|------------------------------------------|
| Size Rule | Each piece ≤ size of piece below |
| Scoring | Taller clown = more points |
| Win Condition | Tallest completed clown |
