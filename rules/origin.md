...
source_pdf: "origin-rulebook.pdf"
...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Innovation tile scoring completely fabricated: summary claims L1=1 L2=3 L3=6 L4=10 L5=15 but source says every tile=1pt except L5=5pts; summary also invents "Villages on the board: Various bonuses" scoring category with no source support; 3 of 4 game-end triggers omitted. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/origin-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Origin&game=origin)
<!-- verification:end -->
...
- 1 Main board (world map)
- 1 Rewards board
- 36 Tribe Pawns (varying in Height/Speed 1-3, Strength/Width 1-3, Color: White/Brown/Black)
- 36 Villages (9 per player color)
- 30 Innovation tiles (Levels 1-5)
- 6 Hunting tokens (values 4-8)
- 11 Strait tokens (values 3-5)
- 48 Cards: 14 Action (yellow), 14 Permanent (orange), 20 Objective (purple)
...
- Evolution placement: if adjacent to multiple occupied territories, only needs to match 2 characteristics with **one** neighbor.
- Pawns and Villages can never be removed from the board once placed.
- Swap: the displaced player does not lose possessions and does not gain Rewards.
- Strait control is checked only at time of placement, not later.
- Cards: Action (immediate effect, discarded), Permanent (requires matching Innovation level, lasts all game), Objective (scored at game end).
- May play up to 1 card of each color per turn.
