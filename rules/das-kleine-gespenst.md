...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Das%20Kleine%20Gespenst&game=das-kleine-gespenst)
<!-- verification:end -->
...
1. Set up the castle board with all doors closed.
2. Place picture tiles randomly behind the castle doors (hidden from players).
3. Prepare the clock mechanism with picture cards.
4. Distribute starting components to players.
5. Determine the starting player.

## Turn Structure

Each round:
1. **Clock Strikes:** The clock mechanism reveals a picture in the viewing window.
2. **Search Phase:** The active player uses the magnetic key to open doors and search for the matching picture behind them.
3. **Reward:** If the correct door is found, the player earns a cannonball token.
4. **Memory:** Close the doors. Players try to remember what is behind each door for future turns.
