...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Colomino&game=colomino)
<!-- verification:end -->
...
1. Place the starting tile or board in the center of the play area.
2. Shuffle and distribute tiles to each player (or create a shared draw pile, depending on variant).
3. Determine a starting player.

## Turn Structure

On your turn:
1. **Select a tile** from your hand or draw pile.
2. **Place the tile** adjacent to at least one existing tile on the board.
3. **Score points** for color matches along shared edges.
4. Draw a replacement tile if applicable.
...
| Placement Rule | Colors on shared edges must match |
|---------------|----------------------------------|
| Scoring | Points per matched edge + bonus for enclosed areas |
| Win Condition | Most points when all tiles placed |
