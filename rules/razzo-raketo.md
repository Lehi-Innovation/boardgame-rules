...
source_pdf: "razzo-raketo-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Razzo%20Raketo&game=razzo-raketo)
<!-- verification:end -->
...
1. Shuffle all spaceship tiles face-down.
2. Distribute pilot and copilot chips to each player.
3. Clear the room of obvious hiding spots or establish boundaries for the hiding area.
4. Determine the starting player (youngest goes first).

## Turn Structure

On each turn:

1. **Flip a Tile:** The active player flips one tile from the face-down pool.
2. **Docked Ship:** If the tile shows a docked ship:
...
3. **Flying Ship:** If the tile shows a flying ship:
...
| Tile Type | Action |
|---|---|
| Docked Ship | Place chips, close eyes, active player hides ship |
| Flying Ship | Search for a previously hidden ship within time limit |

**Win:** Find the most ships
