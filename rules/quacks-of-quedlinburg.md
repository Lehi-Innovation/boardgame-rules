...
source_pdf: "quacks-of-quedlinburg-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/quacks-of-quedlinburg-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20The%20Quacks%20of%20Quedlinburg&game=quacks-of-quedlinburg)
<!-- verification:end -->
...
- **Purple card:** Carried out immediately before the turn starts.
- **Blue card:** Applies for the whole turn or at turn's end.
- Cards may grant yellow or purple chips — only if that ingredient book is already in play.
...
- A **1-chip** fills 1 space, a **2-chip** fills 2 spaces, etc. (gaps in between remain empty).
...
- Chip costs are listed in their ingredient books (left = 1-chip cost, middle = 2-chip, right = 4-chip).
- Higher-value chips advance you further in your pot and interact with some book effects.
- Supply is limited — if no chips remain, you may not buy that ingredient.

**F — End of Turn:** Players may spend rubies (multiple times):
- **2 rubies:** Move droplet 1 space forward (if a chip occupies the space, move it aside).
- **2 rubies:** Refill empty flask (flip to full side).
...
- **Turn 6:** Before the turn begins, each player adds **1 white 1-chip** to their bag.
- **Turn 9 (last turn) — Potions Phase:** The start player calls "Stir!" and all active players draw simultaneously each round (reaching into bags at the same time).
- **Turn 9 — End of Turn:** After the Evaluation Phase, each player may spend **5 coins or 2 rubies** to gain **1 VP**, repeatable as many times as they like.
- **Yellow book** is revealed before turn 2; **purple book** before turn 3. Players may not buy those chips before their books are out.
- **Explosion and rubies:** Exploded players still receive rubies from Evaluation Phase C.
- **Explosion and bonus die:** Exploded players may not roll the Bonus Die.
- **Chip actions (end-of-phase only):** Green, purple, and black chip effects are resolved in Phase B, not when drawn.
- **Blue chips (Set 1):** When drawn, draw extra chips from your bag per the blue chip's value; keep 1 (or none) and return the rest. If the kept chip also has a bonus, resolve it immediately.
- **Multiple blue chips:** If a blue chip's "safety" effect (Set 2) is still active when another blue chip is placed, apply the better of the two effects — they do not stack.
