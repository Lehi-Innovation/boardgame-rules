...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Sushi%20Go%21&game=sushi-go)
<!-- verification:end -->
...
- Player(s) with the most Pudding cards: 6 points (split if tied).
- Player(s) with the fewest Pudding cards: -6 points (split if tied).
- In a 2-player game, there is no penalty for fewest Pudding.

**Winner:** Highest total score after 3 rounds. Tiebreaker: most Pudding cards.

## Special Rules & Edge Cases

- Each Wasabi card can hold only 1 Nigiri card. If you have multiple Wasabi without Nigiri, only the first available one is used when you play a Nigiri.
- You may have multiple Chopsticks in play, but can only use 1 per turn.
- When splitting tie points for Maki or Pudding, round down and ignore remainders.
- Pudding cards are the only cards retained between rounds; all others are discarded.
- If the draw pile runs out during re-dealing, reshuffle all discarded cards (except retained Pudding).
