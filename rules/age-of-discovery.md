...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Age%20of%20Discovery&game=age-of-discovery)
<!-- verification:end -->
...
1. Place the 12 expedition cards in the center of the table, each assigned a number (3-8) and a color.
2. Stack the trade contract cards and reveal the top 4 face-up.
3. Create the shipyard: arrange 5 stacks of ship cards with 2 rows of 5 ships (a purchase row and a filling row).
4. Each player receives wooden cubes, 6 coins, and places a cube on the scoring and trade contract tracks.
5. Each player receives special action cards, secret mission cards, and one flagship card.

## Turn Structure

On each turn, a player selects **two** of the following four available actions:

1. **Take Two Coins**: Draw 2 coins from the bank.
2. **Trade Contract**: Purchase 1 face-up or unrevealed trade contract card (costs 1 coin).
3. **Purchase Ships**: Buy ships from the bottom (purchase) row of the shipyard. The filling row replenishes the purchase row.
4. **Send Ships**: Deploy ships to expeditions or trade contracts.
