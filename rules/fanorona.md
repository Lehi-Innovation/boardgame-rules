...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/fanorona-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Fanorona&game=fanorona)
<!-- verification:end -->
...
- 1 Board (9x5 grid with diagonal lines at alternating intersections)
- 22 White pieces
- 22 Black pieces
...
- Cannot traverse a line already used during the current chain
- Cannot return to a point already occupied during the current chain
- Cannot capture in the same direction as the immediately preceding capture

## Scoring / Victory Conditions

- **Win:** Capture all of the opponent's pieces.
- **Draw:** If a player cannot move or capture on their turn, the game is a draw.

## Special Rules & Edge Cases

- **Compulsory capture:** If a capture is available, the player must capture (cannot make a simple move instead).
- **Approach vs. Withdrawal:** When both are possible on the same move, choose one.
- **Chain capture restrictions:** Remember the path; no retracing lines, no revisiting points, no repeating direction.

### Common Variations
- **First capture restriction:** The first time each player captures in a game, only a single capture is allowed (no chain).
- **Optional continuation:** After the first capture in a chain, further captures are optional (player may stop at any point).
