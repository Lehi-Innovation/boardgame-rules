...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Mister%20Zero&game=mister-zero)
<!-- verification:end -->
...
- 1 game board (network of paths and intersections)
- Numbered chips
- Mister Zero figure
- Goal spaces
...
### Placing Chips
- On your turn, place one numbered chip on any unoccupied path segment.
- Strategic placement controls where Mister Zero will travel.

### Mister Zero's Movement
- Mister Zero moves from the start position.
- At each intersection, he always takes the path with the lowest-numbered chip.
- Each chip he passes over is flipped (inverted).
- His journey ends when he reaches a goal space or has no valid path.
...
- If Mister Zero cannot reach a goal space, no points are awarded.
- The placement phase is the strategic core; predicting Mister Zero's path is key.
- Multiple rounds may be played with score accumulation.
