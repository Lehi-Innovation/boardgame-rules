...
source_pdf: "qin-rules.pdf"
extracted_date: "2026-03-22"
summarized_date: "2026-03-22"
rulebook_version: "eggertspiele / Pegasus Spiele (2012)"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/qin-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Qin&game=qin)
<!-- verification:end -->
...
- Tiles may only be placed on **grassland** spaces (never on province spaces, villages, or water).
- At least one province space of the tile must share an edge with an existing province space of any color.
- Tiles may be rotated (horizontal or vertical orientation).
- Corner-only adjacency does not count.
- Villages are not considered province spaces for adjacency.
...
**Restrictions:**
- You may NOT join provinces if two or more players would contribute the same number of spaces (no owner can be determined).
- **Major provinces are safe** -- they can never be absorbed. You cannot combine two major provinces. However, a single major province may absorb smaller provinces.
...
- If no pile has tiles remaining, you cannot draw and must play from remaining hand tiles.
- You may place a tile that creates only single (unconnected) province spaces, but this is rarely advantageous.
- When a province is absorbed, the connecting tile's spaces are not counted when determining the new owner.
- A village can change ownership multiple times as surrounding provinces grow.
