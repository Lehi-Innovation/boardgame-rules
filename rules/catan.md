...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/catan-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Catan&game=catan)
<!-- verification:end -->
...
- 19 terrain hexes: hills (brick), forest (lumber), mountains (ore), fields (grain), pasture (wool), 1 desert (produces nothing)
- 6 sea frame pieces (numbered 1-1, 2-2, etc. for assembly)
- 9 harbor pieces (4 special 2:1 harbors + 5 generic 3:1 harbors... actually: 5 special 2:1 harbors, one per resource + 4 generic 3:1 harbors)
- 18 circular number tokens (labeled 2-12, no 7; each number appears twice except 2 and 12 which appear once)
- 95 resource cards (brick, grain, lumber, ore, wool)
- 25 development cards: 14 knight cards (purple), 6 progress cards (green: 2 Road Building, 2 Year of Plenty, 2 Monopoly), 5 victory point cards (orange: Chapel, Great Hall, etc.)
- 4 "Building Costs" cards (one per player color)
- 2 special cards: "Longest Road" and "Largest Army"
- 16 cities (4 per color, church-shaped)
- 20 settlements (5 per color, house-shaped)
- 60 roads (15 per color, bar-shaped)
...
- **Settlement:** 1 resource card of the hex's type
- **City:** 2 resource cards of the hex's type
- A player with multiple buildings on a hex receives for each one
...
**Rules:**
- Only the active player may initiate trades — other players trade only with the active player, never with each other
- You cannot give away cards for free
- You cannot trade identical resources (e.g., 2 wool for 1 wool)
- You cannot trade development cards
- You may trade as many times as you wish on your turn
- If you choose not to trade, no one trades this turn
...
- **Road Building (×2):** Immediately place 2 free roads (following normal placement rules)
- **Year of Plenty (×2):** Take any 2 resource cards from the supply (same or different types). You may use them to build this turn.
- **Monopoly (×2):** Name 1 resource type. **All** other players must give you **all** cards of that type from their hands.
...
### Longest Road
- First player to build a continuous road of **at least 5 segments** (not counting forks) receives the "Longest Road" card (2 VP).
- If another player builds a **longer** continuous road, they take the card immediately (a 4 VP swing).
- Only count the single longest branch — forks don't add up.
- An opponent's settlement/city built on an intersection along your road **breaks** it into separate segments.
- **Tie-breaking:** If the holder's road is broken and they tie with another player, they keep the card. If the holder no longer has the longest road and 2+ players are tied for longest, the card is set aside (no one holds it) until one player has the sole longest road of 5+.

### Largest Army
- First player to play **3 knight cards** face-up receives the "Largest Army" card (2 VP).
- If another player plays **more** knight cards than the current holder, they take the card immediately.

### Resource Supply Shortage
- If the bank runs out of a resource type during production and the shortage affects **multiple players**, **no one** receives that resource this turn.
- If the shortage only affects **1 player**, they receive as many as remain; extras are lost.
- Other resource types are unaffected.

### Building Limits
- Maximum: 5 settlements, 4 cities, 15 roads per player.
- If all 5 settlements are placed, you must upgrade one to a city to free up a settlement piece.
- When a city replaces a settlement, the settlement returns to your supply.

### Player Information
- Players must truthfully answer how many resource cards they hold.
- Resource cards are hidden. Development cards are hidden until played (except VP cards which stay hidden until game end).

### The Desert
- Produces no resources. The robber starts here.
- Settlements/cities adjacent to the desert receive fewer resources overall.
...
- Settlement: 1 VP
- City: 2 VP
- Longest Road (5+ segments, sole longest): 2 VP
- Largest Army (3+ knights, most played): 2 VP
- Victory Point card: 1 VP each

### Key Numbers
- Win condition: 10 VP on your turn
- Starting VP: 2 (from 2 settlements)
- Robber discard threshold: 8+ cards → discard half (rounded down)
- Trade rates: 4:1 (default), 3:1 (generic harbor), 2:1 (special harbor)
- Development cards in deck: 25 (14 knights, 6 progress, 5 VP)
- Max pieces per player: 5 settlements, 4 cities, 15 roads
