...
play_time: "30-45 min"
designer: "Unknown"
source_pdf: "amuseamaze-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20AmuseAmaze&game=amuseamaze)
<!-- verification:end -->
...
1. Assemble the 18-part game board to create the letter maze. The configuration can vary each game.
2. Place colored target markers on designated spaces.
3. Each player selects a pawn and places it at the start of the maze.
4. Shuffle and prepare target cards.
...
1. **Forms a word** using letters printed on the board and/or letter tiles.
2. **Moves their pawn** from the first letter of the word, through each adjacent letter in sequence, to the last letter. The path must follow connected/adjacent spaces through the maze.
3. **Collects target cards** if they land on or pass through colored target spaces during their word path.
...
The first player to:
1. Visit the required number of colored target spaces (collecting their target cards), AND
2. Reach the end of the maze (bottom row in 3-6 player games, or a space adjacent to the start in 2-player games)
...
| Phase | Action |
|-------|--------|
| Spell | Form a word using adjacent board letters |
| Move | Move pawn through each letter space in the word |
| Collect | Pick up target cards from colored spaces visited |
| Win | Collect required targets + reach the maze end |
