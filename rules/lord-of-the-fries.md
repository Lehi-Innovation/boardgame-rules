---
title: "Lord of the Fries"
bgg_id: 1536
player_count: "3-8"
play_time: "30 min"
designer: "James Ernest"
source_pdf: "lord-of-the-fries-rules.pdf"
extracted_date: "2026-03-21"
summarized_date: "2026-03-21"
verification: "verified"
verification_date: "2026-06-14"
---
<!-- verification:begin -->
> ✅ **Verified** — this summary was fact-checked against the rulebook text and no significant issues were found.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/lord-of-the-fries-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Lord%20of%20the%20Fries&game=lord-of-the-fries)
<!-- verification:end -->


## Overview

Lord of the Fries is a fast-food themed card game set at Friedey's, the restaurant of the damned staffed by zombies. Players try to fill food orders from a menu using ingredient cards. Over multiple shifts (rounds), the player who fills the best orders and gets rid of cards scores the most points. After four shifts, the highest total score wins.

## Components

- 98 Ingredient cards (each with a point value)
- 16 Menu cards (8 different restaurants, double-sided)
- 2 Six-sided dice (1 black, 1 green)
- 28 Tokens
- 1 Sand timer (30 seconds)
- 2 Blank replacement cards

## Setup

1. Choose which restaurant menu to play (8 options).
2. Build the deck using the recipe on the back of the rules: 3-5 players use a smaller deck; 6-8 players use a larger deck.
3. Spread menus so all players can see one.
4. Choose a dealer for the first shift.
5. Recommended: play 4 shifts (rounds).

## Turn Structure

### Starting Each Shift
Deal out all cards evenly (slight imbalance is fine).

### The First Order
The dealer determines the first order by:
- **Calling:** Choose any order from the menu. Players who pass hand a card to their left.
- **Rolling:** Roll both dice (green = menu section, black = item number). Players who pass hand a card to the roller.

Do not use the timer on the first order.

### Filling Orders
Starting with the player to the dealer's left, clockwise:
- **Fill the order:** Play the required ingredient cards from your hand face-up. Stack them as your scoring pile. You become the new leader.
- **Pass:** Decline (even if you could fill it). Hand 1 card face-down: to the left (if called) or to the leader (if rolled).

### Short Orders
If an order goes completely around the table unfilled:
- **2nd pass:** You may omit 1 item from the order.
- **Each additional pass:** Omit 1 more item.
- If reduced to 0 items, the customer walks out. Player to leader's left takes the lead.

### More Orders
When you fill an order, you become leader and call or roll the next order. Use the sand timer on all orders after the first. If the timer runs out, other players may call "Now!" — the leader must immediately call or roll.

### Ending the Shift
The shift ends immediately when any player gets rid of their last card (by filling an order or passing).

## Actions

| Action | When | Effect |
|--------|------|--------|
| Fill Order | Your turn in the circle | Play required cards; gain scoring pile |
| Pass | Your turn in the circle | Give 1 card away; order continues |
| Call Order | As leader | Choose any menu item |
| Roll Order | As leader | Dice determine the order |

## Scoring / Victory Conditions

**End of Shift Scoring:**
- Add point values of all cards in your scoring pile.
- Add any tokens earned.
- Subtract point values of all cards remaining in your hand.
- Negative scores are possible.

**Dealer for next shift:** Player with the lowest score.

**Winning:** After 4 shifts, the player with the highest total score wins.

## Special Rules & Edge Cases

- Each restaurant menu adds a special rule that modifies gameplay.
- You can voluntarily pass even if you have the cards to fill an order.
- The shift ends immediately when any player plays or passes their last card — not at the end of the round.
- Cards passed to you go into your hand and can be used.
- You may change restaurants between shifts (winner of last shift picks).

## Player Reference

**Turn Summary:**
1. Leader calls/rolls an order
2. Clockwise from leader's left: fill or pass
3. Short orders reduce requirements each lap
4. Filling = become leader, take scoring pile
5. Shift ends when someone runs out of cards

**Scoring:** Cards in scoring pile (+) minus cards in hand (-) = shift score
