...
player_count: "5-8"
play_time: "45 min"
designer: "Mark Goadrich, Brett Myers"
source_pdf: "nanuk-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/nanuk-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Nanuk&game=nanuk)
<!-- verification:end -->
...
- 100 Cards (24 each of Deer, Seal, Fish, Bird; 4 Inuksuk; 12 cards with Nanuk bear symbols; some cards are "doubles")
- 1 Day Token
- 1 Gameboard
- 10 Number Tokens
- 10 Hunt/Doom Tokens
...
- **Raising days** (max 12): more days = more cards drawn = higher chance of success but also more Nanuk risk
- **Raising the number of animals:** you may also change the animal type when raising the number
- **Raising both days and animals**
- You may **not** change the animal type without also raising the number.
...
- Cards are drawn one per day of the bid.
- Nanuk symbols on drawn cards trigger failure unless cancelled by an Inuksuk (each Inuksuk cancels exactly one Nanuk).
- If the target is reached early, drawing continues until all days are complete (risking additional Nanuks).
...
- **Set of 4 different animals** (one each of Deer, Seal, Fish, Bird): 3 points
- **Pair of matching cards** (same animal or Inuksuk): 1 point
- **Inuksuks** are wild for pairs and sets
- **Double cards** count as 2 separate cards for scoring
- **Most Nanuk cards:** 2 bonus points (if tied, each tied player gets 1 point)
- Each card may only be used once for scoring
...
- **Wind Token:** Flipped each round to determine direction of play (clockwise or counterclockwise).
- **Winter is Near:** If face-up cards at deck bottom are reached during a hunt, stop drawing. Players return hands to dealer, who shuffles them with discards. Remaining days are drawn from the new deck. This is the final hunt.
- **New Cards:** After each hunt, all players are dealt cards to bring their hand back to 3. If a face-up card appears during dealing, the game ends immediately.
- **Next Round First Player:** The successful leader (Hunt Leader if hunt succeeded, Doom Leader if it failed) becomes first player for the next round.
- **Optional Rule:** Instead of simultaneous Hunt/Doom decisions, players declare sequentially in wind direction after the doom declaration.
- **Longer Games:** Players may agree to play multiple "seasons" and total all scores.
