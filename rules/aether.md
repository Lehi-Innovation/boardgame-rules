...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Aether&game=aether)
<!-- verification:end -->
...
1. Place the hexagonal board between the two players.
2. Distribute element tiles and elemental figures to each player.
...
1. **Place an element tile** on any empty hexagonal space on the board.
2. **Optionally place an elemental** on top of the tile just placed.
...
|-------|--------|----------|
| 1 | Place element tile on empty hex | Mandatory |
| 2 | Place elemental on just-placed tile | Optional |
