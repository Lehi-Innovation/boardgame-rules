...
source_pdf: "monster-trap-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/monster-trap-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Monster%20Trap&game=monster-trap)
<!-- verification:end -->
...
- 1 game board (mansion corridors inside the box)
- 4 sliders (2 vertical, 2 horizontal)
- 1 sacked monster piece
- 14 monster tokens (on the board)
- Monster cards (matching the tokens)
- Eye tokens (12 total, 3 per player)
- 1 sand timer (hourglass)
- 6 location cards (advanced variant)
...
- **2-player game**: Each player plays solo (controls all 4 sliders). Takes 3 eye tokens. Gets double time (hourglass flipped twice per turn). Game plays over 6 rounds.
- **Advanced variant**: Draw a location card with each monster card. Push the monster to that location first (bathroom, cat, refrigerator) before pushing it into the trap.
- If the last monster is not trapped when time expires, the card is placed face up next to the deck (not scored).
- When the monster card deck runs out, reshuffle discarded face-up cards.
