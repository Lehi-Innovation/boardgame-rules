...
1. Read the entire rulebook before the first game (some rules differ from standard Ticket to Ride).
2. Set up the game board and initial routes as described in the first session's instructions.
3. Each player chooses a character card with unique abilities.
4. Deal train car cards and destination tickets per session rules.
5. Apply any stickers or modifications from previous sessions.
6. Open any legacy deck cards as instructed.
...
| Action | Details |
|---|---|
| Draw Cards | Take 2 (or 1 face-up locomotive) |
| Claim Route | Play matching cards = route length |
| Draw Tickets | Draw and keep at least 1 |
| Legacy Event | Triggered by game conditions |
