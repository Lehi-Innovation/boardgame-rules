...
designer: "Michael E. Matheny"
source_pdf: "gladiator-rules.pdf"
...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Invented point-allocation character creation (source uses dice rolls on charts); turn structure wrong (simultaneous written orders across 8 phases, not alternating Movement/Combat/Record). Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/gladiator-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Gladiator&game=gladiator)
<!-- verification:end -->
...
- 1 hex grid map (16" x 11", representing the arena)
- 12 gladiator counters with plastic stands
- Gladiator record sheets (for tracking stats and damage)
- Dice
- Rules booklet
...
- Declare attack type (thrust, swing, etc.)
- Resolve attack: Compare attack value (based on weapon skill + training + dice roll) against defense value (based on agility + shield + armor + dice roll).
- If the attack hits, determine damage location (body part) and severity.
...
### Attack Types
- **Thrust:** Sword thrust at specific body area. Moderate damage, moderate difficulty.
- **Swing:** Wide sword swing. Higher damage potential, easier to defend.
- **Net Throw (Advanced):** Retiarius throws net to entangle opponent. Entangled gladiators suffer movement and combat penalties.
- **Trident Attack (Advanced):** Long-reach attack unique to retiarii.

### Defense
- **Active defense:** Using shield to block (requires facing toward attacker)
- **Dodge:** Spend movement points to evade
- **Armor:** Passive damage reduction based on armor type and hit location

### Special Maneuvers
- **Shield bash:** Push opponent back
- **Disarm:** Attempt to knock weapon from opponent's hand
- **Feint:** Mislead opponent about attack direction
...
| Type | Weapons | Armor | Style |
|------|---------|-------|-------|
| Secutor | Gladius (sword), Scutum (large shield) | Heavy | Close combat, defensive |
| Retiarius | Trident, Net | Light/none | Ranged, mobility |
...
|------|-------|
| Players | 2 |
| Map | 16" x 11" hex grid |
| Gladiator counters | 12 |
| Basic gladiator type | Secutor (sword + shield) |
| Advanced gladiator type | Retiarius (net + trident) |
