...
source_pdf: "rock-island-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rock%20Island&game=rock-island)
<!-- verification:end -->
...
- 90 hexagonal tiles with railroad tracks on one side (blank on the back)
- 10 river tiles (4 of which include bridges/track crossings)
- 1 Start tile (Chicago)
- 1 Finish tile (Kansas City)
- 4 train pieces (one per player)
- 4 prospector pieces (one per player)
...
- Tiles must be placed track-side up and must connect logically to adjacent tiles already in play.
- River tiles without bridges are impassable barriers; routing around them is often necessary.
- Players share the same growing network; track built by one player may benefit or hinder others.
- If no valid tile placement is possible, the player's turn may be limited.
- The game plays well with different numbers of players, with more players creating more complex and competitive track networks.
