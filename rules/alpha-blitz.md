---
title: "Alpha Blitz"
bgg_id: 1443
player_count: "2-6"
play_time: "30 min"
designer: "Mike Selinker"
source_pdf: "alpha-blitz-rules.pdf"
extracted_date: "2026-03-18"
summarized_date: "2026-03-18"
verification: "unverifiable"
verification_date: "2026-06-12"
---
<!-- verification:begin -->
> ⚠️ **Unverifiable** — no source rulebook text is available to check this AI-generated summary against. Treat all details with caution.
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Alpha%20Blitz&game=alpha-blitz)
<!-- verification:end -->


## Overview

Alpha Blitz is a word card game published by Wizards of the Coast. Players form words using letter cards on the table as a shared "letter bank" — each visible letter can be reused as many times as needed within a single word (e.g., LENS can become SENSELESSNESS). The game includes two modes: "Alpha" for 2 players (methodical, turn-based) and "Blitz" for 3-6 players (simultaneous, speed-based). The deck contains 98 letter cards and 10 Blitz (lightning bolt) cards that destroy letters and push the game toward its end.

## Components

- 98 letter cards (including 4 red-bordered letters: J, Qu, X, Z)
- 10 Blitz cards (lightning bolt symbols)

## Setup

Shuffle all cards together. Deal a starting tableau of letter cards face-up on the table (the "letter bank"). Each player draws a hand of cards. The specific setup varies by game mode.

## Turn Structure

### Alpha Mode (2 Players)

Players alternate turns. On your turn:

1. **Replace or Destroy:** Either swap one letter card from your hand with one on the table, OR play a Blitz card to destroy (remove) a letter from the table.
2. **Form a Word:** Make the longest word you can using the letters currently visible on the table. Each table letter can be used multiple times in your word.
3. **Score:** Earn 1 point for forming a valid word, plus 1 bonus point for each letter in the word beyond the number of letter cards on the table. Red-bordered letters (J, Qu, X, Z) each score an additional point when used.

### Blitz Mode (3-6 Players)

Played in simultaneous rounds:

1. Reveal the current letter bank.
2. All players simultaneously race to call out a valid word using the visible letters.
3. The first player to call a valid word scores 1 point (plus bonuses for red-bordered letters) and is then out for the rest of the round.
4. Remaining players continue, but each subsequent word must be **at least one letter longer** than the previous word called.
5. The second player to call a valid word scores 2 points plus bonuses, the third scores 3 points, and so on.
6. The round ends when no one can form a longer word or all players have scored.

## Actions

### Form a Word

Announce a word using the letters visible on the table. Every letter on the table is available to every player and can be repeated. Words must be standard dictionary words (no proper nouns, abbreviations, or foreign words).

### Replace a Letter (Alpha Mode)

Swap a card from your hand with a face-up letter on the table, changing the available letter bank.

### Play a Blitz Card

Destroy one letter card on the table, permanently removing it from the game. This shrinks the letter bank and pushes the game closer to ending.

## Scoring / Victory Conditions

The game ends when the draw pile is exhausted or when a set number of Blitz cards have been played (removing enough letters to end the game).

**Alpha Mode:** The player with the most points wins.

**Blitz Mode:** The player with the most points across all rounds wins.

### Scoring Summary

| Element | Points |
|---------|--------|
| Valid word formed | 1 point |
| Each letter beyond table count (Alpha) | +1 point each |
| Each red-bordered letter used (J, Qu, X, Z) | +1 point each |
| Position bonus (Blitz: 2nd caller = 2 pts, 3rd = 3, etc.) | Varies |

## Special Rules & Edge Cases

- Letters on the table form a shared bank — you are not limited to using each card once per word.
- Red-bordered letters (J, Qu, X, Z) are harder to use but provide bonus points.
- Blitz cards cannot be saved indefinitely; they must be played when drawn (or per mode-specific rules).
- In Blitz mode, calling an invalid word may result in a penalty (losing your turn for that round).
- "Qu" counts as a single letter card.

## Player Reference

**Alpha (2p) Turn:** Replace 1 letter or play Blitz → Form longest word → Score

**Blitz (3-6p) Round:** All players race to call words → Each word must be longer than the last → Higher position = more points

**Key Mechanic:** Letters are a shared, reusable bank — creativity with the available letters is rewarded.
