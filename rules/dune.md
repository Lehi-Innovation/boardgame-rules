...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: 3-player solo win still needs 3 strongholds (summary wrongly says 4); Spacing Guild/Fremen special victories are standard not Advanced-Game-only. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/dune-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Dune&game=dune)
<!-- verification:end -->
...
- Game board (map of Dune with sand, rock, stronghold, and Polar Sink territories; 18 sectors)
- 6 faction sets, each containing: Player Shield, Player Sheet, Player Marker, 5 Leader Discs, 20 force tokens
- 2 Battle Wheels (assembled with center pin)
- Spice Deck (21 cards)
- 156 Spice Tokens (denominations of 1, 2, 5, and 10)
- Treachery Deck (33 cards)
- Traitor Deck (30 cards)
- Alliance Deck (6 cards)
- Storm Deck (6 cards), Prediction Deck (16 cards)
- Storm Marker, Game Turn Counter, Destroyed Shield Wall Token
- 6 Quick Reference Cards
- Advanced components: 5 Special Emperor forces, Bene Gesserit advisor forces, Kwisatz Haderach Card and Counter, Sandworm Token, 3 Special Fremen forces
...
### Treachery Cards
- **Weapons:** Kill opponent's leader if undefended
- **Defenses:** Block specific weapon types (Projectile/Poison)
- **Worthless Cards:** No combat effect
- **Special Cards:** Karama (advanced game), Cheap Hero, etc.
...
- **Storm:** Kills all forces in sand sectors it passes through (except Imperial Basin). The Polar Sink is never in storm. Forces cannot battle across storm sectors.
- **First Player:** Determined by whose Player Marker the storm next approaches.
- **Treachery Card limit:** 4 cards maximum. Must declare count during Bidding.
- **Faction advantages** override standard rules. Each faction has unique abilities detailed on Player Sheets.
- **Advanced Game** adds: double spice blow, stronghold income (Arrakeen/Carthag: 2 spice; Tuek's Sietch: 1 spice per collection), spice-supported combat (1 spice per force for full strength; unsupported forces at half), enhanced Karama Cards, and unique faction advanced abilities.
- **Deals and Bribes:** Non-allied players may make verbal deals that must be honored. Cannot transfer cards, leaders, forces, or faction advantages.
