...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Babylon%205%3A%20A%20Call%20to%20Arms&game=babylon-5-a-call-to-arms)
<!-- verification:end -->
...
1. Choose a scenario or agree on a point value for fleet building.
2. Each player builds a fleet using Fleet Allocation Points (FAP), selecting ships from their chosen race's fleet list. Each ship has a priority level (Patrol, Skirmish, Raid, Battle, War, Armageddon) determining its cost and power.
3. Set up the playing area (typically a flat surface, 4'x4' or larger).
4. Deploy ships according to scenario instructions or standard deployment zones.
5. Place any terrain features (asteroids, space stations, etc.) as specified.
