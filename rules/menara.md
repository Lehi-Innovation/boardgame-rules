...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/menara-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Menara&game=menara)
<!-- verification:end -->
...
- 76 columns: 30 yellow, 20 red, 8 white, 6 blue, 12 black
- 1 cloth bag
- 18 temple floors (double-sided, each with 3-6 colored column bases; one light side, one dark side)
- 35 construction plan cards: 9 blue glyphs (easy), 12 yellow glyphs (medium), 14 red glyphs (hard)
- 5 level cards (backs and fronts without a glyph)
- 1 camp (cardboard holder for 6 columns)
...
6. Lay out level cards in a row to set the target number of levels:
   - Easy: 3 level cards
...
- Each column must be placed on a **color-matching base** and should not protrude beyond the base outline.
- Columns can be placed simultaneously.
- Columns may be placed on empty bases that have already been overbuilt by a temple floor above (shove carefully between floors).
...
- The temple collapses (any floors fall or columns cannot be righted in place)
- A player cannot replenish columns to full count at end of turn
- The last construction plan card is used
- The last temple floor is placed
...
- Temple floors placed as base extensions do NOT count as levels; only floors on columns count.
- A single level can consist of multiple temple floors if they sit at the same height on columns.
- If columns fall during a move/place attempt and cannot be put back exactly where they were, the game ends (potential loss).
- The game is cooperative: players should discuss strategy and plan turns together openly.
- When multiple temple floors need to be placed in a single turn (filling multiple floors), each triggers immediately before continuing the card.
