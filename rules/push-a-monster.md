...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/push-a-monster-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Push%20a%20Monster&game=push-a-monster)
<!-- verification:end -->
...
- After placing a monster on the large pusher, you may **not** touch it with your hands -- only the smaller pusher.
- If the monster falls off the pusher before reaching the arena, pick it up and try again.
- You may rotate the arena or walk around the table for better access, but do not touch any monsters. If a monster falls from this, remove it from the game (no tokens awarded).
- A monster must be pushed completely onto the arena -- no part may protrude over the edge.
- Monsters may not lie on top of one another.
- Your turn ends only when the monster lies completely on the arena.
