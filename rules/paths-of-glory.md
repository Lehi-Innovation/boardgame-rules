---
title: "Paths of Glory"
bgg_id: 91
player_count: "2"
play_time: "360-480 minutes"
designer: "Ted Raicer"
source_pdf: "paths-of-glory-rules.pdf"
extracted_date: "2026-03-22"
summarized_date: "2026-03-22"
verification: "inaccurate"
verification_date: "2026-06-14"
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Automatic Victory wrongly described as "controlling a sufficient number of VP spaces" — source says VP marker reaches 20 (CP) or 0 (Allies); end-game victory omits Draw ranges per scenario; Armistice game-end trigger (Combined War Status >= 40) missing entirely. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/paths-of-glory-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Paths%20of%20Glory&game=paths-of-glory)
<!-- verification:end -->


## Overview

Paths of Glory is a card-driven grand strategy wargame covering the entirety of World War I (1914-1918). One player commands the Central Powers (Germany, Austria-Hungary, Ottoman Empire, Bulgaria) and the other the Allied Powers (France, Britain, Russia, Italy, and others). Players use strategy cards for operations, events, replacements, or strategic redeployment, managing both the Western and Eastern Fronts as well as the Near East.

## Components

- 1 game map (Europe and the Near East)
- Strategy cards (Allied deck and Central Powers deck)
- Unit counters (corps-level infantry, cavalry, and special units)
- Various markers (VP, War Status, turn, supply, etc.)
- Combat Results Tables and Player Aid Cards
- Dice

## Setup

Follow the Set Up Information for the chosen scenario:
- **Introductory Scenario:** Begins August 1914, shorter learning game
- **Limited War Scenario:** Standard start, covers the mobilization period
- **Campaign Scenario:** Full game from 1914 to 1918
- Place units in their designated starting spaces per the setup chart
- Set VP markers, War Status markers, and other tracks to starting positions
- Deal initial strategy cards to each player

## Turn Structure

Each game turn represents approximately 3 months. The sequence of play:

### 1. Mandated Offensive Phase
Check if either side must conduct a mandated offensive (required attacks during certain turns/conditions).

### 2. Action Phase
Players alternate playing strategy cards for a number of action rounds (typically 6 per turn). On each action round, the active player plays one card for one of four uses:
- **Operations (OPS):** Move and/or attack with units (OPS value determines number of activation points)
- **Strategic Redeployment (SR):** Move units via rail networks
- **Replacement Points (RP):** Rebuild destroyed or depleted units
- **Event:** Trigger the card's historical event

### 3. Attrition Phase
Check supply for all units; unsupplied units suffer attrition.

### 4. Siege Phase
Resolve sieges of fortified positions.

### 5. War Status Phase
Adjust war status levels based on events and territorial control.

### 6. Replacement/Card Draw Phase
Receive replacement points and draw new strategy cards for next turn.

## Actions

### Operations
Spend OPS points to activate spaces. Units in activated spaces may move and/or attack. Each OPS point activates one space (group of units).

### Movement
- Infantry moves 1 space; cavalry moves 2.
- Movement is restricted by terrain, enemy zones of control, and entrenchment.
- **Trenches:** Can be built during operations; provide significant defensive advantages. Trenches represent the static warfare characteristic of WWI.

### Combat
1. Attacker declares all attacking units and the target space.
2. Both sides may play combat cards for modifiers.
3. Calculate total attack and defense strengths.
4. Roll dice and consult the Combat Results Table.
5. Apply losses, retreats, and advances.

**Flank Attacks:** If the attacker has units in multiple spaces adjacent to the defender, combat modifiers improve.

**Loss types:** Step losses (flip units to reduced side or eliminate), retreats (move defending units).

### Strategic Redeployment
Move units along connected rail lines. Number of units moved equals the card's SR value.

### Replacements
Spend RP to rebuild eliminated or reduced units. Corps must be rebuilt in supply and at appropriate locations.

## Scoring / Victory Conditions

**Automatic Victory:** Either side wins immediately by controlling a sufficient number of Victory Point spaces (total VP threshold depends on scenario).

**End-of-game Victory:** If no automatic victory, the side with more VP at game end wins. The Central Powers must achieve their objectives before American forces tip the balance.

**Key VP spaces include:** Paris, Berlin, Vienna, Constantinople, Warsaw, Brussels, Belgrade, and others.

**War Status:** Tracks the political will of each nation. Nations can be forced out of the war through events or territorial losses (e.g., Russian Capitulation, US Entry).

## Special Rules & Edge Cases

- **Limited War vs. Total War:** The game begins in a Limited War state with restrictions on movement and combat. As the war escalates, Total War rules unlock more aggressive options.
- **US Entry:** American forces enter based on events and War Status; their arrival is a major Allied advantage.
- **Russian Capitulation:** If triggered, Russia exits the war and Central Powers gain advantages.
- **Near East:** Special movement and supply restrictions apply to the Ottoman theater.
- **Forts:** Major fortifications (Verdun, Liege, etc.) require sieges to capture.
- **Combined Arms:** Different unit types interact with combat modifiers.
- **Mandated Offensives:** During certain periods, players must attack with specified forces, reflecting historical political pressure.
- **Peace Terms:** End-war negotiations can occur under certain conditions.

## Player Reference

| Card Use | Effect |
|----------|--------|
| OPS | Activate spaces for movement/combat |
| SR | Rail movement of units |
| RP | Rebuild units |
| Event | Trigger historical event |

| Phase | Action |
|-------|--------|
| 1. Mandated Offensive | Required attacks |
| 2. Action Rounds | Alternate card plays (typically 6 rounds) |
| 3. Attrition | Supply check |
| 4. Siege | Resolve fort sieges |
| 5. War Status | Adjust political tracks |
| 6. Draw | Get new cards and replacements |
