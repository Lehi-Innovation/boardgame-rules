...
source_pdf: "hansa-teutonica-rules.pdf"
extracted_date: "2026-03-02"
summarized_date: "2026-03-02"
rulebook_version: "Big Box Edition (2020, Pegasus Spiele)"
...
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Bonus Marker scoring table (1/3/6/10/15/21 PP) has no source support; the source table lost its entire point-value column during extraction. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/hansa-teutonica-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Hansa%20Teutonica&game=hansa-teutonica)
<!-- verification:end -->
...
- 3 game boards (double-sided): The Hanseatic League (front 4-5 players, back 3 players), The Eastern Hanseatic League (3-5 players), Britannia (front 4-5 players, back 3 players)
- 135 traders (wooden cubes, 27 per player color, 5 colors)
- 20 merchants (wooden discs, 4 per player color, 5 colors)
- 5 writing desks / player boards (1 per player color)
- 15 bonus markers + 3 gold start bonus markers + 2 blank bonus markers
- 9 mission cards (for experienced play variant)
- 6 Emperor's Favour tiles (for experienced play variant)
- 1 black wooden cube (Completed Cities track marker)
- 1 scorepad

### Bonus Marker Types

- **+3 Actions** (3x): Grants 3 additional actions on your turn
- **+4 Actions** (4x): Grants 4 additional actions on your turn
- **Move 3 Tradesmen**: Remove up to 3 opponents' tradesmen from connection points and re-place them on free connection points
- **Exchange Trading Posts**: Exchange positions of 2 tradesmen in adjacent trading post spaces in a city (you must own one)
- **Develop 1 Ability**: Develop any 1 ability on your writing desk
- **Additional Trading Post**: Place a tradesman to the left of existing trading posts to create an extra trading post in a city
...
- There cannot be any tradesmen on the trade route.
- At least 1 trading post space must be empty in at least 1 of the 2 cities at either end of the trade route.

#### Step 3: Choose 1 Option (optional -- you may skip this step entirely)

**3a) Establish 1 Trading Post:**
Remove all your tradesmen from the trade route. Return all but 1 to the general stock. Use the remaining tradesman to fill the leftmost vacant trading post space in one of the two cities at either end of the route.

Rules:
- You cannot skip over a vacant trading post space; you must fill the lowest vacant space.
- Trading post space values increase from white (lowest) to orange to pink to black (highest). You must have developed your Privilege ability to the corresponding level to place in non-white spaces.
- Square trading post spaces require a trader (cube). Round trading post spaces require a merchant (disc).
- The tradesman placed must be one from the trade route, not from the general stock or personal supply.
- On The Hanseatic League board, certain cities grant 1 immediate prestige point when you establish the first trading post there (marked with a gold "1" icon): Groningen, Lübeck, Coellen, and Halle on the 4-5 player side; Groningen, Stade, Lübeck, Coellen, Warburg, and Halle on the 3-player side.
- If you fill the last vacant space in a city, advance the Completed Cities marker 1 space. If the 10th city is fully occupied, the game ends immediately after this action.
...
**3b) Develop 1 Ability:**
If one of the two cities at either end of the trade route has a pictogram of one of the 5 abilities, you may develop that ability instead of establishing a trading post. Return ALL tradesmen from the trade route to the general stock (none are kept). Remove the leftmost trader (for City Keys, Actions, Privilege, Bank) or merchant (for Book of Knowledge) from the corresponding ability on your writing desk and place it in your personal supply. From now on, the improved value is available. If you just revealed a higher Actions value, the additional action is immediately available this turn.

**3c) Score Special Prestige Points (Coellen):**
If you create a trade route between Coellen and Warburg, you may place a merchant from the trade route onto 1 of the 4 special prestige point spaces (valued 7, 8, 9, or 11). The space must be empty. You may skip over empty spaces (unlike trading posts). You must have the appropriate Privilege level matching the space's color. Return remaining tradesmen to the general stock. The merchant stays until end-game scoring.
...
- **Personal supply:** Tradesmen available for immediate use (placing, displacing, etc.).
- **General stock:** Tradesmen not yet hired. You must use Action A (Income) to transfer them to your personal supply before they can be used for Action B.
...
- You may use a bonus marker at any time during your turn.
- You may NOT use a bonus marker during the same action in which you took it.
- Using a bonus marker does not cost an action.
- +3/+4 Actions bonus markers are used before placing new bonus markers at the end of your turn.

### Exchange Trading Posts Bonus Marker

- Exchange 2 tradesmen in adjacent established trading post spaces in a city.
- You must own one of the two trading posts.
- Shape and color requirements are ignored: a trader and merchant can swap, and you can occupy a space for which you have not developed the privilege.
- Cannot be applied to additional trading posts (those placed to the left of normal trading post spaces).

### Additional Trading Post Bonus Marker

- Used during Step 3a of Action E.
- Place 1 tradesman from the trade route to the LEFT of the existing trading post spaces.
- The leftmost (lowest) space must already have a trading post established by any player.
- The additional trading post's value is lower than any trading post to its right.
- If multiple additional trading posts are established, each new one goes further to the left.
- Counts like any other trading post for all evaluations.

### Move 3 Tradesmen Bonus Marker

- Remove up to 3 opponents' tradesmen from connection points and re-place them on free connection points.
- The 3 tradesmen may belong to 1, 2, or 3 different players.
- You may use this to rearrange positions (e.g., move a merchant out, then place a trader into the vacated spot).
- You cannot displace another tradesman during the movement (each must go to a free connection point).
...
- Only the existence of a continuous chain of your trading posts from Stendal to Arnheim matters.
- Route length, trading post positions within cities, and city control are irrelevant.
- Points: 7 (first), 4 (second), 2 (third).
...
- Starting actions per turn: 2
- Maximum actions per turn: 5 (plus bonus markers)
- Game-end prestige threshold: 20 points
- Game-end completed cities: 10 (8 on Britannia)
- East-West Connection points: 7 / 4 / 2 (1st / 2nd / 3rd)
- Fully developed ability bonus: 4 PP (except City Keys: 0 PP)
- Displacement cost: 1 extra tradesman for traders, 2 for merchants
- Traders per player: 27 cubes
- Merchants per player: 4 discs
