...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%2030%20Seconds&game=30-seconds)
<!-- verification:end -->
...
- 240 Cards (2,400 names total — 5 names per side, 2 sides per card)
- 1 Game Board
- 1 Thirty-Second Timer (sand timer)
- 1 Die (faces: 0, 0, 1, 1, 2, 2)
- 4 Playing Tokens
...
- Sing or hum
- Gesture
- Use the words "and" and "the"
- Describe names in any order
- Give contextual clues about the name

### Describing Rules (Prohibited)
- Say the actual name or any part of it
- Use rhyming clues ("sounds like...")
- Mention individual letters or spell anything
- Translate names into another language
- Point at objects in the room

### Guessing Rules
- Teammates may use alphabet references to narrow down answers
- There is no penalty for incorrect guesses during the 30-second window
...
- **Card Sides:** Cards have blue and yellow sides. Use the side matching the color of the square your token currently occupies.
- **Three Players:** When playing with only 3 players, 1 person acts as the describer for both teams (alternating which team guesses).
- **Rule Violations:** If a describer breaks a rule (says a name, rhymes, etc.), the team moves back 1 square as a penalty.
- **Die Values:** The die has only 0, 1, and 2 (each appears twice), so the handicap is always small.
- **No actions = no move:** If a team guesses 0 names, they don't move regardless of the die roll.
