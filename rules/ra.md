...
>
> 📄 [Full rulebook text](https://jonnyallred.github.io/boardgame-rules/extracted/ra-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Ra&game=ra)
<!-- verification:end -->
...
**Auction rules:**
- If you invoke Ra voluntarily and everyone else passes, you **must** bid.
- If Ra was invoked because the auction track is full and you don't play a God, you may pass even if all others pass (tiles are returned to box).
- If a Ra tile triggers the auction, all players including the Ra player may pass (tiles stay).

### Disasters
When won at auction, disasters must be fulfilled by discarding 2 tiles of the matching category:
- Funeral: discard 2 Pharaohs
- Drought: discard Floods first, then Niles (total 2)
- Unrest: discard 2 Civilizations (your choice)
- Earthquake: discard 2 Monuments (your choice)
...
- A player with no face-up suns takes no turns for the rest of the epoch.
- Minimum score is 0 per game (cannot go into debt across epochs, but can lose all points).
- Ra track size: 6 spaces (2p), 8 (3p), 9 (4p), 10 (5p).
- Tiles removed from game may not be examined.
