---
title: "Destination London"
bgg_id: 15182
player_count: "2-4"
play_time: "30-60 min"
designer: "Rachel Lowe"
source_pdf: ""
extracted_date: "2026-03-19"
summarized_date: "2026-03-19"
verification: "unverifiable"
verification_date: "2026-06-12"
---
<!-- verification:begin -->
> ⚠️ **Unverifiable** — no source rulebook text is available to check this AI-generated summary against. Treat all details with caution.
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Destination%20London&game=destination)
<!-- verification:end -->


## Overview

Destination London (part of the Destination series) is a roll-and-move family board game first published in 2004. Players compete as taxi drivers navigating London, visiting tourist attractions to earn money during a single shift. The game features risk elements like running out of gas or losing your driver's license, along with bonus opportunities for special passengers. The Destination brand has spawned multiple city-specific versions.

## Components

- 1 game board depicting a map of London with connected routes
- Destination cards (tourist attraction locations to visit)
- Event/action cards
- Player taxi pawns
- Dice
- Play money
- Rules booklet

## Setup

1. Place the board in the center of the table.
2. Each player chooses a taxi pawn and a starting location.
3. Deal destination cards to each player -- these are the tourist attractions you must visit.
4. Distribute starting money to each player.
5. Shuffle and place event cards as indicated.

## Turn Structure

On your turn:
1. **Roll the Die:** Roll and move your taxi along the routes on the board.
2. **Resolve Location:** If you land on one of your destination card locations, reveal the card and collect the fare.
3. **Draw Event (if applicable):** Some spaces trigger event card draws with special effects.
4. **Return Home:** After visiting all destinations, race back to your starting point.

## Actions

### Moving
- Roll the die and move your taxi along connected routes on the board.
- Navigate through London's streets toward your destination card locations.

### Visiting Destinations
- When you arrive at a location matching one of your destination cards, reveal the card and earn the indicated fare money.

### Event Cards
- Landing on certain spaces or drawing event cards can trigger random events -- some beneficial (bonus passengers, shortcuts), some harmful (running out of gas, losing your license).

### Special Passengers
- Bonus points or money can be earned for carrying special passengers (e.g., Japanese tourists in some editions).

## Scoring / Victory Conditions

The first player to visit all of their destination card locations and return to their starting point wins the game. In some variants, the winner is determined by total money earned across all visited destinations.

## Special Rules & Edge Cases

- **Running Out of Gas:** Some events cause you to run out of fuel, costing you a turn or money.
- **Losing Your License:** A severe penalty that prevents movement for one or more turns.
- **Multiple Editions:** The Destination brand includes London, New York, Paris, and other city variants, all using similar core rules with different maps.
- **Family Friendly:** The game is designed for family play and is straightforward in its mechanics.

## Player Reference

**Turn:** Roll die → Move taxi → Visit destinations (collect fare) → Resolve events

**Win:** Visit all destination cards and return home first

**Hazards:** Running out of gas, losing your license

**Scoring:** Earn fares from visiting destination card locations
