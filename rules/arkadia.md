...
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: Contradicts source: workers never removed after completion, neutral workers earn no seals; invents 2:1 neutral deployment ratio; omits banner's 2-worker gain; misstates end-game trigger. Until it is re-written, prefer the full rulebook text linked below.
>
...
- Cannot overlap other buildings or the castle lot.
...
- Each worker adjacent to the completed building earns its owner 1 seal matching the family color shown on the building card.
- Neutral workers also earn seals, which go to the player who completed the building.
- After earning seals, remove workers from around the completed building and return them to owners.
...
**B) Deploy Workers:**
- Place 1 or more workers from behind your screen onto empty squares adjacent to buildings.
- You may also place neutral workers from the pool (1 neutral for every 2 of your own workers placed).

**Architect Banner:**
- After your mandatory action, you may place 1 architect banner on the board.
- When used, you may convert any number of your seals to gold at their current exchange rate.
- The exchange rate = number of castle pieces of that family's color currently visible on the castle.
- Each banner can only be used once per game (4 total per player).
...
- The game ends when all castle pieces have been placed or a specific trigger occurs.
- Final scoring: all players convert remaining seals at end-game exchange rates.
...
- Neutral workers can be deployed alongside your own workers.
...
|------------|-------------|
| Place building | Must be adjacent to existing building/castle |
| Deploy workers | Place own workers (+ neutrals at 2:1 ratio) |
| Architect banner | Convert seals to gold at current rate (once each) |
