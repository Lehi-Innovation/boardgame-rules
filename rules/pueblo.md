...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Pueblo&game=pueblo)
<!-- verification:end -->
...
- Neutral blocks cover and hide colored blocks effectively.
- Height matters: blocks visible from the top of the pueblo score more penalties.
- The Chieftain's viewpoint changes based on position, so blocks may be hidden from one angle but visible from another.
- Players must consider both current and future Chieftain positions when placing blocks.
