...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rise%21&game=rise)
<!-- verification:end -->
...
- Worker pieces in two player colors
- Tower pieces (3 per player, each with 3 layers)
- Starting tile
...
- Tower rings: The six surrounding workers must all be the same color (the building player's color).
- A tower layer is only added at the start of the building player's turn if the ring is still intact.
- If a ring is broken and later reformed, construction resumes from where it left off.
- Land tiles can only be placed adjacent to existing tiles, gradually expanding the board.
- Workers on the board edge have fewer adjacencies, making them harder to surround but also harder to maintain rings near edges.
