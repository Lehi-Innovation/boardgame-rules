...
player_count: "2"
play_time: "30-60 min"
designer: "Uwe Rosenberg, Hagen Dorgathen"
source_pdf: "babel-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/babel-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Babel&game=babel)
<!-- verification:end -->
...
- 1 board with 5 construction sites per player and a quarry for Temple cards
- 45 Temple cards (10x level 1, 9x level 2, 8x level 3, 7x level 4, 6x level 5, 5x level 6)
- 60 Nation cards (12 per nation): Assyrians, Hittites, Medes, Persians, Sumerians
- 2 building pawns

## Setup

1. Place the board in the middle of the table.
2. Each player takes 1 level 1 Temple card (must be built on their first turn).
3. Shuffle remaining Temple cards; place face-down in the Quarry.
4. Each player places their pawn on their side of the Quarry.
5. Shuffle Nation cards. Determine a start player.
6. Start player draws **3 Nation cards**; opponent draws **5 Nation cards**.
7. Remaining Nation cards form the face-down draw pile.

## Turn Structure

Each turn has 3 phases performed in order:

### Phase 1: Draw 3 Nation Cards
...
### A) Move Building Pawn
- Discard a Nation card whose colour matches the target construction site.
- Both pawns may occupy the same site.
- First move is from the Quarry to a site.

### B) Deploy Nation Cards
- Place Nation card(s) from hand on top of previously deployed cards at pawn's current site.
- Cards must be visible to both players (stacked so names remain visible).
- Multiple cards may be deployed at the same site in one turn.

### C) Construct a Temple
- Take the top Temple card from **either** temple stack.
- Place it at your pawn's current construction site.
- Temples must be built in numerical sequence (1 → 2 → 3 → 4 → 5 → 6).
- **Requirement:** The number of your Nation cards at the site must be **greater than or equal to** the Temple card level being built.
- Once built, temple levels are not affected by removal of Nation cards.

### D) Migrate
- Once per turn only.
- Move exactly 3 of your Nation cards from one site to another.
- Only the **top 3** cards at the source site are eligible.
- Moved cards are placed on top at the destination site, maintaining their order.
- Your pawn does not need to be at either site.

### E) Nation Special Abilities
...
1. One player's temples reach a combined value of **20 or more**, OR
2. One player's temples drop to a combined value of **9 or less**.
...
- **First turn obligation:** Each player must construct their starting level 1 Temple card on their first turn. This is the only card with this restriction.
- **Temple construction is permanent:** Once a temple level is built, removing Nation cards from that site does not affect the temple.
- **Migration precision:** Exactly 3 cards must be moved; never more or less.
- **Special ability card counts:** For Hittites and Persians abilities, the discarded card counts toward the Nation card requirement.
- **Destroyed temple cards:** Return to the top of the Temple draw pile (not the discard pile), in descending order.
- **Hittites steal on empty site:** You may steal a Temple card even if your construction site has no temple yet.
- **Hand disclosure:** At the end of your turn, if you have 4+ Nation cards, you must tell your opponent how many cards you hold.
...
**Game End Triggers:**
- Player A ≥ 15 AND Player B ≤ 9 → immediate end
- Player A ≥ 15 AND Player B ≥ 10 → enter end phase (end at 20+ or ≤ 9)
- Temple draw pile empty → immediate end
