...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rat%20Hot&game=rat-hot)
<!-- verification:end -->
...
1. Shuffle all tiles face-down to form a draw pile.
2. Determine the starting player.
3. Leave an open area on the table for the tile layout.

## Turn Structure

On each turn:

1. **Draw Tiles:** Draw 2 tiles from the face-down pile.
2. **Place Tiles:** Add the drawn tiles to the existing layout, one at a time. Tiles are placed so they partially overlap previous tiles in the layout. Each 3x1 tile covers parts of the tiles underneath.
3. **Check for Rats:** After placement, check if any dangerous rat configurations are visible. If so, the active player loses immediately.
...
| Action | Description |
|---|---|
| Draw 2 Tiles | Take 2 tiles from the face-down draw pile |
| Place Tiles | Add tiles to the layout, partially overlapping existing tiles |
| Rats Visible | Too many rats visible = instant loss for active player |
| Spice Combos | Visible hot spice combinations score points |
