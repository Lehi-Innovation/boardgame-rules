...
play_time: "30-60 min"
designer: "Unknown"
source_pdf: "rapier-dagger-rules.pdf"
...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Rapier%20%26%20Dagger&game=rapier-dagger)
<!-- verification:end -->
...
1. Each player creates a character using the character sheet, selecting a fencing school and weapon combination.
2. Players determine their combatant's attributes including skill, endurance, and fighting style.
3. If using miniatures, place figures on the play surface at starting distance. If using pencil-and-paper, record starting positions.
4. Prepare the reference sheet for quick lookup of combat results during play.

## Turn Structure

Each turn represents a short interval of fencing action and follows this sequence:

1. **Initiative Phase:** Both players simultaneously declare their intended action (attack, parry, riposte, retreat, etc.).
2. **Resolution Phase:** Actions are resolved using the combat tables, cross-referencing the attacker's weapon and school against the defender's chosen action.
3. **Damage Phase:** Successful hits are resolved, reducing the opponent's endurance or causing specific wound effects.
4. **Position Update:** Update positions of combatants (advance, retreat, sidestep) based on the actions taken.
...
| Action | Effect |
|---|---|
| Attack | Strike at opponent; resolved on combat table |
| Parry | Deflect incoming attack |
| Riposte | Counterattack after successful parry |
| Retreat | Move away from opponent |
| Advance | Close distance with opponent |
| Feint | Deceptive action to draw parry |
