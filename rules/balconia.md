...
>
> 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Balconia&game=balconia)
<!-- verification:end -->
...
1. Place the building frame between the two players, one on each side.
2. Shuffle or randomly arrange the 15 building blocks face-down.
3. Determine who goes first (the "Chooser" for the first turn).

## Turn Structure

Each turn involves two roles: the **Chooser** and the **Placer**.

1. The Chooser reveals a building block.
2. The Chooser selects which two adjacent sides of the block will be the scoring sides (one facing each player).
3. The Placer picks one of those two orientations and places the block in the grid, adjacent to previously placed blocks.
4. Roles alternate each turn.
...
**Turn roles:**
1. Chooser: Reveals block, picks 2 adjacent sides to offer
2. Placer: Picks orientation, places in grid
