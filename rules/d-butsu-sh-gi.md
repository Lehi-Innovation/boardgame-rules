...
player_count: "2"
play_time: "5-15 min"
source_pdf: "d-butsu-sh-gi-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/d-butsu-sh-gi-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Dobutsu%20shogi&game=d-butsu-sh-gi)
<!-- verification:end -->
...
- 1 game board (3x4 grid)
- 8 animal pieces (4 per player in two colors):
  - 2 Lions (king equivalent)
  - 2 Elephants (diagonal mover)
  - 2 Giraffes (orthogonal mover)
  - 2 Chicks (forward only; promotes to Hen)

## Setup

Each player places their 4 pieces on their nearest row (rank 1) and second row (rank 2):
- Back row (nearest): Giraffe, Lion, Elephant (left to right from that player's perspective)
- Front row: Chick in center column
...
- All piece movements are indicated on the pieces themselves with dots.
- Captured pieces change color/allegiance when dropped.
- A Hen reverts to a Chick when captured.
- You cannot drop a Chick onto the farthest rank (it would have no legal moves).
- Advancing Lion to far rank while in check does not count as a win.
