---
title: "Amerigo"
bgg_id: 137408
player_count: "2-4"
play_time: "90 min"
designer: "Stefan Feld"
source_pdf: "amerigo-rules.pdf"
extracted_date: "2026-03-18"
summarized_date: "2026-03-18"
verification: "inaccurate"
verification_date: "2026-06-12"
---
<!-- verification:begin -->
> ❗ **Known errors** — an audit found inaccuracies in this summary that could mislead players: 4 of 7 cube-color actions mis-mapped, invented production wheel and trade/produce actions, action structure wrong, end scoring omits gold/tracks and invents cannon scoring. Until it is re-written, prefer the full rulebook text linked below.
>
> 📄 [Full rulebook text](https://jonnyallred.github.io/boardgame-rules/extracted/amerigo-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Amerigo&game=amerigo)
<!-- verification:end -->


## Overview

Amerigo is a strategy game where players explore islands, build settlements, and collect goods in the New World. The game features a unique cube tower mechanism that determines available actions each round. Players sail ships to discover islands, place landscape tiles to settle them, and trade goods for victory points. After 5 rounds, the player with the most points wins.

## Components

- 1 Game board (ocean grid with islands)
- 1 Cube tower
- Action cubes in 7 colors (each color = different action type)
- Player boards
- Ships, settlement markers
- Landscape tiles in various shapes
- Goods tokens
- Progress tokens
- Production wheel
- Victory point track
- Pirate tiles
- Cannons

## Setup

1. Place the game board centrally. Each player takes a player board, ships, and settlement markers.
2. Load the cube tower with an initial set of cubes.
3. Place landscape tiles, goods, and progress tokens in designated areas.
4. Set up the production wheel.
5. Choose starting positions for player ships.

## Turn Structure

The game is played over 5 rounds, each with 7 phases. Each phase:

1. Drop cubes of the current phase's color into the cube tower.
2. Cubes that fall through determine the strength (number) of available actions.
3. Each player takes actions equal to the number of cubes that fell.

## Actions

Actions depend on the color of cubes that emerge from the tower:

| Cube Color | Action |
|-----------|--------|
| Blue | Move ships across the ocean |
| Green | Place landscape tiles on islands |
| Red | Buy progress tokens (special abilities) |
| Black | Build cannons for pirate defense |
| White | Produce goods from settled islands |
| Yellow | Plan/store actions for later use |
| Brown | Trade goods for victory points |

### Exploration and Settlement

- Move ships to reach new islands.
- Place tetromino-like landscape tiles to claim island spaces.
- Complete islands (fill all spaces) for bonus points.

### Production and Trade

- Produce goods from islands where you have settlements.
- Trade sets of goods for victory points.

### Pirates

- At certain points, pirates attack. Players must have enough cannons to defend or lose points.

## Scoring / Victory Conditions

After 5 rounds, players score:
- Victory points from landscape tile placement
- Completed island bonuses
- Goods traded
- Progress token bonuses
- Remaining cannons
- Penalties for undefended pirate attacks

Highest total wins.

## Special Rules & Edge Cases

- The cube tower is the central randomizer -- cubes dropped in may be retained by the tower (caught on internal ledges) and fall out in later rounds, creating unpredictability.
- Players share the same pool of available action cubes each phase.
- Landscape tiles must fit legal positions on islands.
- Progress tokens provide permanent advantages.
- Some islands produce specific goods; others are wild.

## Player Reference

**Round Structure:** 7 phases per round, 5 rounds total.

**Phase Flow:** Drop cubes → Count fallen cubes → All players take that many actions of that type.

**Key Actions:** Move ships (blue), Place tiles (green), Buy progress (red), Build cannons (black), Produce (white), Plan (yellow), Trade (brown).
