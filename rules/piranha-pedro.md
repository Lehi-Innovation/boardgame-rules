...
source_pdf: "piranha-pedro-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/piranha-pedro-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Piranha%20Pedro&game=piranha-pedro)
<!-- verification:end -->
...
1. Put all seven piranhas on the marked fields.
2. Put Pedro on the palmtree field.
3. Put a landscape card on each side of the game board.
4. One player takes the sombrero card.
5. Each player takes a set of move cards (each set contains 12 cards with the same backside color) in hand.
6. Each player gets a private stock of 4 stones.

## Turn Structure

### Move Round
1. Each player chooses one of their hand cards and puts it face-down in front of them.
2. The player with the sombrero card shows their card first and **moves Pedro** the number of fields shown on the move card (1, 2, or 3 arrows) in the indicated direction.
   - **Note:** Before Pedro can step on a water field, the player must place one of their stones on it.
3. After moving Pedro, the player shows the card they played (placed face-down half under the game board).
4. The sombrero card is passed to the left neighbor and a new move round begins.
...
| Cards in hand | Stones received |
|--------------|-----------------|
| Every single move card with 1 arrow | 1 stone each |
| Every move card with 2 arrows | No stones |
| Every move card with 3 arrows | No stones |
...
| Card Type | Movement | Stones from evaluation |
|-----------|----------|----------------------|
| 1 arrow | 1 space | 1 stone per card in hand |
| 2 arrows | 2 spaces | 0 stones |
| 3 arrows | 3 spaces | 0 stones |
