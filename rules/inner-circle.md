...
player_count: "2-4"
play_time: "30 min"
designer: "Virginia Charves, Wayne J. Yee"
source_pdf: "inner-circle-rules.pdf"
...
>
> 📄 [Full rulebook text](https://lehi-innovation.github.io/boardgame-rules/extracted/inner-circle-rules.txt) · 🚩 [Report a rules error](https://github.com/Lehi-Innovation/boardgame-rules/issues/new?template=rule-error.yml&labels=rule-error&title=%5BRule%20error%5D%20Inner%20Circle&game=inner-circle)
<!-- verification:end -->
...
- 1 Gameboard with 4 stackable hexagonal levels (each level progressively smaller)
- Pawns in 4 player colors
- Dots on the underside of pawn positions indicating movement values

## Setup

1. Peel off the three rubber feet and attach them to the plastic gameboard unit.
2. Punch out the four cardboard gameboard levels. Stack them: place the largest, then the slightly larger blue/green board on top (right on top), then the smallest, then finally the largest yellow/blue gameboard right on top.
3. Boards may be turned in any direction as long as they are face-up.
4. Each player is assigned pawns by player count:
   - **2 players:** 9 pawns each (6 red or 6 green)
   - **3 players:** 6 pawns each
   - **4 players:** 4 pawns each
5. Players take turns placing **one pawn at a time** on the spaces around the perimeter of the outermost level (1st Level Gameboard).
6. The first player to go first is the one who places the first pawn; play continues clockwise.

## Turn Structure

1. **Placement Phase:** Players alternate placing pawns one at a time on the outer edge of the 1st Level Gameboard.
2. **Movement Phase:** Once all pawns are placed, players take turns moving pawns.
...
| Level | Description |
|-------|-------------|
| 1st (outer) | Largest board; most Survival Spots |
| 2nd | Smaller board |
| 3rd | Even smaller |
| 4th (inner) | Single central Survival Spot; landing here wins |
